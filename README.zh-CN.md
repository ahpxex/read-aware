...
- **换台设备，接着读。** 通过 ReadAware 同步中继或自己的 WebDAV 服务器，
  端到端加密同步书籍、阅读进度、批注、对话和记忆。
...
跨设备同步现已可用。书籍、阅读进度、高亮、笔记、对话和记忆都可以在设备间
同步。支持漫游的应用、AI 和插件偏好也会同步；阅读排版等设备专属设置保留在
本机。你可以选择加密数据存放在哪里：

- **ReadAware 同步中继。** 在「设置 > 数据与同步」连接 ReadAware 账号，
  存储额度与方案见[定价页](https://readaware.app/zh/pricing/)。
- **自己的 WebDAV 服务器。** 从插件市场安装 **WebDAV Sync**，在插件设置中
  填写服务器地址、用户名、密码（或应用专用密码）和文件夹，再到「设置 >
  数据与同步」连接 WebDAV。数据存储在你的 WebDAV 服务上，而非 ReadAware
  中继，不需要 ReadAware 同步账号。

两种方式使用同一套同步引擎：数据在设备上加密后才上传，远端只存储密文。
连接同一账号或 WebDAV 文件夹的设备使用同一个加密口令。请妥善保管口令，
遗忘后无法找回。WebDAV 登录密码与数据加密口令是两回事。

ReadAware 依然本地优先：离线也能阅读和记笔记，重新联网后合并变更；断开同步
不会删除本地数据。可选的 AI 推理与同步加密是独立的：相关阅读上下文会发送给
你选择的模型服务商。
...
└── 同步存储              ReadAware 中继或 WebDAV；加密事件与 blob
```
