...
function GettingStartedPage() {
  return (
    <article className="doc-prose">
      ...
      <p>
        同一处外观设置也管排版：字体、字号、字重、行距、段距、页边距和页面配色。对齐方式默认是
        <strong>遵从原书</strong>——书自己的样式表要求什么就是什么；如果你希望处处一致，也可以强制左对齐或两端对齐。
      </p>

      <h2>朗读</h2>
      <p>
        任何一本书都可以朗读。朗读搭在你阅读时用的逐句／逐段导航之上——启动导航，再按下导航条上的播放，书就会一个单元一个单元地推进，正在朗读的文字会被标出。
      </p>
      <p>
        开箱即用时它用你设备自带的语音，不需要密钥，也不需要联网。启用一个语音插件——内置的
        <strong>TTS Voices</strong>
        插件，或是其他任何一个——这个启用动作本身就是「改用该引擎朗读」的选择；具体用哪个声音，在那个插件自己的设置里配置，和它的提供方、自定义端点在一起。没有另一个需要同步的语音选择器。
      </p>
