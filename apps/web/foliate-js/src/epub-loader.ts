import type { MaybePromise, ResourceTransformDetail } from './book.js'
import type { Archive, ManifestItem, Resources } from './epub-resources.js'
import { NS, MIME, isExternal, resolveURL } from './epub-dom.js'

export type ResourceLoadDetail = { type: string; isScript: boolean; allow: MaybePromise<boolean> }
...
const replaceSeries = async (text: string, pattern: RegExp,
    replace: (match: RegExpExecArray) => MaybePromise<string>): Promise<string> => {
...
const pathRelative = (from: string, to: string): string => {
    ...
    return index < 0 ? '' : [...Array<string>(a.length - index).fill('..'), ...b.slice(index)].join('/')
}
const pathDirname = (path: string) => path.slice(0, path.lastIndexOf('/') + 1)
const regexEscape = (text: string) => text.replace(/[-/\\^$*+?.()|[\]{}]/g, '\\$&')
const isDocumentType = (type: string): type is 'application/xhtml+xml' | 'text/html' | 'image/svg+xml' =>
    type === MIME.XHTML || type === MIME.HTML || type === MIME.SVG
...
export class Loader {
    ...
    constructor(private readonly archive: Pick<Archive, 'loadText' | 'loadBlob'>,
        private readonly resources: Pick<Resources, 'manifest'>) {}

    async createURL(href: string, data: MaybePromise<string | Blob | null | undefined>,
        type: string, parent?: string): Promise<string | null> {
        const originalData = Promise.resolve(data).then(value => {
                if (value == null) throw new MissingResourceError(`Missing EPUB resource: ${href}`)
                return value
            })
        ...
        const detail: ResourceTransformDetail = { name: href, type, data: originalData }
        Object.defineProperty(detail, 'name', { writable: false })
        this.eventTarget.dispatchEvent(new CustomEvent<ResourceTransformDetail>('data', { detail }))
        let transformed: [string | Blob, string]
        try { transformed = await Promise.all([detail.data, detail.type]) }
        catch (error) {
        ...
        if (this.#destroyed) throw new Error('EPUB resources were closed')
        // Concurrent section loads share resources, including their ownership.
        const existing = this.#cache.get(href)
        if (existing) { this.#ref(href, parent); return existing }
    ...
    #unref(href: string) {
        ...
        if (previous > 1) { this.#refCount.set(href, previous - 1); return }
        ...
        if (children) for (const child of children) this.#unref(child)
    }
    async loadItem(item?: ManifestItem, parents: string[] = []): Promise<string | null> {
        ...
        if (parents.includes(href)) return null
        const isScript = MIME.JS.test(mediaType)
        const detail: ResourceLoadDetail = { type: mediaType, isScript, allow: true }
        this.eventTarget.dispatchEvent(new CustomEvent<ResourceLoadDetail>('load', { detail }))
        if (!await detail.allow) return null
        const parent = parents.at(-1)
        const cached = this.#cache.get(href)
        if (cached) { this.#ref(href, parent); return cached }
    ...
    async #loadHref(href: string, base: string, parents: string[]): Promise<string> {
        if (!href || href.startsWith('#') || isExternal(href)) return href
    ...
    async #loadReplaced(item: ManifestItem, parents: string[]): Promise<string | null> {
        ...
        try { text = await this.archive.loadText(href) }
        catch (error) {
        ...
        if (text == null) return this.createURL(href, text, mediaType, parent)
        if (isDocumentType(mediaType)) {
            ...
            for (const node of Array.from(doc.childNodes)) {
                ...
                const replaced = await replaceSeries(data, /(?:^|\s*)(href\s*=\s*['"])([^'"]*)(['"])/i,
                    async ([, a, url, b]) => `${a}${await this.#loadHref(url, href, parents)}${b}`)
            ...
            const replace = async (element: Element, attribute: string) => {
                ...
                if (value != null) element.setAttribute(attribute, await this.#loadHref(value, href, parents))
            }
            for (const [selector, attribute] of [['link[href]', 'href'], ['[src]', 'src'],
                ['[poster]', 'poster'], ['object[data]', 'data']] as const)
                for (const element of doc.querySelectorAll(selector)) await replace(element, attribute)
            for (const element of doc.querySelectorAll('[*|href]:not([href])')) {
                const value = element.getAttributeNS(NS.XLINK, 'href')
                if (value != null) element.setAttributeNS(NS.XLINK, 'xlink:href', await this.#loadHref(value, href, parents))
            }
            for (const element of doc.querySelectorAll('[srcset]')) element.setAttribute('srcset',
                await replaceSeries(element.getAttribute('srcset') ?? '',
                    /(\s*)(.+?)\s*((?:\s[\d.]+[wx])+\s*(?:,|$)|,\s+|$)/g,
                    async ([, a, url, b]) => `${a}${await this.#loadHref(url, href, parents)}${b}`))
            for (const element of doc.querySelectorAll('style'))
                if (element.textContent) element.textContent = await this.#replaceCSS(element.textContent, href, parents)
            for (const element of doc.querySelectorAll('[style]')) element.setAttribute('style',
                await this.#replaceCSS(element.getAttribute('style') ?? '', href, parents))
    ...
    async #replaceCSS(text: string, href: string, parents: string[]) {
        const urls = await replaceSeries(text, /url\(\s*["']?([^'"\n]*?)\s*["']?\s*\)/gi,
            async ([, url]) => `url("${await this.#loadHref(url, href, parents)}")`)
        return replaceSeries(urls, /@import\s*["']([^"'\n]*?)["']/gi,
            async ([, url]) => `@import "${await this.#loadHref(url, href, parents)}"`)
    }
    async #replaceString(text: string, href: string, parents: string[]) {
        const assets = new Map<string, ManifestItem>()
        for (const asset of this.resources.manifest) {
            ...
            for (const path of [relative, encodeURI(relative), root, encodeURI(root)]) assets.set(path, asset)
        }
        if (!assets.size) return text
        const pattern = new RegExp(Array.from(assets.keys(), regexEscape).join('|'), 'g')
        return replaceSeries(text, pattern, async ([match]) =>
            await this.loadItem(assets.get(match), [...parents, href]) ?? '')
    }
    unloadItem(item: ManifestItem) { this.#unref(item.href) }
    destroy() {
        this.#destroyed = true
