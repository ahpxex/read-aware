...
const partsToNode = (root: Element, parts: CFIStep[], filter?: CFIFilter): Boundary => {
    ...
    for (const { index } of parts) {
        ...
        const newNode: IndexedNode = indexed[index]
        // handle non-existent nodes
        if (newNode === 'before') return { node, offset: 0 }
        if (newNode === 'after') return { node, offset: node.childNodes.length }
        if (newNode === null || newNode === 'first' || newNode === 'last') {
            ...
            if (!adjacent?.parentNode) return { node, offset: 0 }
...
const boundaryToParts = (node: Node, offset: number, filter?: CFIFilter): CFIStep[] => {
    if (isTextNode(node)) return nodeToParts(node, offset, filter)
    const doc = node.ownerDocument
    if (!doc) throw new Error('Cannot create a CFI without a content document')
    ...
    if (next && isTextNode(next)) return nodeToParts(next, 0, filter)
    if (previous && isTextNode(previous)) return nodeToParts(previous, previous.length, filter)
    const adjacent = next ?? previous
    if (!adjacent) return nodeToParts(node, null, filter).concat({ index: 1, offset: 0 })
    const parts = nodeToParts(adjacent, null, filter)
    const last = parts.at(-1)
    if (!last) throw new Error('Cannot create a CFI for an ignored boundary')
