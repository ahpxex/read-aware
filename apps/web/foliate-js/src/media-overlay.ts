import type { BookSection, MaybePromise } from './book.js'
import { NS, childGetter, parseClock, resolveURL } from './epub-dom.js'

export type MediaOverlayItem = { text: string; begin: number; end?: number }
type AudioEntry = { src: string; items: MediaOverlayItem[] }
type ItemFilter = (item: MediaOverlayItem, index: number, items: MediaOverlayItem[]) => boolean
type OverlayBook = { sections: BookSection[]; loadBlob: (href: string) => MaybePromise<Blob | null | undefined> }
type LoadXML = (href: string) => Promise<Document | null>
export type OverlayAudio = EventTarget & Pick<HTMLAudioElement,
    'src' | 'volume' | 'playbackRate' | 'currentTime' | 'paused' | 'play' | 'pause'>

export const parseSMIL = (doc: Document, href: string): AudioEntry[] => {
    ...
    for (const par of $$$(doc, 'par')) {
        ...
        if (previous?.src === resolved) previous.items.push(item)
        else entries.push({ src: resolved, items: [item] })
    }
    return entries
}
...
export class MediaOverlay extends EventTarget {
    #entries: AudioEntry[] = []
    #lastSMIL: string | undefined
    #sectionIndex = -1
    #audioIndex = -1
    #itemIndex = -1
    #audio: OverlayAudio | undefined
    #volume = 1
    #rate = 1
    #state: 'playing' | 'paused' | 'stopped' = 'stopped'
    #generation = 0
    constructor(readonly book: OverlayBook, readonly loadXML: LoadXML,
        private readonly createAudio: (url: string) => OverlayAudio = url => new Audio(url)) {
        super()
    }
    ...
    #error(error: unknown) {
        ...
        this.dispatchEvent(new CustomEvent<unknown>('error', { detail: error }))
    }
    #highlight() {
        this.dispatchEvent(new CustomEvent<MediaOverlayItem | undefined>('highlight', { detail: this.#activeItem }))
    }
    #unhighlight() {
        this.dispatchEvent(new CustomEvent<MediaOverlayItem | undefined>('unhighlight', { detail: this.#activeItem }))
    }
    async #play(audioIndex: number, itemIndex: number): Promise<void> {
        const generation = ++this.#generation
        this.#stopAudio()
        if (itemIndex >= (this.#entries[audioIndex]?.items.length ?? 0)) { audioIndex++; itemIndex = 0 }
        this.#audioIndex = audioIndex
        this.#itemIndex = itemIndex
        const entry = this.#activeAudio, item = this.#activeItem
        if (!entry || !item) return this.start(this.#sectionIndex + 1)
        const blob = await this.book.loadBlob(entry.src)
        if (generation !== this.#generation) return
        if (!blob) throw new Error(`Missing EPUB media audio: ${entry.src}`)
        ...
        audio.addEventListener('timeupdate', () => {
            ...
            while (entry.items[this.#itemIndex + 1]?.begin <= time) this.#itemIndex++
            if (this.#itemIndex !== previous) this.#highlight()
        })
        audio.addEventListener('error', () => {
            if (audio === this.#audio) this.#error(new Error(`Failed to load ${entry.src}`))
        })
    ...
    async #start(sectionIndex: number, filter: ItemFilter, backwards = false): Promise<void> {
        const generation = ++this.#generation
        this.#stopAudio()
        ...
        for (let index = sectionIndex; index >= 0 && index < this.book.sections.length; index += step) {
            ...
            if (href !== this.#lastSMIL) {
                ...
                if (generation !== this.#generation) return
                if (!doc) throw new Error(`Missing EPUB media overlay: ${href}`)
                this.#entries = parseSMIL(doc, href)
                this.#lastSMIL = href
            }
            this.#sectionIndex = index
            const audioIndices = Array.from(this.#entries.keys())
            if (backwards) audioIndices.reverse()
            for (const i of audioIndices) {
                const { items } = this.#entries[i]
                const itemIndices = Array.from(items.keys())
                if (backwards) itemIndices.reverse()
                for (const j of itemIndices) if (items[j].text.split('#')[0] === section.id && filter(items[j], j, items))
