...
export type RectMapper = (rect: DOMRect) => { left: number; right: number }
...
export const uncollapse = (range: Element | Range): Element | Range | null => {
    ...
    if (endOffset + 1 < (endContainer.textContent?.length ?? 0)) range.setEnd(endContainer, endOffset + 1)
    else if (endOffset > 1) range.setStart(endContainer, endOffset - 1)
    else return endContainer.parentElement
    return range
}
...
export const getVisibleRange = (doc: Document, start: number, end: number, mapRect: RectMapper): Range => {
...
export const getDirection = (doc: Document) => {
    ...
    if (!defaultView) throw new Error('Page document has no window')
...
export const getBackground = (doc: Document): string => {
    if (!doc.defaultView) throw new Error('Page document has no window')
...
export const makeMarginals = (length: number, part: string) => Array.from({ length }, () => {
...
export const setStylesImportant = (el: HTMLElement | SVGElement, styles: Record<string, string>) => {
    const { style } = el
