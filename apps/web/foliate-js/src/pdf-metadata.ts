import type { BookMetadata, LanguageMap, LocalizedText } from './book.js'
import type { PDFMetadata } from './vendor/pdfjs/pdf.mjs'

const languageMap = (value: unknown): value is LanguageMap => value !== null
    && typeof value === 'object' && !Array.isArray(value)
    && Object.values(value).every((item: unknown) => typeof item === 'string')
const localized = (value: unknown): LocalizedText | undefined =>
    typeof value === 'string' || languageMap(value) ? value : undefined
const text = (value: unknown): string | undefined => typeof value === 'string' ? value : undefined
const strings = (value: unknown): string | string[] | undefined => {
    if (typeof value === 'string') return value
    if (Array.isArray(value) && value.every((item: unknown) => typeof item === 'string')) return value
}

export const getPDFMetadata = (metadata: PDFMetadata | null, info: Record<string, unknown>): BookMetadata => ({
