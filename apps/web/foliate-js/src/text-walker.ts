...
const walkRange = (range: Range, walker: TreeWalker): Node[] => {
    ...
    for (let node: Node | null = walker.currentNode; node; node = walker.nextNode()) {
        ...
        if (compare > 0) break
        if ((node.nodeType === 3 || node.nodeType === 4) && range.intersectsNode(node)) nodes.push(node)
    }
    return nodes
}

const walkDocument = (walker: TreeWalker): Node[] => {
    const nodes: Node[] = []
...
export function* textWalker<T>(x: Node | Range, func: TextProcessor<T>, filterFunc?: (node: Node) => number): Generator<T> {
    ...
    const strs = nodes.map((node, index) => (node.nodeValue ?? '').slice(offsets[index],
