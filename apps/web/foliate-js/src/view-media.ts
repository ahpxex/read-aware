...
import type { MediaOverlayItem } from './media-overlay.js'
import type { Content } from './renderer.js'
...
export class ViewMedia {
    ...
    #generation = 0
    #active: WeakRef<Element> | undefined
    constructor(private readonly book: Book,
        navigate: (href: string) => Promise<ResolvedNavigation | undefined>,
        contents: () => Content[]) {
        this.overlay = book.getMediaOverlay?.()
        this.overlay?.addEventListener('highlight', event => {
            this.#clear()
            const generation = ++this.#generation
            const item = (event as CustomEvent<MediaOverlayItem | undefined>).detail
            if (!item) return
            void navigate(item.text).then(resolved => {
                ...
                if (playbackActiveClass) element.ownerDocument.documentElement.classList.add(playbackActiveClass)
                this.#active = new WeakRef(element)
            }).catch((error: unknown) => console.error('Could not highlight media overlay', error))
    ...
    #clear() {
        ...
        if (playbackActiveClass) element.ownerDocument.documentElement.classList.remove(playbackActiveClass)
    }
    destroy() {
        this.#generation++
        this.overlay?.stop()
