...
const hasSignature = async (file, signature) => {
...
export const getFileEntries = async (entry) => {
...
const makeDirectoryLoader = async (entry) => {
    ...
    const files = await Promise.all(entries.map(async (child) => {
        ...
        return [child.fullPath.slice(entry.fullPath.length + 1), file];
    }));
    const map = new Map(files);
    return {
        loadText: (name) => map.get(name)?.text() ?? null,
        loadBlob: (name) => map.get(name) ?? null,
...
export const makeBook = async (input) => {
    ...
    if (await hasSignature(file, [0x50, 0x4b, 0x03, 0x04])) {
        ...
        if (file.type === 'application/x-zip-compressed-fb2' || hasExtension(file, '.fb2.zip', '.fbz')) {
            ...
            if (!blob)
                throw new NotFoundError('No FictionBook document in archive');
            return makeFB2(blob);
        }
        const { EPUB } = await import('./epub.js');
