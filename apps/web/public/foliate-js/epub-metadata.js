...
const getPrefixes = (doc) => {
    ...
    if (value)
        for (const [, prefix, url] of value.matchAll(/([^\s:]+):\s+(\S+)/g))
            map.set(prefix, url);
    return map;
};
...
const one = (values) => values?.[0]?.value;
const prop = (value, name) => one(value.props[name]);
const oneOrMany = (values) => !values?.length ? undefined : values.length === 1 ? values[0] : values;
export const getMetadata = (opf) => {
    const { $ } = childGetter(opf, NS.OPF);
    const element = $(opf.documentElement, 'metadata');
    if (!element)
        throw new Error('EPUB package has no metadata element');
    const children = Object.groupBy(Array.from(element.children), el => el.namespaceURI === NS.DC ? 'dc'
    ...
    const getProperties = (parent, ancestors = new Set()) => {
        ...
        for (const el of refines.get(parent ? '#' + id : null) ?? []) {
            ...
            if (parsed.property)
                (properties[parsed.property] ??= []).push(parsed);
        }
        return properties;
    };
    const parse = (el, ancestors = new Set()) => {
    ...
    const dc = {};
    for (const el of children.dc ?? [])
        (dc[el.localName] ??= []).push(parse(el));
    const properties = getProperties();
    const legacy = new Map((children.legacy ?? []).map(el => [el.getAttribute('name'), el.getAttribute('content')]));
    const localized = (value) => {
        if (!value)
            return;
        const alts = value.props['alternate-script'] ?? [];
        const altRep = value.attrs['alt-rep'];
        if (!alts.length && (!value.lang || value.lang === baseLang) && !altRep)
            return value.value;
        const map = { [value.lang ?? baseLang]: value.value };
        if (altRep)
            map[value.attrs['alt-rep-lang'] || baseLang] = altRep;
        for (const alt of alts)
            map[alt.lang ?? baseLang] ??= alt.value;
        return map;
    };
    const roles = (value) => value.props.role?.filter(role => role.scheme === PREFIX.marc + 'relators')
    ...
    const contributor = (value) => {
        ...
        if (typeof name === 'string' && !sortAs && !role && !code && !scheme)
            return name;
    ...
    const collections = Object.groupBy(properties['belongs-to-collection'] ?? [], item => prop(item, 'collection-type') === 'series' ? 'series' : 'collection');
    ...
    for (const [items, fallback] of [[dc.creator, 'author'], [dc.contributor, 'contributor']]) {
    ...
    for (const [key, values] of Object.entries(properties)) {
        if (key.startsWith(PREFIX.rendition))
            rendition[camel(key.slice(PREFIX.rendition.length))] = one(values);
        else if (key.startsWith(PREFIX.media))
            mediaProperties[camel(key.slice(PREFIX.media.length))] = one(values);
    }
    const media = { ...mediaProperties, duration: parseClock(mediaProperties.duration) };
