...
export class Resources {
    ...
    constructor(opf, resolveHref) {
        ...
        if (!manifest || !spine)
            throw new Error('EPUB package requires a manifest and spine');
        ...
        if (guide)
            this.guide = $$(guide, 'reference').map(el => {
    ...
    getItemByID(id) { return id ? this.manifestById.get(id) : undefined; }
    getItemByHref(href) { return this.manifest.find(item => item.href === href); }
    getItemByProperty(property) { return this.manifest.find(item => item.properties.includes(property)); }
    resolveCFI(cfi) {
        const parts = CFI.parse(cfi);
        const top = (Array.isArray(parts) ? parts : parts.parent).shift();
        if (!top?.length)
            throw new Error('EPUB CFI has no package path');
        ...
        if (index < 0)
            throw new Error('EPUB CFI points outside the reading order');
...
const webCryptoSHA1 = async (text) => new Uint8Array(await globalThis.crypto.subtle.digest('SHA-1', new TextEncoder().encode(text)));
export class Encryption {
    sha1;
    #decoders = new Map();
    constructor(sha1 = webCryptoSHA1) {
        this.sha1 = sha1;
    }
    async init(encryption, opf) {
        ...
        const algorithms = {
            ...
            'http://ns.adobe.com/pdf/enc#RC': {
                key: opf => {
                    ...
                    if (!uuid)
                        throw new Error('Adobe font obfuscation requires a UUID identifier');
        ...
        for (const el of encryption.getElementsByTagNameNS(NS.ENC, 'EncryptedData')) {
            ...
            if (!algorithm || !uri)
                throw new Error('Incomplete EPUB encryption descriptor');
            let decoder = decoders.get(algorithm);
            if (!decoder) {
                const implementation = algorithms[algorithm];
                if (!implementation)
                    throw new UnsupportedEncryptionError('EPUB', algorithm);
                const key = await implementation.key(opf);
                if (!key.length)
                    throw new Error('EPUB font obfuscation key is empty');
    ...
    decode(uri, blob) { return this.#decoders.get(uri)?.(blob) ?? blob; }
}
