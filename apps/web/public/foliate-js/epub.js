...
export class EPUB {
    ...
    get resources() {
        if (!this.#resources)
            throw new Error('EPUB has not been initialized');
    ...
    async #init() {
        if (this.#closed)
            throw new Error('EPUB was closed');
        ...
        this.sections = this.resources.spine.map(({ item, linear, properties, cfi }) => ({
            ...
            load: async () => {
                ...
                if (!url)
                    throw new Error(`EPUB section was not loaded: ${item.href}`);
                return url;
            },
        ...
        if (!this.sections.length)
            throw new Error('EPUB has no readable sections');
        const { navPath, ncxPath } = this.resources;
        if (navPath)
            try {
                const doc = await this.#loadXML(navPath);
                if (!doc)
                    throw new Error('Missing EPUB navigation document');
                ...
                console.warn('Could not read EPUB navigation', error);
            }
        if (!this.toc && ncxPath)
            try {
                const doc = await this.#loadXML(ncxPath);
                if (!doc)
                    throw new Error('Missing EPUB NCX document');
                const ncx = parseNCX(doc, href => resolveURL(href, ncxPath));
                this.toc = ncx.toc;
                this.pageList ??= ncx.pageList;
            }
            catch (error) {
                console.warn('Could not read EPUB NCX navigation', error);
            }
        ...
        const display = getDisplayOptions(await this.#loadXML('META-INF/com.apple.ibooks.display-options.xml')
            ?? await this.#loadXML('META-INF/com.kobobooks.display-options.xml'));
        if (display?.fixedLayout === 'true')
    ...
    async loadDocument(item) {
        ...
        if (text == null)
            throw new Error(`Missing EPUB document: ${item.href}`);
    ...
    resolveCFI(cfi) { return this.resources.resolveCFI(cfi); }
    resolveHref(href) {
        const [path, hash] = href.split('#');
        const index = this.sections.findIndex(section => section.id === decodeURI(path));
        if (index < 0)
            return null;
        return { index, anchor: (doc) => hash ? getHTMLFragment(doc, decodeURIComponent(hash)) : 0 };
    }
    splitTOCHref(href) {
        const [path, hash] = href.split('#');
    ...
    isExternal(uri) { return isExternal(uri); }
    async getCover() {
    ...
    async getCalibreBookmarks() {
        ...
        if (text?.startsWith(magic))
            return JSON.parse(atob(text.slice(magic.length)));
    }
    destroy() {
        this.#closed = true;
