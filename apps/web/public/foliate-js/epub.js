...
const getMetadata = opf => {
    ...
    const metadataChildren = Array.from($metadata.children);
    const els = Object.groupBy(metadataChildren, el => el.namespaceURI === NS.DC ? 'dc'
    ...
    for (const [keys, val] of [].concat(dc.creator?.map(makeContributor)?.map(remapContributor('author')) ?? [], dc.contributor?.map(makeContributor)?.map(remapContributor('contributor')) ?? []))
...
const getIdentifier = opf => getElementText(opf.getElementById(opf.documentElement.getAttribute('unique-identifier'))
        ...
        if ([MIME.XHTML, MIME.HTML, MIME.SVG].includes(mediaType)) {
            ...
            if ([MIME.XHTML, MIME.SVG].includes(item.mediaType)) {
                ...
                while (child instanceof ProcessingInstruction) {
                    if (child.data) {
                        const replacedData = await replaceSeries(child.data, /(?:^|\s*)(href\s*=\s*['"])([^'"]*)(['"])/i, (_, p1, p2, p3) => this.loadHref(p2, href, parents)
                            .then(p2 => `${p1}${p2}${p3}`));
            ...
            for (const el of doc.querySelectorAll('[srcset]'))
                el.setAttribute('srcset', await replaceSeries(el.getAttribute('srcset'), /(\s*)(.+?)\s*((?:\s[\d.]+[wx])+\s*(?:,|$)|,\s+|$)/g, (_, p1, p2, p3) => this.loadHref(p2, href, parents)
                    .then(p2 => `${p1}${p2}${p3}`)));
...
${doc.querySelector('parsererror')?.textContent}`);
    ...
    async init() {
        ...
        const opfs = Array.from($container.getElementsByTagNameNS(NS.CONTAINER, 'rootfile'), getAttributes('full-path', 'media-type'))
        ...
        const displayOptions = getDisplayOptions(await this.#loadXML('META-INF/com.apple.ibooks.display-options.xml')
