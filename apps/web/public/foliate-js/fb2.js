...
export const makeFB2 = async (blob) => {
    ...
    const sectionData = bodyData[0][0]
        ...
        urls.push(url);
        const title = normalizeWhitespace(el.querySelector('.title, .subtitle, p')?.textContent
        ...
        return {
            ...
            size: blob.size - Array.from(el.querySelectorAll('[src]'), (el) => el.getAttribute('src')?.length ?? 0)
                .reduce((a, b) => a + b, 0),
            linear,
        };
    });
