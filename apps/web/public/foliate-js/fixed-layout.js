...
export class FixedLayout extends HTMLElement {
    ...
    #onStackScroll = () => {
        if (!this.#stack || this.#stackScrollTimer)
            return;
    ...
    async #createFrame({ index, src: srcOption }, parent = this.#root) {
        ...
        if (!src)
            return { blank: true, hidden: true, element, iframe, overlay, index };
        return new Promise((resolve, reject) => {
            const fail = (error) => { element.remove(); reject(error); };
            iframe.addEventListener('error', () => fail(new Error('Could not load fixed-layout page')), { once: true });
            iframe.addEventListener('load', () => {
                const doc = iframe.contentDocument;
                if (!doc)
                    return fail(new Error('Fixed-layout page document is inaccessible'));
    ...
    #renderFrameAt(frame, scale) {
        ...
        frame.renderPromise = frame.onZoom({
            ...
            if (error instanceof Error && error.name === 'RenderCancelledError')
                return;
            if (frame.failedScale === scale)
                frame.failCount = (frame.failCount ?? 0) + 1;
            else {
                frame.failedScale = scale;
                frame.failCount = 1;
            }
            console.error(error);
        })
            ...
            : typeof this.#zoom === 'number' && !isNaN(this.#zoom)
                ...
                : (this.#zoom === 'fit-width'
                    ...
                    : (portrait || center
                        ...
                        : Math.min(width / ((l.width ?? blankWidth) + (r.width ?? blankWidth)), height / Math.max(l.height ?? blankHeight, r.height ?? blankHeight)))) || 1;
    ...
    #render(side = this.#side) {
        ...
        const { scale, portrait, target, blankWidth, blankHeight } = this.#scaleFor(this.#left, this.#right, this.#center, side);
    ...
    #framesFor(spreadIndex) {
        ...
        if (!book || !spread)
            return Promise.reject(new Error('Fixed-layout spread does not exist'));
    ...
    #showFrames(frames, side) {
        ...
        for (const frame of [this.#left, this.#right, this.#center])
            if (frame && !next.has(frame))
                this.#setFrameHidden(frame, true);
    ...
    #evict() {
        ...
        if (this.#liveFrames.size <= cap)
            return;
        // Never evict the spread being read or its immediate window — those
        // are exactly the ones a turn is about to need.
        const keep = new Set();
        for (let i = this.#index - PRELOAD_SPREADS_BEHIND; i <= this.#index + PRELOAD_SPREADS_AHEAD; i++)
            keep.add(i);
            ...
            for (const spreadIndex of order) {
                ...
                try {
                    ...
                    const side = spread.center ? 'center'
                        : this.rtl ? 'right' : 'left';
                    const { scale } = this.#scaleFor(frames.left, frames.right, frames.center, side);
                    this.#eachFrame(frames, frame => this.#renderFrameAt(frame, scale));
                    await Promise.allSettled([frames.left, frames.right, frames.center]
    ...
    #stackDims(entry) {
        if (entry.width && entry.height)
            return entry;
        if (this.#stackDefaultDims)
            return this.#stackDefaultDims;
    ...
    #teardownStack() {
        ...
        for (const i of [...this.#stackLive])
            this.#demoteStackFrame(i);
        for (const entry of this.#stack)
            entry.slot.remove();
        this.#stack = null;
        this.#stackLive.clear();
        this.#stackWanted.clear();
        if (this.#stackReportTimer)
            clearTimeout(this.#stackReportTimer);
    }
    ...
    #stackIndexAt(scrollCenter) {
        ...
        while (lo < hi) {
            ...
            if (entry.top + entry.pixelHeight <= scrollCenter)
                lo = mid + 1;
            else
                hi = mid;
        }
    ...
    async #ensureStackFrame(entryIndex) {
        ...
        const promise = Promise.resolve().then(async () => {
            ...
            const frame = await this.#createFrame({ index: sectionIndex, src }, entry.slot);
    ...
    #demoteStackFrame(entryIndex) {
        const entry = this.#stack?.[entryIndex];
        if (!entry || (!entry.frame && !entry.framePromise))
            return;
        const frame = entry.frame;
        if (frame) {
            this.#cancelFrameRender(frame);
    ...
    #updateStackWindow(reason) {
        ...
        if (full) {
            ...
            for (const i of [...this.#stackLive]) {
                ...
                else if (!wanted.has(i))
                    this.#cancelFrameRender(this.#stack[i]?.frame);
            }
        }
        ...
        try {
            for (;;) {
                ...
                for (const i of this.#stackWanted) {
                    const entry = this.#stack[i];
                    if (!entry || !this.#stackEntryNeedsWork(entry))
                        continue;
                ...
                try {
                    ...
                    if (!this.#stack || this.#stack[best] !== entry)
                        continue;
    ...
    #onResize() {
        if (this.scrolled && this.#stack) {
            ...
            this.#layoutStack();
            if (entry)
                this.scrollTop = entry.top + offset * entry.pixelHeight;
            return;
        }
        this.#render();
    }
    ...
    #goLeft() {
        if (this.#center || this.#left?.blank)
            return;
    ...
    #goRight() {
        if (this.#center || this.#right?.blank)
            return;
    ...
    #rebuildSpreads() {
        ...
        this.#index = -1;
        if (currentIndex >= 0)
            void this.goTo({ index: currentIndex });
    }
    ...
    setPageColors(pageColors) {
        ...
        if (next?.background === this.#pageColors?.background
            && next?.foreground === this.#pageColors?.foreground)
            return;
    ...
    async goToSpread(index, side, reason = 'navigation') {
        if (index < 0 || index > this.#spreads.length - 1)
            return;
        if (this.scrolled)
            return this.#goToStack(index, reason);
        if (index === this.#index) {
            this.#render(side);
            return;
        }
