...
export const concatTypedArray = (a, b) => {
    const result = new Uint8Array(a.length + b.length);
    result.set(a);
    result.set(b, a.length);
    return result;
};
export const concatTypedArray3 = (a, b, c) => {
...
export const getUint = (buffer) => {
    const l = buffer.byteLength;
    if (l !== 1 && l !== 2 && l !== 4)
        throw new Error(`Invalid MOBI integer width: ${l}`);
    const func = l === 4 ? 'getUint32' : l === 2 ? 'getUint16' : 'getUint8';
    return new DataView(buffer)[func](0);
};
export const getStruct = (def, buffer) => 
...
export const getDecoder = (encoding) => new TextDecoder(MOBI_ENCODING[encoding]);
export const getVarLen = (byteArray, i = 0) => {
...
export const getVarLenFromEnd = (byteArray) => {
    let value = 0;
...
export const decompressPalmDOC = (array) => {
    ...
    for (let i = 0; i < array.length; i++) {
        ...
        if (byte === 0)
            ...
            if (i + byte >= array.length)
                throw new Error('Truncated PalmDOC literal');
...
export const read32Bits = (byteArray, from) => {
...
export const huffcdic = async (mobi, loadRecord) => {
    ...
    const decompress = (byteArray) => {
        ...
        for (let i = 0; i < bitLength;) {
            ...
            if (!codeLength)
                throw new Error('Invalid HUFF code length');
            if (!found) {
                while (codeLength <= 32 && bits >>> (32 - codeLength) < table2[codeLength - 1][0])
                    codeLength += 1;
                if (codeLength > 32)
                    throw new Error('HUFF code exceeds 32 bits');
            ...
            if (!entry || active.has(code))
                throw new Error('Invalid recursive CDIC reference');
...
export const getIndexData = async (indxIndex, loadRecord) => {
    ...
    for (let i = 0; i < indx.numCncx; i++) {
        ...
        for (let pos = 0; pos < array.byteLength;) {
            ...
            if (array[pos] === 0 && array.subarray(pos).every(byte => byte === 0))
                break;
            const index = pos;
...
export const getNCX = async (indxIndex, loadRecord) => {
    ...
    const getChildren = (item, parents = []) => {
        if (parents.includes(item.index))
            throw new Error('Cyclic MOBI navigation index');
...
export const getEXTH = (buf, encoding) => {
    ...
    for (let i = 0; i < count; i++) {
        ...
        if (length < 8 || offset + length > buf.byteLength)
            throw new Error('Truncated EXTH record');
        const spec = EXTH_RECORD_TYPE[type];
        if (spec) {
            const data = buf.slice(offset + 8, offset + length);
            if (spec.length === 3)
                (results[spec[0]] ??= []).push(decoder.decode(data));
            else if (spec.length === 2)
                results[spec[0]] = getUint(data);
            else
                results[spec[0]] = decoder.decode(data);
        }
        offset += length;
    }
    return results;
};
export const getFont = async (buf, unzlib) => {
    ...
    if (flags & 0b10) {
        ...
        if (!key.length)
            throw new Error('MOBI font obfuscation key is empty');
