...
export class MOBI6 {
    ...
    async init() {
        ...
        try {
            ...
            if (tocHref) {
                ...
                if (!target)
                    throw new Error('Invalid MOBI TOC target');
    ...
    async loadRecindex(recindex) {
        if (!recindex)
            throw new Error('Missing MOBI resource index');
    ...
    async replaceResources(doc) {
        ...
        for (const media of doc.querySelectorAll('[mediarecindex]')) {
            ...
            try {
                ...
                if (recindex)
                    media.setAttribute('poster', await this.loadRecindex(recindex));
            }
            catch {
    ...
    resolveHref(href) {
        ...
        if (index < 0 || !Number.isSafeInteger(number))
            return;
    ...
    getSectionHref(index) {
        ...
        if (!section)
            return;
        const anchor = this.#fileposList.find(({ number }) => number >= section.start && number < section.end);
