...
export const getFeed = (doc) => {
    ...
    return {
        ...
        ...items,
        groups,
        facets: Array.from(groupByArray(linksByRel.get(REL.FACET) ?? [], link => link[FACET_GROUP]), ([facet, links]) => ({ metadata: { title: facet }, links })),
    };
};
...
export const getOpenSearch = (doc) => {
    ...
    if (!$url)
        throw new Error('document must contain at least one Url element');
    const regex = /{(?:([^}]+?):)?(.+?)(\?)?}/g;
