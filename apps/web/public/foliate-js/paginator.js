import { SectionView } from "./paginator-view.js";
import { uncollapse, getVisibleRange, selectionIsBackward, setSelectionTo, getBackground, makeMarginals } from "./paginator-geometry.js";
...
export class Paginator extends HTMLElement {
    ...
    constructor() {
        ...
        const templateElement = (id) => {
            ...
            if (!element)
                throw new Error(`Missing paginator template element: ${id}`);
            return element;
        };
        ...
        this.addEventListener('load', event => {
            ...
            doc.addEventListener('focusin', e => {
                ...
                if (this.scrolled || !target || !('nodeType' in target) || target.nodeType !== 1)
                    return;
                // NOTE: `requestAnimationFrame` is needed in WebKit
                requestAnimationFrame(() => this.#scrollToAnchor(target));
            });
        });
    ...
    scrollBy(dxOrOptions = 0, dy = 0) {
        const dx = typeof dxOrOptions === 'number' ? dxOrOptions : dxOrOptions.left ?? 0;
        if (typeof dxOrOptions !== 'number')
            dy = dxOrOptions.top ?? 0;
    ...
    snap(vx, vy) {
        ...
        const page = Math.floor(Math.max(min, Math.min(max, (start + end) / 2
            + (isNaN(d) ? 0 : d))) / size);
    ...
    #getVisibleRange() {
        ...
        if (!doc)
            return null;
        if (this.scrolled)
            return getVisibleRange(doc, this.start + this.#margin, this.end - this.#margin, this.#getRectMapper());
        const size = this.#rtl ? -this.size : this.size;
        return getVisibleRange(doc, this.start - size, this.end - size, this.#getRectMapper());
    }
    #afterScroll(reason) {
    ...
    async #goTo({ index, anchor, select }, navigation = ++this.#navigation) {
    ...
    #scrollPrev(distance) {
        ...
        if (this.scrolled) {
            if (this.start > 0)
                return this.#scrollTo(Math.max(0, this.start - (distance ?? this.size)), null, true);
            return true;
        }
        if (this.atStart)
            return;
