...
export default {
  activate(ctx) {
    ...
    for (const id of ["inspect", "page", "observe", "stop"] as const) ctx.contributions.commands.register({
      id, title: id, run: async () => {
        ...
        return { toast: JSON.stringify({ current, next, seen, hasReading: Boolean(reading),
