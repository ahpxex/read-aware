 ...
 *
 * Presentation only: the marks are loaded by the container
 * (`ReaderCompletionScreen`) and handed in, which is what lets every state of
 * this screen — with marks and without, finished and not, under each reading
 * palette — be rendered on its own.
 */
...
type Props = {
  book: LibraryBook;
  /**
   * The reader's own marks, already loaded (see `useBookMarks`). `null` while
   * the read is still in flight, which renders as no marks section rather than
   * an empty one.
   */
