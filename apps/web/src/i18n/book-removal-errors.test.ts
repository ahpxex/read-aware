...
test("batch removal errors have stable localized copy without raw details or blind retry", async () => {
  await initI18n("en");
  for (const code of ["library/invalid-removal", "library/removal-cleanup-pending", "library/book-reappeared", "library/invalid-cleanup-query"]) {
...
test("all eight locales explain removal errors and preserve the exact approval subject", async () => {
  for (const locale of ["en", "zh-Hans", "zh-Hant", "ja", "ru", "fr", "de", "es"]) {
    ...
    for (const key of ["bookRemovalInvalid", "bookRemovalCleanupPending", "bookRemovalReappeared", "bookCleanupInvalid", "bookCleanupStale"]) expect(common.errors[key]).toBeTruthy();
    ...
    expect(ai.chat.tools.list_book_removal_cleanup).toBeTruthy();
  }
});
