...
现场装配的（书线程每个章节会话装配一次，全局线程每轮装配），两种 scope
走不同深度：

**书线程 —— 章节会话，跨章无状态**（memory-first 不变，连续性的粒度从
"轮"放宽到"章"）：

同一章节内发起的所有轮次共享同一个上下文树 —— `agent.state` 在会话内连续
累积（含工具调用与结果，`windowByTurns` 封顶）。围绕一章的多次提问（划第
一段话问、再划另一段话追问）天然属于同一场讨论，agent 全程在场。**翻页
不清 state**：用户翻到 n+1 章只是浏览、没发消息，回到 n 章接着问，会话
仍在。**当且仅当用户在另一个章节发出新消息**，旧 state 才被扔掉，重置为
无状态基线：
...
消息状态本来就重置了，没有缓存可失。全局线程维持每轮重建。

边界信号是每轮请求携带的章节 href：选区 attachment 的章节优先于当前阅读
位置（问的是哪段话，会话就属于哪一章）；章节未知不算换章。会话是纯内存
的：重启、运行时重建、轮次异常中断后，下一轮自动落回基线装配 —— 基线即
旧的"每轮无状态"行为，退化路径 = 现状，只会少不会错。

被扔掉的历史不丢：UI 的连续转录在持久层；agent 用 `get_recent_turns`
