...
```ts
type ThreadScope = { kind: "book"; bookId: string } | { kind: "global" };
```

| | 书线程（每本书一个） | 全局线程（一个） |
|---|---|---|
| UI 挂载点 | Reader 右侧面板（`ChatPanel`，不动） | Context 页（§9） |
...
4. **检索打分。** 排序 = 语义相关性*加上* recency、importance、显式用户反馈
   （`memory.feedback`）、跨书重复出现 —— 按 `CLAUDE.md` 的要求。
...
v1 检索是 SQLite 上的 scope + 关键词 + recency + importance
（`ix_memories_scope_book` 正是为此准备的）。LanceDB embedding 之后作为派生
索引加入（`docs/data-model.md` §6）；届时 agent API 不变 —— `search_memory`
只是变得更好。
...
在 `transformContext()` 里现场装配：

```
system prompt（该 scope 的角色 framing）
  + user_profile_context bundle
  + book_memory_context bundle        （书线程）
  + reading_intent_context bundle     （当前位置、近期高亮）
  + conversation_insights bundle      （滚动摘要 —— 压缩层）
+ 最近 N 轮原文
+ 当前这轮（+ 选区 attachment）
+ 循环过程中的工具结果
```
...
```ts
type ModelRole = "smart" | "fast";        // 之后 + "embedding"
...
3. **`ai_provider_configs`。** 一般化为 `LlmAccount` 形态：`kind`
   ...
   （`smart_model`、`fast_model`；之后 `embedding_model`）。
...
5. **书籍正文抽取**（`read_passage` + 未来 embedding 的前置条件）：导入时
...
- **Phase 2 —— 记忆写路径：** 逐轮提炼（`fast`）、`memory.*` 事件生产者、
  `memories` 投影、结构化 `search_memory`、ask-note。
  *交付：agent 会记忆，提问留痕。*
...
- **Phase 4 —— onboarding + 巩固：** 混合式 onboarding、渐进式画像、巩固
  批处理（去重/矛盾/衰减/升格）。
- **Phase 5 —— 纵深：** 文本抽取管道 → `read_passage`；embedding + LanceDB
  检索；订阅 OAuth 账户。
