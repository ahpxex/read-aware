...
对话批次已接线：conversations 1.1 增加运行态查询/观察以及创建、选择、停止、清空。Agent 增加双 scope 状态查询与全局管理工具，清空须批准，拒绝工具停止/清空自身线程；插件以 conversations:read/write 分权。原生聊天与线程删除复用同一回合排空，防止清空后旧回复最后一次保存恢复 transcript。选中空草稿不伪造消息，也不代表页面导航。AI02 标为接通待集成验证，AI03 的待确认文本草稿/发送/重试仍未接。
