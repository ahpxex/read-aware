...
- Agent：接通 139、部分 58、扩展 12、未接 17、自动 13、内部 4。
...
| <a id="SYS06"></a>SYS06 | 原生网络 HTTP 请求与响应 | 实装 | **内部**：推理端口/插件工具，无通用 fetch 工具<br>[设计] 有用途/域名约束网络工具 | **接通**：services.network 2.1：Request/二进制/取消/逐跳授权；有界响应所有权<br>[设计] 完整有界 HTTP 服务 | RSS 0.8/TTS 0.6/WebDAV 0.3；旧隔离 Tauri wire probe | fetch保留64 MiB正文，激活8/全部插件32个请求，120秒绝对请求寿命覆盖头/跳转/正文；并发槽等原生收尾才释放，取消不回滚远端效果。networkAccess逐跳授权、最多10跳、拒HTTPS降级，禁原生自动跟随/共享Cookie；标准认证头跨来源去除，方法/正文转换和最终URL/redirected保真。fetch经同一响应owner，不绕过流的预算；2.0插件无需改调用。基础及Worker桥接检查通过，接通不表示新原生链路/实际大文件/生产CSP已验；Agent用途受限入口仍未接。 | [HTTP](../apps/web/src/platform/http-client.ts) [CTX](../apps/web/src/features/plugins/runtime/plugin-context.ts) [WIRE](../apps/web/src/features/plugins/runtime/plugin-worker-host.ts) [WORKER](../apps/web/src/features/plugins/runtime/plugin-sandbox.worker.ts) [RSS](../plugins/rss-reader/src/index.ts) [TTS](../plugins/tts/src/index.ts) [NETWORKSERVICE](../apps/web/src/features/plugins/runtime/plugin-network.ts) [NETWORKPROOF](../apps/web/src/features/plugins/runtime/plugin-network.test.ts) [NETWORKREQUESTS](../apps/web/src/features/plugins/runtime/plugin-network-requests.ts) [NETWORKSTREAMPROOF](../apps/web/src/features/plugins/runtime/plugin-network-requests.test.ts) | P03 |
| <a id="SYS07"></a>SYS07 | 网络域名授权、预算、下载流和离线重试 | 部分 | **未接**：无正式入口<br>[设计] 用途受限任务 | **部分**：network 2.1 policy/openStream/readStream/closeStream；累计额度/重试仍缺<br>[设计] 授权/任务/缓存原语 | 安装/更新授权；请求所有权；Worker分块探针 | 最多32个精确HTTP(S) origin或单独*，缺省拒绝；范围安装/更新展示且旧宿主不能忽略。响应句柄仅本激活，先头后按offset拉取，默认64 KiB/最大1 MiB，单流单个在途读，不可重放；总下载1 GiB，上传和fetch正文64 MiB。8/32并发与120秒绝对寿命共用，EOF/close/到期/停用释放，晚原生结果先收尾再退槽；open的signal只取消开头，取得句柄后显式close。已交付/保存字节不回滚，丢失读回执不能盲重试；不承诺严格总内存预算，原生分块/IPC有额外缓冲且单块最多64 MiB。第一方自定义地址仍显式全网包含本地服务，不是IP/DNS隔离。累计速率/字节额度、重试协调与Agent受限入口仍缺；socket/耐久离线队列非当前宿主能力。实际大下载/慢网/桌面组合留集中验收。 | [API](../packages/plugin-types/src/index.ts) [HTTP](../apps/web/src/platform/http-client.ts) [WIRE](../apps/web/src/features/plugins/runtime/plugin-worker-host.ts) [CATALOG](../packages/core/src/capabilities.ts) [NETWORKPOLICY](../apps/web/src/features/plugins/lib/plugin-network-policy.ts) [NETWORKSERVICE](../apps/web/src/features/plugins/runtime/plugin-network.ts) [NETWORKPROOF](../apps/web/src/features/plugins/runtime/plugin-network.test.ts) [NETWORKREQUESTS](../apps/web/src/features/plugins/runtime/plugin-network-requests.ts) [NETWORKSTREAMPROOF](../apps/web/src/features/plugins/runtime/plugin-network-requests.test.ts) | P04, P06 |
...
以下“已映射”只保证注册项可追到矩阵行，不意味着目标已实现。Plugin ctx 是授予当前全部 manifest 权限后的 200 个顶层可调用路径；返回的 collection/session 方法单列。Settings 74 路径是在 custom + 主/快模型配置的完整条件快照中生成，不表示未配置 AI 时也显示全部路径。Native command 包含 cfg/no-op 历史项，见 SYS18，不能算桌面能力全部对插件开放。
