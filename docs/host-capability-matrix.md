...
- Agent：接通 149、部分 54、扩展 11、自动 13、未接 15、内部 1。
- 插件：接通 165、部分 70、未接 8。
...
| <a id="MEM06"></a>MEM06 | 读取用户画像并注入上下文 | 实装 | **接通**：ProfilePort → prompt；get_user_profile[双域]<br>[设计] 自动上下文/受控查询 | **接通**：memory 1.6 queries.profile / events.observe(kind=profile)<br>[设计] 授权现有画像字段查询 | Agent system prompt；Memory Desk 0.8 画像分页与版本化编辑 | 同一 device-local KV 纯文本摘要，无结构化字段可枚举；memory:read（write 隐含）授权现有摘要，不授予原始 KV、画像写入或 profile/entity 投影。页面默认 4000、上限 16000 UTF-16 单元，nextOffset 不截断字符对；后续页强制 expectedRevision，内容变化 conflict 重读，token 是内容身份而非写 CAS。exists 区分未存与空字符串，错误不当成空画像，退休拒绝迟到页。观察复用有界串行轮询/变化/错误/恢复，无模型查询自动注册观察；不推断字段、不启动访谈或改记忆。只过定向测试，真实 Worker/Agent 对话组合待集中 Tauri 验收；MEM07 条件写入口已接，完整访谈编排、MEM08 投影与 MEM13 bundle 仍缺。 Memory Desk 0.8 已用现有公共API组合版本化画像分页与完整候选确认替换；33项插件测试及构建通过，新增视图显式刷新，真实Worker/Tauri新路径待集中验收。 | [PROFILEPORT](../apps/web/src/features/ai/agent/ports/profile-port.ts) [THREAD](../packages/agent/src/runtime/thread.ts) [PORTS](../apps/web/src/features/ai/agent/ports/index.ts) [API](../packages/plugin-types/src/index.ts) [CTX](../apps/web/src/features/plugins/runtime/plugin-context.ts) [MEMORYPROFILEVIEW](../plugins/memory-desk/src/profile.ts) [MEMORYCONTEXTPROOF](../plugins/memory-desk/tests/context.test.ts) | M05 |
| <a id="MEM07"></a>MEM07 | Onboarding 访谈写入画像 | 部分 | **接通**：update_user_profile 双scope，完整候选/版本绑定批准<br>[设计] 明确接入用户确认后的写流程 | **接通**：memory 1.7 commands.updateProfile，memory:write<br>[设计] 候选画像变更流程 | Agent 画像纠正/访谈摘要；Memory Desk 0.8 完整候选确认编辑 | 现有device-local纯文本摘要已接条件替换，最多16000 UTF-16单元，空文本清摘要但不抹记忆/历史/备份。必须提交所见profile1内容版本，等待旧KV写结算，异步hash后同步复核原值并入队，冲突重读/重确认；非跨进程/跨设备事件CAS，ABA按相同内容处理。Agent每次宿主展示完整候选后批准；插件需memory:write并由自身UI确认，不把插件声称确认当宿主票据。写成功才返回changed，取消阻止未派发写，已派发写不回滚且参与退休drain；失败保留旧持久值并由caller呈现。下一用户轮同章prompt也读取新摘要，不清对话。定向测试通过，真实Worker/Tauri组合留集中验收；profile.updated/entity投影、完整访谈与种子记忆原子编排仍待建。 Memory Desk 0.8 已用现有公共API组合版本化画像分页与完整候选确认替换；33项插件测试及构建通过，新增视图显式刷新，真实Worker/Tauri新路径待集中验收。 | [ONBOARD](../packages/agent/src/onboarding.ts) [PROFILEPORT](../apps/web/src/features/ai/agent/ports/profile-port.ts) [THREAD](../packages/agent/src/runtime/thread.ts) [PROFILEWRITE](../apps/web/src/domain/user-profile.ts) [PROFILEWRITETOOL](../packages/agent/src/tools/user-profile-write-tool.ts) [PROFILEWRITEPROOF](../apps/web/src/domain/user-profile.test.ts) [MEMORYPROFILEVIEW](../plugins/memory-desk/src/profile.ts) [MEMORYCONTEXTPROOF](../plugins/memory-desk/tests/context.test.ts) | M05 |
...
以下“已映射”只保证注册项可追到矩阵行，不意味着目标已实现。Plugin ctx 是授予当前全部 manifest 权限后的 225 个顶层可调用路径；返回的 collection/session 方法单列。Settings 74 路径是在 custom + 主/快模型配置的完整条件快照中生成，不表示未配置 AI 时也显示全部路径。Native command 包含 cfg/no-op 历史项，见 SYS18，不能算桌面能力全部对插件开放。
...
| `memory-desk` | [MEM01](#MEM01) [MEM04](#MEM04) [MEM05](#MEM05) [MEM09](#MEM09) [MEM10](#MEM10) [MEM11](#MEM11) [READ01](#READ01) [EXT02](#EXT02) [EXT05](#EXT05) | [代码] 源码版本 0.10.0；源码存在不等于打包、安装、启用或模型可调用 |
