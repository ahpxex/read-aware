...
| <a id="SYS10"></a>SYS10 | 保存文本/二进制文件与取消回执 | 实装 | **未接**：无正式入口<br>[设计] 宿主文件导出工具 | **接通**：ui.exportFile(filename,content,mimeType) → boolean<br>[设计] 用户确认的文件导出 | Dictionary CSV；Annotation Desk 页内/选中项 JSON/CSV；原生保存 | Annotation Desk 仅导出已观察项，非整库冻结快照；CSV 防公式执行、JSON 保留原文，不导出本机 revision。取消 false 不提示成功。原生保存对话框端到端验收仍待补；没有流式 FileRef | [EXPORT](../apps/web/src/platform/export-file.ts) [CTX](../apps/web/src/features/plugins/runtime/plugin-context.ts) [API](../packages/plugin-types/src/index.ts) [DICTEXPORT](../plugins/dictionary/src/export.ts) [DESKEXPORT](../plugins/annotation-desk/src/export.ts) | P02 |
