...
- Agent：接通 117、未接 44、部分 47、扩展 14、自动 16、内部 5。
...
| <a id="ANN04"></a>ANN04 | 高亮改色/删除 | 实装 | **接通**：edit_annotation(expectedRevision) / delete_annotation+版本绑定批准 / apply_annotation_changes<br>[设计] 受控写工具 | **接通**：recolorHighlight/removeHighlight；applyChanges 条件改色/样式/删除<br>[设计] 写领域 | 标注菜单；Agent；Annotation Desk；桌面 Worker | 新版 applyChanges 可同时改 color/style；Annotation Desk 用此接口批量改色/样式及确认删除。原生菜单现用所见列表 revision 条件改色/删除，不在保存前刷新 token；改色不重写旧 style。旧单项公共命令仍无调用者版本条件。高亮原文修改不开放 | [ANNOT](../apps/web/src/domain/annotations.ts) [ANNTOOLS](../packages/agent/src/tools/annotation-tools.ts) [ANNBATCHTOOL](../packages/agent/src/tools/annotation-batch-tool.ts) [ANNMUTATIONS](../apps/desktop/src-tauri/src/storage/annotation_mutations.rs) [CTX](../apps/web/src/features/plugins/runtime/plugin-context.ts) [DESKBATCH](../plugins/annotation-desk/src/batch.ts) [NATIVEANNWRITES](../apps/web/src/features/annotations/lib/native-annotation-mutations.ts) [NATIVEANNEDITOR](../apps/web/src/features/reader/hooks/useReaderNoteEditor.ts) [NATIVECASPROOF](../docs/evidence/native-annotation-cas-2026-09-10.json) | F02 |
| <a id="ANN05"></a>ANN05 | 创建/编辑/删除笔记 | 实装 | **接通**：create/edit/delete_annotation<br>[设计] 受控写工具 | **接通**：createNote/updateNote/removeNote；applyChanges 条件写<br>[设计] 写领域 | NoteEditor；Agent；Annotation Desk 编辑/删除 | 可无位置笔记；Annotation Desk 不创建笔记。原生编辑器冻结所见 note/token，冲突留草稿；保存中去重并禁用输入/提交，旧草稿完成不关闭替代草稿。原生列表/书籍详情删除同样使用所见版本。实际 Worker 与 Agent 修改后的旧草稿冲突、重新打开后保存已验；100 个存储及两个封面同步命令迁入阻塞工作池，锁等待不占 UI/异步执行线程；封面协议异步响应。隔离 macOS 已验真实锁中 busy/Cancel/重开、释放后旧保存成功且新草稿保留、超时本地化失败及重试；排队查询/封面请求不阻断界面。取消不撤销已受理写；启动与原生窗口退出仍有同步路径，非全部原生工作无阻塞保证，最大队列/载荷和跨平台未验。删除 Agent 批准和插件授权不是同一种策略 | [ANNOT](../apps/web/src/domain/annotations.ts) [ANNTOOLS](../packages/agent/src/tools/annotation-tools.ts) [CTX](../apps/web/src/features/plugins/runtime/plugin-context.ts) [DESK](../plugins/annotation-desk/src/views.ts) [NATIVEANNWRITES](../apps/web/src/features/annotations/lib/native-annotation-mutations.ts) [NATIVEANNEDITOR](../apps/web/src/features/reader/hooks/useReaderNoteEditor.ts) [NATIVECASPROOF](../docs/evidence/native-annotation-cas-2026-09-10.json) [STORAGEEXEC](../apps/desktop/src-tauri/src/storage/execution.rs) [STORAGEEXECPROOF](../docs/evidence/storage-execution-2026-09-10.json) | F02 |
...
以下“已映射”只保证注册项可追到矩阵行，不意味着目标已实现。Plugin ctx 是授予当前全部 manifest 权限后的 125 个顶层可调用路径；返回的 collection/session 方法单列。Settings 74 路径是在 custom + 主/快模型配置的完整条件快照中生成，不表示未配置 AI 时也显示全部路径。Native command 包含 cfg/no-op 历史项，见 SYS18，不能算桌面能力全部对插件开放。
...
| 当前注册项 | 矩阵行 | 说明 |
| --- | --- | --- |
| `annotation-desk` | [ANN01](#ANN01) [ANN04](#ANN04) [ANN05](#ANN05) [ANN08](#ANN08) [ANN09](#ANN09) [EXT02](#EXT02) [EXT05](#EXT05) [SYS10](#SYS10) | [代码] 源码版本 0.2.0；源码存在不等于打包、安装、启用或模型可调用 |
