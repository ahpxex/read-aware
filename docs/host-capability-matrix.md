...
- Agent：接通 137、部分 59、扩展 13、未接 17、自动 13、内部 4。
- 插件：接通 151、部分 82、未接 10。
...
| <a id="READ05"></a>READ05 | 按进度/固定版式页索引定位 | 实装 | **部分**：open_book；list_book_navigation_targets[双域]<br>[设计] 进度/页位置工具 | **部分**：reading 2.12 goTo；library 1.14 listNavigationTargets<br>[设计] seek 命令 | 阅读进度拖动；两端桌面探针 | 0–1 进度与零起始源分节索引已接；sectionIndex 必须带版本、不可混用 CFI/href/fraction/textQuote，非法索引/旧版本/缺失源节分别拒绝。PDF/漫画每节一源页，固定版式双页展示不改索引；其他格式不将源节冒充页面或目录章节。返回原有真实位置并进入共享历史，定向测试通过，真实多格式组合待集中验收。library 1.14 已接版本化源分节/page-list 分页目录及精确标签过滤，重复标签保留多目标，外部/失效链接不给位置，查询不读正文；无 page-list 与无匹配分开。EPUB nav/NCX 与 PDF 标签已接同源目录；PDF.js 标签按需合并读取、成功缓存/失败可重试，关闭拒绝迟到结果；原生页进度同源异步初始化，不挡首屏，读取失败不当缺表。PDF 映射/生命周期是受控端口单测，实际 PDF.js Worker/桌面页码与重排屏幕页数仍待验或接通 | [READTOOLS](../packages/agent/src/tools/reader-tools.ts) [NAV](../apps/web/src/domain/reading-session-controller.ts) [NAVADAPTER](../apps/web/src/features/reader/lib/reading-engine-adapter.ts) [NAVADAPTERPROOF](../apps/web/src/features/reader/lib/reading-engine-adapter.test.ts) [API](../packages/plugin-types/src/index.ts) [NAVTARGETS](../apps/web/src/features/library/lib/book-navigation-targets.ts) [NAVTARGETTYPES](../packages/core/src/book-navigation-targets.ts) [NAVTARGETPROOF](../apps/web/tests/book-navigation-targets.test.ts) [PDFPAGES](../apps/web/foliate-js/src/pdf-navigation.ts) [PDFPAGEPROOF](../apps/web/tests/foliate-pdf.test.ts) | D05 |
