...
| <a id="READ04"></a>READ04 | 前后翻页、章节、书首书尾 | 实装 | **部分**：navigate_reading(next/previous/next-section/previous-section/start/end)<br>[设计] 导航步骤工具 | **部分**：reading 2.12 commands.step(page/section/start/end,guard?)<br>[设计] 导航步骤命令 | 键盘/页面按钮/滚轮；桌面探针 | 已接页面、相邻线性源分节与书首书尾；分节跳过 linear=no，定位相邻分节开头，边界保持实际位置。首尾复用引擎 fraction 0/1；不是 TOC 节点或抽取章节步进。分节/首尾跳转记共享历史，普通页步进不记；复用会话 guard、队列、超时与固定版式 paint 等待。定向测试通过，Tauri/Worker 多格式待集中验收；library 1.14 已接已有 page-list 标签查询后按位置导航；PDF 标签已接同源页码目录；TOC 语义章节步进与全部原生入口收敛仍缺 | [NAV](../apps/web/src/domain/reading-session-controller.ts) [NAVADAPTER](../apps/web/src/features/reader/lib/reading-engine-adapter.ts) [NAVADAPTERPROOF](../apps/web/src/features/reader/lib/reading-engine-adapter.test.ts) [SHORTCUT](../apps/web/src/features/settings/lib/shortcuts.ts) [API](../packages/plugin-types/src/index.ts) [REGISTRY](../packages/agent/src/tools/registry.ts) | D04 |
