...
| <a id="MEM12"></a>MEM12 | 跨对话 insights 与滚动摘要 | 实装 | **接通**：get_conversation_insights[全局] bookId/threadId/当前线程；rolling summary 自动<br>[设计] 检索工具+自动摘要 | **接通**：conversations1.4 queries.getInsights(kind/id)<br>[设计] 受权只读现有摘要 | Agent 与插件共用摘要存储，宿主写入/清除后通知失效 | 对话读权（写隐含读），非memory授权；只读字符串/null，不生成摘要、不暴露内部状态或写API。缺失非无聊天，空字符串与缺失分开；旧global仅映射原始全局线程，损坏db/error不冒充空或覆写。成功写后失效不携正文，失败无成功通知；生命周期/工具signal取消不交付迟到结果。既有本机KV镜像可含未落盘写，摘要可滞后，无来源水位/时间戳/CAS/跨设备协议，不是原话或正式bundle。基础共享存储/授权/Agent流程通过，真实Worker/Tauri组合待集中验收。 | [CHATTOOLS](../packages/agent/src/tools/conversation-tools.ts) [THREAD](../packages/agent/src/runtime/thread.ts) [CHATPORT](../apps/web/src/features/ai/agent/ports/conversation-port.ts) [CONVERSATIONINSIGHTS](../apps/web/src/features/ai/lib/conversation-insights-store.ts) [CONVERSATIONINSIGHTSPROOF](../apps/web/src/domain/conversation-insights.test.ts) | 新增盘点 |
...
| `memory-desk` | [MEM01](#MEM01) [MEM04](#MEM04) [MEM05](#MEM05) [MEM09](#MEM09) [MEM10](#MEM10) [MEM11](#MEM11) [READ01](#READ01) [EXT02](#EXT02) [EXT05](#EXT05) | [代码] 源码版本 0.7.0；源码存在不等于打包、安装、启用或模型可调用 |
