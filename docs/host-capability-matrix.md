...
| <a id="MORE05"></a>MORE05 | 贡献的动态 visible/enabled/checked 与自有视图刷新 | 部分 | **部分**：工具按 scope 注册，无统一 enablement<br>[设计] 操作 availability | **部分**：views 1.1 live.subscribe + UI 1.2 publishView<br>[设计] 声明式条件与受控视图状态 | Text Desk 0.3 请求详情；真实 Worker 组合探针 | 宿主签发当前激活代/可见帧通道，按非负安全整数 revision 接受新完整视图；foreign/retired/unknown 同报 inactive，旧 revision 为 stale。push/modal/关闭立即撤通道并退订，back/resume 新通道重取当前值；更新不导航、不重置帧键，保留现有表单草稿协调。普通 RPC 参数改用可转移租约，已绘制和最新未绘制回调保留，中间快照释放；修复共享退出监听被旧租约误移除。失败保留上次视图并显示安全 InlineError。隔离 macOS debug 验新回调/草稿/逆序/跨 actor/嵌套/晚 ACK/停用与 Text Desk 自动完成，1200/800 窗口已看。每激活最多 16 通道，回执不是 DOM 或绘制完成；贡献 visible/enabled/checked、Agent enablement、全部表单/焦点/大数据与 packaged/跨平台仍缺；不开放 DOM/Jotai。 | [API](../packages/plugin-types/src/index.ts) [RENDER](../apps/web/src/features/plugins/components/PluginViewRenderer.tsx) [CTX](../apps/web/src/features/plugins/runtime/plugin-context.ts) [REGISTRY](../packages/agent/src/tools/registry.ts) [VIEWSESSION](../apps/web/src/features/plugins/lib/plugin-view-session.ts) [LIVEVIEW](../apps/web/src/features/plugins/lib/plugin-live-view.ts) [VIEWCHANNELS](../apps/web/src/features/plugins/lib/plugin-view-channels.ts) [CALLBACKWIRE](../apps/web/src/features/plugins/runtime/plugin-callback-wire.ts) [LIVEVIEWPROOF](../docs/evidence/plugin-live-views-2026-09-09.json) | I07, J04, J05 |
