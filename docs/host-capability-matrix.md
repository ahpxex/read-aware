...
- Agent：接通 148、部分 55、扩展 11、自动 13、未接 15、内部 1。
...
| <a id="CFG01"></a>CFG01 | 设置 discover/read/update 与动态选项 | 实装 | **接通**：get_settings/update_settings；等待本地事务提交<br>[设计] 设置工具 | **接通**：settings 1.5 snapshot/discover/read/update；原子保存与授权结果<br>[设计] 路径授权设置领域 | Agent；Theme Schedule；TTS options；Workspace Profiles | snapshot 等待此前命令/UI 写结算后一次读取，按路径授权过滤；单个命令跨 KV 记录原子提交；失败不发 settings.changed，下一命令基于已结算状态；结果快照按 read/write grant 过滤，writable 反映当前 actor 授权，discover 不泄露快捷键运行态。事务不包含密钥、远端漫游提交或尚未接通的效果；设置 API 接通不证明值有消费者 Workspace Profiles 0.4 已组合字体搜索/固定版本40项分页/预览与应用、独立内容字体跟随开关和v2十项预设；v1七项原样保留。16项插件测试、构建和类型检查通过，实际字体生效与Worker/Tauri新路径待集中验收。 Workspace Profiles0.6 已消费相同存储与宿主批准：只读list/inspect/current、确认保存/应用/删除；保存复核十项设置指纹，应用与删除校验所见文档revision，删除为条件事务。UI40项/Agent最多20项分页，旧游标失效重读，v1/v2可用；34项插件测试、编译注册/构建/类型通过，新协议原生Worker/SQLite/批准与模型组合待集中验收，不将设置与文档当联合事务。 | [SETTINGS](../apps/web/src/domain/settings/catalog.ts) [SETDOMAIN](../apps/web/src/domain/settings/domain.ts) [SETTOOLS](../packages/agent/src/tools/settings-tools.ts) [CTX](../apps/web/src/features/plugins/runtime/plugin-context.ts) [KV](../apps/web/src/platform/local-store.ts) [RUST](../apps/desktop/src-tauri/src/lib.rs) [WORKSPACEFONTS](../plugins/workspace-profiles/src/fonts.ts) [WORKSPACEFONTSPROOF](../plugins/workspace-profiles/tests/fonts.test.ts) [WORKSPACEPROFILETOOLS](../plugins/workspace-profiles/src/tools.ts) [WORKSPACEPROFILETOOLPROOF](../plugins/workspace-profiles/tests/tools.test.ts) [WORKSPACEPROFILEVIEWPROOF](../plugins/workspace-profiles/tests/views.test.ts) | H01 |
...
| <a id="AI06"></a>AI06 | 一次性文本/结构化/流式 LLM 推理 | 实装 | **自动**：Runtime ask/askDetailed + 线程推理<br>[设计] 运行时服务 | **接通**：llm 1.3 ask/askDetailed：readingContext/schema/onText/signal/timeoutMs/maxOutputTokens<br>[设计] 受预算约束的推理服务 | Dictionary 1.3；Agent 后台管线；推理基础探针 | readingContext仍由宿主过滤，收紧立即取消；必需正文禁止报ai/context-withheld，非法结构报ai/invalid-reading-context。两接口信号穿Worker到普通/结构化/流式源；结构化最多两次共用截止，流式等待每次onText回调，取消后不发旧增量。1.3成功metadata含每次尝试，原ask仍只返原结果。任意prompt不做来源推断，字典本地缓存策略不变。已有隐私macOS证据不覆盖本批新路径；基础测试通过，真实Tauri/服务商组合留集中验收。 | [RUNTIME](../packages/agent/src/runtime/runtime.ts) [CTX](../apps/web/src/features/plugins/runtime/plugin-context.ts) [API](../packages/plugin-types/src/index.ts) [DICT](../plugins/dictionary/src/index.ts) [STRUCTUREDREADING](../packages/agent/src/runtime/one-shot.ts) [STRUCTUREDREADINGPROOF](../docs/evidence/structured-reading-context-2026-09-09.json) [PLUGINLLM](../apps/web/src/features/plugins/runtime/plugin-llm.ts) | L01 |
| <a id="AI07"></a>AI07 | 推理取消、超时、用量/预算/成本可见性 | 部分 | **部分**：线程abort；共享Runtime.ask/askDetailed可接signal、输出上限及成功metadata<br>[设计] 统一任务/预算 | **部分**：llm 1.3 askDetailed + maxOutputTokens + policy<br>[设计] 取消/截止/并发及成功用量估算已接；总预算与失败回执仍缺 | 聊天Stop；插件推理基础探针 | 默认60秒、整数1..110000ms，整次ask含结构化重试；每plugin ID2/全App8个插件推理，跨激活共享，原提供者terminal promise未结算不退槽。askDetailed成功返回value/attempts，每次记录解析模型、停止原因、SDK token计数和USD费用估算，含首次无效结构化尝试，不含原文/密钥/端点；未知或全零用量及零价格目录返回null，不等于免费。两接口可请求每次1..65536输出token，模型/账户/relay可更严，结构化两次各请求一次，不是总token或账单预算。Worker取消保留本地reason，宿主RPC为plugin/cancelled，内部ai/request-cancelled；超时ai/request-timeout，忙ai/busy。不保证SDK网络背压或输出内存上限；失败/取消后的最终用量回执、总token/费用预算和统一TaskRef仍缺，取消不能保证远端未处理/未收费。真实Tauri/服务商与业务插件组合集中后置。 | [THREAD](../packages/agent/src/runtime/thread.ts) [RUNTIME](../packages/agent/src/runtime/runtime.ts) [CTX](../apps/web/src/features/plugins/runtime/plugin-context.ts) [WIRE](../apps/web/src/features/plugins/runtime/plugin-worker-host.ts) [PLUGINLLM](../apps/web/src/features/plugins/runtime/plugin-llm.ts) | L02 |
...
| <a id="MORE06"></a>MORE06 | 发现/复用类型化提供者与跨插件权限交集 | 部分 | **部分**：list_plugin_contributions[双域]；host聚合tool/retrieval<br>[设计] 所有已注册扩展点身份发现 | **部分**：plugins1.1 contributions/observeContributions<br>[设计] 15种注册条目查询/观察，不增权 | 宿主消费voices/content/modes/tools/transports；双端注册身份目录；Maintenance Desk 0.2 | 共享registry自动覆盖selection/header/context动作、commands、settingsOptions、voice/content/readerModes、Agent tools/context/retrieval/memory candidates、themes/fonts，另合并syncTransports。仅point/pluginId/key，查询不调用插件或打开传输会话；注册非健康/可用/本轮可调用保证，key不是通用调用句柄或设置值。按扩展点/插件/搜索分页，nextOffset按实际页长，更新后重读；观察覆盖注册/替换/更新/注销，单次初始与串行合并回调，退休清理。现有宿主条目发现已接；跨插件任意invoke与权限交集broker原本未建，仍缺，不把注册权当调用权或读取他人storage。Maintenance Desk 0.2已组合全目录或固定插件贡献查询/分页/观察，只读身份；基础与编译入口测试通过，真实Worker/Tauri集中后置。 | [EXTOOLS](../apps/web/src/features/plugins/runtime/plugin-tools.ts) [CTX](../apps/web/src/features/plugins/runtime/plugin-context.ts) [SYNCTRANSPORT](../apps/web/src/platform/sync/transport-registry.ts) [API](../packages/plugin-types/src/index.ts) [CONTRIBUTIONDISCOVERY](../apps/web/src/services/plugin-contributions.ts) [CONTRIBUTIONDISCOVERYPROOF](../apps/web/src/services/plugin-contributions.test.ts) [MAINTENANCEADMIN](../plugins/maintenance-desk/src/plugin-directory.ts) [MAINTENANCEADMINPROOF](../plugins/maintenance-desk/tests/admin.test.ts) | N04, N06 |
...
以下“已映射”只保证注册项可追到矩阵行，不意味着目标已实现。Plugin ctx 是授予当前全部 manifest 权限后的 222 个顶层可调用路径；返回的 collection/session 方法单列。Settings 74 路径是在 custom + 主/快模型配置的完整条件快照中生成，不表示未配置 AI 时也显示全部路径。Native command 包含 cfg/no-op 历史项，见 SYS18，不能算桌面能力全部对插件开放。
...
| `jumper` | [TXT02](#TXT02) [TXT07](#TXT07) [READ01](#READ01) [READ06](#READ06) [READ07](#READ07) [READ13](#READ13) [EXT02](#EXT02) [SYS02](#SYS02) [AI05](#AI05) | [代码] 源码版本 0.7.0；源码存在不等于打包、安装、启用或模型可调用 |
