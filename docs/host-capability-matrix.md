...
- 宿主：实装 203、部分 38、非桌面 1、待建 1。
- Agent：接通 150、部分 43、扩展 11、接通（待 E2E） 13、自动 13、未接 13。
- 插件：接通 165、部分 59、接通（待 E2E） 14、未接 5。
...
| <a id="LIB02"></a>LIB02 | 修改标题/作者 | 实装 | **接通**：update_book<br>[设计] 写工具 | **接通**：library.commands.books.editMetadata<br>[设计] 写领域 | Library Desk 0.8；书架；Agent | 已有单项写，批量与冲突另列 集中桌面已验作者修改及清空；标题修改和重启未验。 | [LIB](../apps/web/src/domain/library.ts) [LIBUI](../apps/web/src/features/library/hooks/useLibraryCommands.ts) [SHELFTOOLS](../packages/agent/src/tools/shelf-tools.ts) [CTX](../apps/web/src/features/plugins/runtime/plugin-context.ts) [LIBRARYORGANIZE](../plugins/library-desk/src/organize-books.ts) [METADATAPATCH](../apps/web/src/features/library/lib/book-metadata-patch.ts) [LIBRARYORGANIZEPROOF](../plugins/library-desk/tests/organize.test.ts) [LIBRARYCONTENTNATIVE](../docs/evidence/library-content-composition-2026-09-11.json) | B02 |
| <a id="LIB03"></a>LIB03 | 收藏/取消收藏 | 实装 | **接通**：update_book.starred<br>[设计] 写工具 | **接通**：library.commands.books.setStarred<br>[设计] 写领域 | Library Desk 0.8；书架；Agent | 按对象授权与写回执仍需基线回归 集中桌面已验收藏写入和原生重读；取消收藏未验。 | [LIB](../apps/web/src/domain/library.ts) [LIBUI](../apps/web/src/features/library/hooks/useLibraryCommands.ts) [SHELFTOOLS](../packages/agent/src/tools/shelf-tools.ts) [CTX](../apps/web/src/features/plugins/runtime/plugin-context.ts) [LIBRARYORGANIZE](../plugins/library-desk/src/organize-books.ts) [LIBRARYORGANIZEPROOF](../plugins/library-desk/tests/organize.test.ts) [LIBRARYCONTENTNATIVE](../docs/evidence/library-content-composition-2026-09-11.json) | B02 |
...
| <a id="LIB11"></a>LIB11 | 重复检测、同源书合并和 ID 重定向 | 实装 | **接通**：list_duplicate_books/preview_book_merge/merge_duplicate_books/resolve_book_reference[全局]<br>[设计] 候选/预览/批准合并/重定向 | **接通**：library 1.10 books.listDuplicates/previewMerge/resolveId/mergeDuplicates<br>[设计] 候选/预览/条件合并/重定向 | Library Desk 0.8；导入去重；同步后与双端共用条件合并 | 仅原文件 hash 相同组，最早 createdAt 再按 ID 保留，不按相似标题任意归并。读需 library:read，插件写需 library:write，global Agent 逐次批准完整组；事务重验成员/展示元数据/身份事件版本，最多 1000 重复记录，原始事件原子提交并回执 ID 映射。阅读/标注按提交时宿主规则归并，不冻结全部状态；聊天不合成单线程。保留唯一原文件，继承封面按投影 key 读取；旧 alias 资产在 keeper 存活时固定保留，非立即磁盘回收。文件复制失败不删原件，SQL 回滚可留额外缓存字节。已派发取消不回滚；未知/已删 ID resolve 为 null。定向与 Rust 测试通过，Worker/批准 UI/真实同步与格式仍待集中 Tauri E2E。 | [DEDUPE](../apps/web/src/platform/book-dedupe.ts) [LIB](../apps/web/src/domain/library.ts) [APPLY](../apps/desktop/src-tauri/src/storage/apply.rs) [BOOKMERGE](../apps/web/src/domain/book-merge.ts) [NATIVEMERGE](../apps/desktop/src-tauri/src/storage/book_merge.rs) [MERGETOOLS](../packages/agent/src/tools/book-merge-tools.ts) [API](../packages/plugin-types/src/index.ts) [CTX](../apps/web/src/features/plugins/runtime/plugin-context.ts) [LIBRARYDUPLICATES](../plugins/library-desk/src/duplicates.ts) [LIBRARYORGANIZEPROOF](../plugins/library-desk/tests/organize.test.ts) | A04, B06 |
| <a id="LIB12"></a>LIB12 | 创建/幂等绑定虚拟书并更新标题 | 实装 | **扩展**：RSS subscribe_feed[全局]<br>[设计] 内容创建工具/贡献消费 | **接通**：addVirtualBook，同 binding 更新标题<br>[设计] 插件自有内容领域 | RSS | 不是所有虚拟书创建都自动成为 Agent 工具；仅 RSS 提供一例 | [CTX](../apps/web/src/features/plugins/runtime/plugin-context.ts) [VIRTUAL](../apps/web/src/features/plugins/lib/virtual-books.ts) [RSSLIB](../plugins/rss-reader/src/feed-library.ts) [RSSTOOLS](../plugins/rss-reader/src/agent-tools.ts) | B07 |
| <a id="LIB13"></a>LIB13 | 移除插件自有虚拟书 | 部分 | **部分**：delete_book 通用删除；全局清理查询/重试；RSS 0.10 经批准退订<br>[设计] 受控删除工具 | **部分**：removeVirtualBook 传播删除失败并等待绑定清理持久化；library 1.6 文件恢复<br>[设计] 自有内容删除回执 | RSS 退订；书架删除后订阅清理 | 确认读前等待既有 KV 写结算，事件提交失败保留绑定，清理等待持久回执；已删书后失败仍拒绝，显式重试完成解绑。通知先于文件释放，绑定可能已清除；再次 removeVirtualBook 不重试文件，但 library 1.6 宿主持久意图保留书 ID/标题，两端 listRemovalCleanup/文件重试与启动恢复不再依赖绑定或旧回执。无源文件的合成虚拟书已验证意图确认失败时绑定消失、队列仍在，移除故障后重启清理完成，不冒充虚拟文件 I/O。损坏绑定报 db/error、不当空表或覆盖；同 ID 绑定变化拒绝清理。既有真实 Worker/Agent 插件工具的事件和 KV 拒绝测试保留；不是自主模型证明；下述原生组合补上正常退订流程。书籍、blob、绑定、RSS 文档不是联合原子事务；RSS 0.9 已接私有正文缓存、同激活同 URL 操作串行和当前引用清理；不是跨存储事务，崩溃遗留缓存回收、绑定恢复及 packaged/跨平台仍缺。 2026-09-11 真实 RSS0.11 Worker/Tauri 已验订阅/刷新/稳定文章导航、503保留数据、源关闭且Worker重启后缓存阅读、退订清理；自有SQL计数0，900/600宽前台画面已验。无宿主API改动，不覆盖应用重启/远程媒体/OPML或Agent模型。 | [CTX](../apps/web/src/features/plugins/runtime/plugin-context.ts) [VIRTUAL](../apps/web/src/features/plugins/lib/virtual-books.ts) [RSSVIEWS](../plugins/rss-reader/src/views.ts) [SHELFTOOLS](../packages/agent/src/tools/shelf-tools.ts) [VIRTUALREMOVALPROOF](../docs/evidence/virtual-book-removal-2026-09-09.json) [BOOKBATCH](../apps/web/src/features/library/lib/book-removal.ts) [BOOKCLEANUP](../apps/desktop/src-tauri/src/storage/library_cleanup.rs) [BOOKCLEANUPPROOF](../docs/evidence/book-removal-recovery-2026-09-09.json) [RSSCOMPOSITIONPROOF](../docs/evidence/rss-composition-2026-09-11.json) | B07 |
| <a id="LIB14"></a>LIB14 | 虚拟内容修订/离线缓存/当前书刷新 | 部分 | **部分**：get_book_content_state / navigate_reading(reload)[双域]；RSS refresh_feed[全局]<br>[设计] 来源状态/刷新与当前书重载 | **部分**：library 1.16 getContentState/observeContentState；1.15 invalidateVirtualBook；reading 2.16 sourceRevision/2.15 reload<br>[设计] 源状态观察、自有失效与独立重载 | 正式插件/Agent 接线；RSS 0.9 消费既有 API | library读可查询源种类、local/missing/provider-registered/provider-unavailable及不透明sourceRevision；不解析/下载/调用provider，不含阅读状态、路径/URL/私有key，虚拟contentVersion=null。源token跟踪失效通知、provider激活身份和绑定key；reading读的ready会话另有装载时sourceRevision，比较同书token可识别已知变化，不证明远端未发生未通知更新。每actor最多64串行观察，首次即读、读和回调结算后一秒复核，去重/稳定错误/恢复，退休丢迟到结果，不是完整日志或全局同步快照。library写的自有invalidate通知不自动拉网/移动当前画面；旧active解析器不跨失效代或binding key复用，在途读拒绝跨代结果。独立reload需reading写，新会话从头加载不套旧CFI/进度，等ready及实际落点，保留guard/取消/30秒截止/共享历史。基础测试含受控IPC、实际上下文和Agent端口，不是原生Tauri/Worker。RSS 0.9 已接内容寻址正文缓存、稳定文章 ID、变更通知与显式打开时的源比较/重载；旧订阅首次读取补缓存，通知失败留待重试，同 URL 操作串行。离线限 feed 正文而非远程媒体，无声明 ID/link 的文章身份随内容改变；旧 ordinal href 不猜测映射。跨版本 CFI/进度/标注迁移与崩溃遗留缓存回收仍缺，保留部分。 2026-09-11 真实 RSS0.11 Worker/Tauri 已验订阅/刷新/稳定文章导航、503保留数据、源关闭且Worker重启后缓存阅读、退订清理；自有SQL计数0，900/600宽前台画面已验。无宿主API改动，不覆盖应用重启/远程媒体/OPML或Agent模型。 | [VIRTUAL](../apps/web/src/features/plugins/lib/virtual-books.ts) [RSSFEED](../plugins/rss-reader/src/feed.ts) [RSSLIB](../plugins/rss-reader/src/feed-library.ts) [RSSCACHE](../plugins/rss-reader/src/content-cache.ts) [RSSCACHEPROOF](../plugins/rss-reader/tests/feed-library.test.ts) [RSSVIEWS](../plugins/rss-reader/src/views.ts) [API](../packages/plugin-types/src/index.ts) [CONTENTSOURCE](../apps/web/src/features/library/lib/book-content-source.ts) [CONTENTINVALIDATION](../apps/web/src/features/library/lib/content-invalidation.ts) [CONTENTREFRESHPROOF](../apps/web/src/features/library/lib/content-invalidation.test.ts) [CONTENTSTATE](../apps/web/src/domain/book-content-state.ts) [CONTENTSTATEPROOF](../apps/web/src/domain/book-content-state.test.ts) [NAV](../apps/web/src/domain/reading-session-controller.ts) [NAVTEST](../apps/web/src/domain/reading-session-controller.test.ts) [READTOOLS](../packages/agent/src/tools/reader-tools.ts) [RSSCOMPOSITIONPROOF](../docs/evidence/rss-composition-2026-09-11.json) | B07, N02 |
| <a id="LIB15"></a>LIB15 | 列出集合及其成员 | 实装 | **接通**：list_collections[全局]<br>[设计] 查询工具 | **接通**：library.queries.collections.list/booksIn<br>[设计] 只读领域 | Library Desk 0.8；书架；Agent | 集合单归属模型保持不变 集中桌面已验集合列表、分配及移除后重读。 | [LIB](../apps/web/src/domain/library.ts) [SHELFTOOLS](../packages/agent/src/tools/shelf-tools.ts) [CTX](../apps/web/src/features/plugins/runtime/plugin-context.ts) [LIBRARYCOLLECTIONS](../plugins/library-desk/src/collections.ts) [LIBRARYORGANIZEPROOF](../plugins/library-desk/tests/organize.test.ts) [LIBRARYCONTENTNATIVE](../docs/evidence/library-content-composition-2026-09-11.json) | B03 |
| <a id="LIB16"></a>LIB16 | 创建/重命名集合 | 实装 | **接通**：manage_collection[全局]<br>[设计] 写工具 | **接通**：collections.create/rename<br>[设计] 写领域 | Library Desk 0.8；书架；Agent | 书内不注册全局集合管理工具是明确 scope 策略 集中桌面真实 Worker 已验集合创建及同 ID 重命名。 | [LIB](../apps/web/src/domain/library.ts) [SHELFTOOLS](../packages/agent/src/tools/shelf-tools.ts) [CTX](../apps/web/src/features/plugins/runtime/plugin-context.ts) [LIBRARYCOLLECTIONS](../plugins/library-desk/src/collections.ts) [LIBRARYORGANIZEPROOF](../plugins/library-desk/tests/organize.test.ts) [LIBRARYCONTENTNATIVE](../docs/evidence/library-content-composition-2026-09-11.json) | B03 |
| <a id="LIB17"></a>LIB17 | 删除集合 | 实装 | **接通**：delete_collection[全局] + 批准<br>[设计] 受控写工具 | **接通**：collections.remove<br>[设计] 写领域/风险策略 | Library Desk 0.8；书架；Agent | 删除集合与删除其中书籍是不同操作 集中桌面已验未确认拒绝、确认移除、书籍保留及归属清空。 | [LIB](../apps/web/src/domain/library.ts) [SHELFTOOLS](../packages/agent/src/tools/shelf-tools.ts) [CTX](../apps/web/src/features/plugins/runtime/plugin-context.ts) [LIBRARYCOLLECTIONS](../plugins/library-desk/src/collections.ts) [LIBRARYORGANIZEPROOF](../plugins/library-desk/tests/organize.test.ts) [LIBRARYCONTENTNATIVE](../docs/evidence/library-content-composition-2026-09-11.json) | B03 |
| <a id="LIB18"></a>LIB18 | 批量分配/移出集合 | 实装 | **接通**：manage_collection.assign[全局]<br>[设计] 批量写工具 | **接通**：collections.assignBooks<br>[设计] 批量写领域 | Library Desk 0.8；书架拖放/多选；Agent | 移动书籍只是成员变更，不是文件移动 集中桌面已验未确认不写入及确认分配；显式移出及并发未验。 | [LIB](../apps/web/src/domain/library.ts) [SHELFTOOLS](../packages/agent/src/tools/shelf-tools.ts) [CTX](../apps/web/src/features/plugins/runtime/plugin-context.ts) [LIBRARYCOLLECTIONS](../plugins/library-desk/src/collections.ts) [LIBRARYORGANIZEPROOF](../plugins/library-desk/tests/organize.test.ts) [LIBRARYCONTENTNATIVE](../docs/evidence/library-content-composition-2026-09-11.json) | B03 |
...
| <a id="READ03"></a>READ03 | 按章节/标注/href/CFI 跳转 | 实装 | **接通（待 E2E）**：open_book 解析章节/标注/sectionIndex/anchor/href/fraction 或 find_book_locations 返回的 location 并等待真实落点<br>[设计] 验证后导航工具 | **接通（待 E2E）**：reading 2.18 goTo(cfi/href/contentVersion/location) + library 1.17 searchLocations<br>[设计] 导航命令 + 实际落点 | 阅读目录/标注；Agent；RSS 文章；Jumper | 无位置标注明确失败，缺失 href 与 stale 版本有错误码；正文搜索（find_book_locations / searchLocations）产出版本化 location，可直接交给 open_book / goTo，等待实际落点。全部 fragment/CFI 失效边界与各格式真实导航属第三段。 | [READTOOLS](../packages/agent/src/tools/reader-tools.ts) [NAVADAPTER](../apps/web/src/features/reader/lib/reading-engine-adapter.ts) [CTX](../apps/web/src/features/plugins/runtime/plugin-context.ts) [NAV](../apps/web/src/domain/reading-session-controller.ts) [NAVPROBE](../apps/web/tests/desktop/desktop-reading-probe.ts) | A05, D03 |
| <a id="READ04"></a>READ04 | 前后翻页、章节、书首书尾 | 实装 | **接通**：navigate_reading(page/chapter/section/start/end)<br>[设计] 导航步骤工具 | **接通**：reading 2.13 commands.step(page/chapter/section/start/end,guard?)<br>[设计] 导航步骤命令 | 原生章节快捷键与公共 step 共用 TOC 目标算法；页面按钮/滚轮 | next/previous-chapter 按扁平 TOC 的可导航节点（含子节）移动，保留同文件 fragment，跳过连续重复目标；无当前匹配时沿用原生首/末目标，缺 TOC 或边界保持实际位置。next/previous-section 仍按线性源分节，首尾仍用 fraction0/1，不解释印刷章号。章节/分节/首尾记共享历史，普通页步进不记；会话 guard、队列、超时与固定版式 paint 保留。接线及定向测试通过，真实 Tauri/Worker 多格式与快捷键组合留集中验收，不将接通标记当作 E2E 已验。 Jumper 0.4 已组合八种受guard保护的步进、源分节/页码目录、精确标签和分节输入、当前分节屏幕计数；保留版本/重复目标，不建第二套历史或全书页数。19项插件测试与构建入口通过，真实多格式/Worker/Tauri新路径待集中验收。 | [NAV](../apps/web/src/domain/reading-session-controller.ts) [NAVADAPTER](../apps/web/src/features/reader/lib/reading-engine-adapter.ts) [NAVADAPTERPROOF](../apps/web/src/features/reader/lib/reading-engine-adapter.test.ts) [SHORTCUT](../apps/web/src/features/settings/lib/shortcuts.ts) [API](../packages/plugin-types/src/index.ts) [REGISTRY](../packages/agent/src/tools/registry.ts) [JUMPERNAVIGATION](../plugins/jumper/src/navigation.ts) [JUMPERNAVIGATIONPROOF](../plugins/jumper/tests/navigation.test.ts) | D04 |
| <a id="READ05"></a>READ05 | 按进度/固定版式页索引定位 | 实装 | **接通**：open_book；list_book_navigation_targets / get_reading_session[双域]<br>[设计] 进度/页位置与当前布局工具 | **接通**：reading 2.14 goTo / session.pagination；library 1.14 listNavigationTargets<br>[设计] seek 与布局快照/观察 | 阅读进度拖动；共享阅读快照 | 0–1进度与版本化零起始源分节已接；sectionIndex不可混用其他定位，PDF/漫画每节一源页，双页展示不改索引，结果为真实落点并入历史。library 1.14分页列出源分节/page-list和精确标签，重复目标不猜测，失效/外链不给位置；EPUB/PDF标签同源，PDF延迟合并读取、成功缓存/失败重试且关闭拒绝迟到。reading 2.14共享pagination含布局/流/源分节，以及当前重排分节屏幕index/count；扣除前后占位屏，viewport/spread不等于单栏或印刷页。滚动/固定版式/无效几何screen=null，未就绪pagination=null，不测量未打开分节或估算全书页数。引擎重排/relocate及导航完成刷新，快照复制、生命周期清空，同会话替换引擎拒绝旧事件；数字不可持久定位。基础及双端接线测试通过，真实多格式/字体/窗口/多栏/RTL/PDF.js Worker/Tauri留集中验收，接通不等于端到端已验。 Jumper 0.4 已组合八种受guard保护的步进、源分节/页码目录、精确标签和分节输入、当前分节屏幕计数；保留版本/重复目标，不建第二套历史或全书页数。19项插件测试与构建入口通过，真实多格式/Worker/Tauri新路径待集中验收。 | [READTOOLS](../packages/agent/src/tools/reader-tools.ts) [NAV](../apps/web/src/domain/reading-session-controller.ts) [NAVADAPTER](../apps/web/src/features/reader/lib/reading-engine-adapter.ts) [NAVADAPTERPROOF](../apps/web/src/features/reader/lib/reading-engine-adapter.test.ts) [API](../packages/plugin-types/src/index.ts) [NAVTARGETS](../apps/web/src/features/library/lib/book-navigation-targets.ts) [NAVTARGETTYPES](../packages/core/src/book-navigation-targets.ts) [NAVTARGETPROOF](../apps/web/tests/book-navigation-targets.test.ts) [PDFPAGES](../apps/web/foliate-js/src/pdf-navigation.ts) [PDFPAGEPROOF](../apps/web/tests/foliate-pdf.test.ts) [PAGINATION](../apps/web/src/features/reader/lib/reading-pagination.ts) [PAGINATIONPROOF](../apps/web/src/features/reader/lib/reading-pagination.test.ts) [JUMPERNAVIGATION](../plugins/jumper/src/navigation.ts) [JUMPERNAVIGATIONPROOF](../plugins/jumper/tests/navigation.test.ts) | D05 |
| <a id="READ06"></a>READ06 | 导航历史 back/forward 及可用性 | 实装 | **接通**：navigate_reading(back/forward) + 快照可用性<br>[设计] 导航历史工具 | **接通**：reading.commands.back/forward + snapshot.history<br>[设计] 共享历史查询/命令 | 共享阅读控制器；目录/标注/进度/完成页回访/普通内链/章节步进 | 原生普通内链先让脚注处理器同步认领预览，未认领的链接阻止引擎默认跳转，带当前书/源版本进入共享队列；失败不退回私有导航，过期/退休/被新意图取代不更新历史或显示旧错误。完成页延迟回访也走共享跳转，绑定点击时 view/session，卸载取消计时器。最多100个成功落点；失败不入历史、back/forward 不分支、新跳转截断 forward。普通翻页/文本单元内部定位/启动恢复不算显式跳转。定向测试通过；真实 Tauri/Worker 的链接、脚注、完成页动画及所有格式入口仍待集中 E2E。 | [NAV](../apps/web/src/domain/reading-session-controller.ts) [NAVTEST](../apps/web/src/domain/reading-session-controller.test.ts) [NAVPROBE](../apps/web/tests/desktop/desktop-reading-probe.ts) [API](../packages/plugin-types/src/index.ts) [READTOOLS](../packages/agent/src/tools/reader-tools.ts) [NATIVELINKNAV](../apps/web/src/features/reader/lib/native-link-navigation.ts) [NATIVELINKPROOF](../apps/web/src/features/reader/lib/native-link-navigation.test.ts) | D06, D07 |
| <a id="READ07"></a>READ07 | 统一当前书/位置/加载/历史快照 | 实装 | **接通**：get_reading_session（书内 scope 不泄露其他书）<br>[设计] 显式会话查询工具 | **接通**：reading.queries.session()<br>[设计] 当前会话快照 | 共享控制器；Agent；可中途启用的插件 | revision/sessionId/bookId/status/location/history 已共享；2.1 新增 playback、2.2 新增 mode、2.3 新增版本化 mode.position（READ18/READ16）。2.5 新增 mode.availableModes；此行不代表正文 selection 已接通 | [NAV](../apps/web/src/domain/reading-session-controller.ts) [READING](../apps/web/src/domain/reading.ts) [READTOOLS](../packages/agent/src/tools/reader-tools.ts) [API](../packages/plugin-types/src/index.ts) [NAVPROBE](../apps/web/tests/desktop/desktop-reading-probe.ts) | D01, D12, Q02 |
...
| <a id="READ17"></a>READ17 | 列声音并合成音频的提供者 | 实装 | **接通**：get_setting_options 查询 TTS 动态声音；update_settings 选择配置<br>[设计] 受控声源选择 | **接通**：voiceProviders.register/listVoices/synthesize<br>[设计] 语音贡献 | tts；宿主系统语音回退 | 声音配置按既有插件设置选择；settings1.10已接动态选项，不另建voice枚举/任意合成模型工具。提供者启用后仍按宿主既有第一个可用提供者规则，不新增声源优先级选择器。Agent不能借设置接口声称已经触发朗读；真实TTS服务商/播放组合待集中验收 | [TTS](../plugins/tts/src/index.ts) [CTX](../apps/web/src/features/plugins/runtime/plugin-context.ts) [API](../packages/plugin-types/src/index.ts) [AUDIO](../apps/web/src/features/reader/hooks/useReadAloud.ts) [DYNAMICOPTIONS](../apps/web/src/domain/settings/dynamic-options.ts) [PLUGINOPTIONSOURCE](../apps/web/src/domain/settings/plugin-option-source.ts) | K03, K05 |
| <a id="READ18"></a>READ18 | 开始/停止朗读、播放位置与 fallback 状态 | 实装 | **接通**：control_read_aloud + get_reading_session.playback<br>[设计] 朗读控制工具 | **接通**：reading 2.1 controlPlayback + session.playback/observeSession<br>[设计] 共享朗读状态与命令 | 文本单元朗读按钮；Agent；Listening Desk 0.10 | 实际音频开始才回执；owner 取消/切书阻断迟到合成，系统 fallback 与单元 CFI 可观察。自动朗读消费公共 stepMode，等待真实下一单元才播，end-of-book 正常停止；单元位置保存失败停止自动朗读并保留 db code，不继续播下一段。停止/换声源取消在途步进，不被本次 React 落点反馈自我取消。35s advance 截止覆盖 30s 导航，不再用 6s 超时猜书尾。隔离 macOS Tauri 已验证；未验 release/Windows/Linux/远端 TTS。非逐字播放时间/暂停恢复。Listening Desk 0.10新增独立实时状态/启停，原模式表单仍按需刷新；操作冻结guard并用Reading2.18 signal，不输出正文/选区，准备中不伪造百分比。15项插件测试及构建/类型通过，新实时路径的真实音频/Worker/Tauri集中后置。 | [PLAYBACK](../apps/web/src/features/reader/lib/read-aloud-controller.ts) [AUDIO](../apps/web/src/features/reader/hooks/useReadAloud.ts) [READTOOLS](../packages/agent/src/tools/reader-tools.ts) [API](../packages/plugin-types/src/index.ts) [LISTENINGDESK](../plugins/listening-desk/src/views.ts) [PLAYBACKPROOF](../docs/evidence/reading-playback-2026-09-09.json) [MODESTEP](../docs/evidence/reading-mode-step-2026-09-09.json) [POSITIONDURABILITY](../docs/evidence/reading-position-durability-2026-09-09.json) [LISTENINGMONITOR](../plugins/listening-desk/src/monitor.ts) [LISTENINGMONITORPROOF](../plugins/listening-desk/tests/monitor.test.ts) | K04, K05 |
...
| <a id="READ20"></a>READ20 | 跨书/并发导航的取消、序列化与回执 | 部分 | **部分**：共享控制器 + Agent AbortSignal + 会话/书籍 guard<br>[设计] 全来源导航事务 | **部分**：reading 2.18末参数options.signal + 既有guard/停用/Promise<br>[设计] 全来源导航事务 | 两端 API；导航取消基础探针 | 新意图淘汰旧请求，同引擎串行、不同引擎互不阻塞，30s导航截止。16个阅读控制方法已从Worker接单调用signal并保留guard位置，原RPC120s上限也到达控制器；不取消兄弟操作，不回滚已经呈现/保存的效果。关闭中已受理的保存排空不被撤销；引擎本身无逐次物理abort，不能关闭完整GAP。真实Worker传输/正式host注入和控制器基础回归通过，业务插件及Tauri实机集中后置。 | [READTOOLS](../packages/agent/src/tools/reader-tools.ts) [NAV](../apps/web/src/domain/reading-session-controller.ts) [NAVTEST](../apps/web/src/domain/reading-session-controller.test.ts) [NAVADAPTER](../apps/web/src/features/reader/lib/reading-engine-adapter.ts) [WIRE](../apps/web/src/features/plugins/runtime/plugin-worker-host.ts) [CALLOPTIONS](../apps/web/src/features/plugins/runtime/plugin-call-options.ts) [CALLOPTIONSPROOF](../apps/web/src/features/plugins/runtime/plugin-call-options.test.ts) | D07, D10 |
...
| <a id="ANN02"></a>ANN02 | 创建高亮 | 实装 | **接通**：create_annotation(kind=highlight)<br>[设计] 写工具 | **接通**：annotations.commands.createHighlight<br>[设计] 写领域 | Annotation Desk 0.4 选区创建；选择菜单；Agent | 来源与 Range 校验仍受 TXT13 限制Annotation Desk0.4只消费既有命令：复制所选书籍/文字/CFI/章节，四色与highlight/underline均可选，成功回执再查看/编辑/导出，不以随后读失败重复创建。无锚点明确提示且保留null，不能伪造或保证源版本条件写；25项插件测试/构建/类型通过，含编译入口，真实Worker/Tauri新路径集中后置。2026-09-11真实Worker/Tauri组合已验Text Desk搜索/选区→原生More→Annotation Desk创建引文笔记/蓝色下划线，编辑保留quote/CFI，另建无锚点书籍笔记；未确认删除保持3条，确认批次后查询/列表/正文标记归零。600/900宽前台画面已验，无新宿主接口；不是物理拖选/完整键盘、其他颜色/格式、重启、源版本条件写或Agent模型证据。 | [ANNOT](../apps/web/src/domain/annotations.ts) [ANNTOOLS](../packages/agent/src/tools/annotation-tools.ts) [CTX](../apps/web/src/features/plugins/runtime/plugin-context.ts) [TEXTACTIONS](../apps/web/src/features/reader/hooks/useReaderTextActions.ts) [DESKCREATE](../plugins/annotation-desk/src/creation.ts) [DESKCREATEPROOF](../plugins/annotation-desk/tests/creation.test.ts) [DESKCREATENATIVE](../docs/evidence/annotation-composition-2026-09-11.json) | F02, F04 |
| <a id="ANN03"></a>ANN03 | 创建下划线样式 | 实装 | **接通**：create_annotation(style=underline) → 共享命令<br>[设计] 标注样式参数 | **接通**：createHighlight(style=underline)<br>[设计] 写领域 | Annotation Desk 0.4 选区创建；选择菜单；Agent；桌面 Worker 探针 | style 经 tool/port/事件持久化及 Worker 查询保真；默认 highlight，非法 style 拒绝。实机已验收落盘和回读，不代表锚定下划线的全部格式视觉验收Annotation Desk0.4只消费既有命令：复制所选书籍/文字/CFI/章节，四色与highlight/underline均可选，成功回执再查看/编辑/导出，不以随后读失败重复创建。无锚点明确提示且保留null，不能伪造或保证源版本条件写；25项插件测试/构建/类型通过，含编译入口，真实Worker/Tauri新路径集中后置。2026-09-11真实Worker/Tauri组合已验Text Desk搜索/选区→原生More→Annotation Desk创建引文笔记/蓝色下划线，编辑保留quote/CFI，另建无锚点书籍笔记；未确认删除保持3条，确认批次后查询/列表/正文标记归零。600/900宽前台画面已验，无新宿主接口；不是物理拖选/完整键盘、其他颜色/格式、重启、源版本条件写或Agent模型证据。 | [ANNOT](../apps/web/src/domain/annotations.ts) [ANNTOOLS](../packages/agent/src/tools/annotation-tools.ts) [ANNPORT](../apps/web/src/features/ai/agent/ports/annotations-port.ts) [ANNPROBE](../apps/web/tests/desktop/desktop-annotation-probe.ts) [TEXTACTIONS](../apps/web/src/features/reader/hooks/useReaderTextActions.ts) [API](../packages/plugin-types/src/index.ts) [DESKCREATE](../plugins/annotation-desk/src/creation.ts) [DESKCREATEPROOF](../plugins/annotation-desk/tests/creation.test.ts) [DESKCREATENATIVE](../docs/evidence/annotation-composition-2026-09-11.json) | F02 |
...
| <a id="ANN05"></a>ANN05 | 创建/编辑/删除笔记 | 实装 | **接通**：create/edit/delete_annotation<br>[设计] 受控写工具 | **接通**：annotations 2 createNote；applyChanges 条件编辑/删除<br>[设计] 写领域 | NoteEditor；Agent；Annotation Desk 0.4 创建/编辑/删除 | 可无位置笔记；Annotation Desk 0.4 已接书籍笔记与选区引文笔记创建。原生编辑器冻结所见 note/token，冲突留草稿；保存中去重并禁用输入/提交，旧草稿完成不关闭替代草稿。原生列表/书籍详情及全局标注弹层删除同样使用所见版本，全局远端先改后删冲突已验，重读后删除成功。实际 Worker 与 Agent 修改后的旧草稿冲突、重新打开后保存已验；100 个存储及两个封面同步命令迁入阻塞工作池，锁等待不占 UI/异步执行线程；封面协议异步响应。隔离 macOS 已验真实锁中 busy/Cancel/重开、释放后旧保存成功且新草稿保留、超时本地化失败及重试；排队查询/封面请求不阻断界面。取消不撤销已受理写；启动与原生窗口退出仍有同步路径，非全部原生工作无阻塞保证，最大队列/载荷和跨平台未验。删除 Agent 批准和插件授权不是同一种策略Annotation Desk0.4只消费既有命令：复制所选书籍/文字/CFI/章节，四色与highlight/underline均可选，成功回执再查看/编辑/导出，不以随后读失败重复创建。无锚点明确提示且保留null，不能伪造或保证源版本条件写；25项插件测试/构建/类型通过，含编译入口，真实Worker/Tauri新路径集中后置。2026-09-11真实Worker/Tauri组合已验Text Desk搜索/选区→原生More→Annotation Desk创建引文笔记/蓝色下划线，编辑保留quote/CFI，另建无锚点书籍笔记；未确认删除保持3条，确认批次后查询/列表/正文标记归零。600/900宽前台画面已验，无新宿主接口；不是物理拖选/完整键盘、其他颜色/格式、重启、源版本条件写或Agent模型证据。 | [ANNOT](../apps/web/src/domain/annotations.ts) [ANNTOOLS](../packages/agent/src/tools/annotation-tools.ts) [CTX](../apps/web/src/features/plugins/runtime/plugin-context.ts) [DESK](../plugins/annotation-desk/src/views.ts) [NATIVEANNWRITES](../apps/web/src/features/annotations/lib/native-annotation-mutations.ts) [NATIVEANNEDITOR](../apps/web/src/features/reader/hooks/useReaderNoteEditor.ts) [NATIVECASPROOF](../docs/evidence/native-annotation-cas-2026-09-10.json) [ANNCONTRACTPROOF](../docs/evidence/annotation-contract-v2-2026-09-10.json) [ANNCOLLECTIONS](../apps/web/src/features/annotations/hooks/useAnnotations.ts) [ANNGLOBAL](../apps/web/src/features/agent/components/AnnotationsPopover.tsx) [STORAGEEXEC](../apps/desktop/src-tauri/src/storage/execution.rs) [STORAGEEXECPROOF](../docs/evidence/storage-execution-2026-09-10.json) [DESKCREATE](../plugins/annotation-desk/src/creation.ts) [DESKCREATEPROOF](../plugins/annotation-desk/tests/creation.test.ts) [DESKCREATENATIVE](../docs/evidence/annotation-composition-2026-09-11.json) | F02 |
| <a id="ANN06"></a>ANN06 | 读取/删除 ask 问题轨迹 | 实装 | **接通**：get_annotations / delete_annotation+批准<br>[设计] 查询/受控删除 | **接通**：annotations 2 queries.get/list/inspect + applyChanges remove(kind:ask)<br>[设计] 查询/受控删除 | Agent；注释视图；隔离桌面 Worker | 写权限包含受控删除，read 不导出 commands，createAsk 仍不开放。删除必须带所见 revision，无 removeAsk 兼容别名；缺失/错误类型返回 annotations/not-found；实际 Agent 批准端口拒绝保留、批准删除已测，未代替聊天批准 UI 验收 | [ANNOT](../apps/web/src/domain/annotations.ts) [ANNTOOLS](../packages/agent/src/tools/annotation-tools.ts) [ANNPORT](../apps/web/src/features/ai/agent/ports/annotations-port.ts) [ANNPROBE](../apps/web/tests/desktop/desktop-annotation-probe.ts) [ANNCONTRACTPROOF](../docs/evidence/annotation-contract-v2-2026-09-10.json) [CTX](../apps/web/src/features/plugins/runtime/plugin-context.ts) | F05 |
...
| <a id="CFG07"></a>CFG07 | AI 提供商/端点/密钥配置 | 实装 | **接通**：只读 provider/credentialConfigured + open_maintenance_settings(ai-connection) 定位宿主配置<br>[设计] 打开宿主敏感配置流程 | **接通**：受权读非敏感存在状态 + maintenance.openSettings(ai-connection)<br>[设计] 打开宿主敏感配置流程 | AIConfigPanel；Maintenance Desk | 不开放：读取宿主密钥；不能把 readonly provider 算作可切换提供商。opened 只表示定位到面板，密钥输入与连接测试由用户在原生面板完成 | [AICONFIG](../apps/web/src/features/ai/lib/ai-config.ts) [AICONFIGUI](../apps/web/src/features/settings/components/AIConfigPanel.tsx) [SETTINGS](../apps/web/src/domain/settings/catalog.ts) [SECRETS](../apps/web/src/platform/secret-store.ts) | H04 |
| <a id="CFG08"></a>CFG08 | 模型目录刷新、连接测试与模型能力 | 实装 | **接通**：get_model_catalog/refresh_model_catalog/request_ai_connection_test[双域]<br>[设计] 目录、元数据及原生测试回执 | **接通**：settings1.9 modelCatalog/refreshModelCatalog；maintenance1.3 requestConnectionTest<br>[设计] 目录、元数据及原生测试回执 | AI 配置页、目录缓存与原生主模型测试 | 目录需主/快模型路径发现权，刷新另需network；默认25/最多100行、120字符搜索、进程revision防旧续页，不给选择/保留旧模型/密钥/端点/价格。自定义/Relay/Codex目录不支持，刷新失败保留旧缓存但拒绝成功。测试请求只定位原生按钮，用户点击才用当前表单发主模型测试，可产生费用；返回responded/empty/cancelled或错误，不给模型身份/回复。改配置或卸载淘汰旧成功，已开始源等结算、不因取消退款或强杀；responded非持久保存/完整能力/Fast模型证明，无新提供者截止或耐久任务。基础授权/分页/挂载/Agent通过；Maintenance Desk真实macOS Worker已验公共目录刷新39行/分页及原生测试交接取消，未执行模型推理，完整Agent/提供者组合仍待验。 | [MODELCATALOG](../apps/web/src/features/ai/lib/model-catalog.ts) [MODELCATALOGDOMAIN](../apps/web/src/domain/settings/model-catalog.ts) [MODELCATALOGPROOF](../apps/web/src/domain/settings/model-catalog.test.ts) [MODELCATALOGTOOLS](../packages/agent/src/tools/model-catalog-tools.ts) [CONNECTIONTESTFLOW](../apps/web/src/features/settings/hooks/useAIConnectionTest.ts) [CONNECTIONTESTPROOF](../apps/web/src/features/settings/hooks/useAIConnectionTest.test.tsx) [AICONFIGUI](../apps/web/src/features/settings/components/AIConfigPanel.tsx) [SETTINGS](../apps/web/src/domain/settings/catalog.ts) [MAINTENANCEDESKNATIVE](../docs/evidence/maintenance-desk-2026-09-11.json) | H04 |
...
| <a id="SET18"></a>SET18 | ai.preferences.features.explainSelection | 实装 | **接通（待 E2E）**：get_settings/update_settings + explain_selection（双scope）<br>[设计] 类型化设置工具 | **接通（待 E2E）**：settings discover/read/update（需路径授权）<br>[设计] 类型化设置领域 | 设置页/授权设置插件；选区菜单/命令面板/Agent动作；既有本书聊天 | 实现关闭条件已满足，接通（待 E2E）：四动作共用生产 ReadingAiActions，选区 More 菜单、命令面板和双 scope Agent 工具随对应开关显示/移除，缓存工具执行再次复核，独立于 askConversation。固定当前书/会话/来源版本与选区或章节；完整选区最多32000 UTF-16，预览不足按版本化 Range 续读，缺失/过长拒绝不截断。章节意图固定提取章节序号，要求既有 Agent 读取整章而非视口；翻译跟随界面语言。原生/全局入口打开本书聊天，等待真实 send 接受；loading 可等，忙/取消/目标失效/关闭不可迟到发送，同步已接受回执不被晚取消覆盖。书内 Agent 返回上下文并在当前回合回答，不递归发送。文本隐私和 localOnly 同一执行门控，等待中 off/on 永久作废旧调用，已接受推理沿用聊天停止/错误/重试。定向服务、实际端口、挂载 UI 和 Agent 请求间发现测试通过，不是模型效果证明；正式插件设置持久/撤权、真实 Tauri 菜单/命令/对话、全格式整章/长选区、失败并发和 packaged 依 host-capability-stage-three.md 验收。 | [SETTINGS](../apps/web/src/domain/settings/catalog.ts) [SETDOMAIN](../apps/web/src/domain/settings/domain.ts) [SETTOOLS](../packages/agent/src/tools/settings-tools.ts) [READINGAIACTIONS](../apps/web/src/services/reading-ai-actions.ts) [READINGAIRUNTIME](../apps/web/src/services/reading-ai-runtime.ts) [READINGAITOOLS](../packages/agent/src/tools/reading-ai-tools.ts) [READINGAIPROOF](../apps/web/src/services/reading-ai-actions.test.ts) [READINGAISURFACES](../apps/web/src/features/ai/hooks/reading-ai-surfaces.test.tsx) [READINGAIPORTPROOF](../apps/web/src/features/ai/agent/ports/reading-ai-port.test.ts) | 新增盘点 |
| <a id="SET19"></a>SET19 | ai.preferences.features.defineTerm | 实装 | **接通（待 E2E）**：get_settings/update_settings + define_term（双scope）<br>[设计] 类型化设置工具 | **接通（待 E2E）**：settings discover/read/update（需路径授权）<br>[设计] 类型化设置领域 | 设置页/授权设置插件；选区菜单/命令面板/Agent动作；既有本书聊天 | 实现关闭条件已满足，接通（待 E2E）：四动作共用生产 ReadingAiActions，选区 More 菜单、命令面板和双 scope Agent 工具随对应开关显示/移除，缓存工具执行再次复核，独立于 askConversation。固定当前书/会话/来源版本与选区或章节；完整选区最多32000 UTF-16，预览不足按版本化 Range 续读，缺失/过长拒绝不截断。章节意图固定提取章节序号，要求既有 Agent 读取整章而非视口；翻译跟随界面语言。原生/全局入口打开本书聊天，等待真实 send 接受；loading 可等，忙/取消/目标失效/关闭不可迟到发送，同步已接受回执不被晚取消覆盖。书内 Agent 返回上下文并在当前回合回答，不递归发送。文本隐私和 localOnly 同一执行门控，等待中 off/on 永久作废旧调用，已接受推理沿用聊天停止/错误/重试。定向服务、实际端口、挂载 UI 和 Agent 请求间发现测试通过，不是模型效果证明；正式插件设置持久/撤权、真实 Tauri 菜单/命令/对话、全格式整章/长选区、失败并发和 packaged 依 host-capability-stage-three.md 验收。 | [SETTINGS](../apps/web/src/domain/settings/catalog.ts) [SETDOMAIN](../apps/web/src/domain/settings/domain.ts) [SETTOOLS](../packages/agent/src/tools/settings-tools.ts) [READINGAIACTIONS](../apps/web/src/services/reading-ai-actions.ts) [READINGAIRUNTIME](../apps/web/src/services/reading-ai-runtime.ts) [READINGAITOOLS](../packages/agent/src/tools/reading-ai-tools.ts) [READINGAIPROOF](../apps/web/src/services/reading-ai-actions.test.ts) [READINGAISURFACES](../apps/web/src/features/ai/hooks/reading-ai-surfaces.test.tsx) [READINGAIPORTPROOF](../apps/web/src/features/ai/agent/ports/reading-ai-port.test.ts) | 新增盘点 |
| <a id="SET20"></a>SET20 | ai.preferences.features.translate | 实装 | **接通（待 E2E）**：get_settings/update_settings + translate_selection（双scope）<br>[设计] 类型化设置工具 | **接通（待 E2E）**：settings discover/read/update（需路径授权）<br>[设计] 类型化设置领域 | 设置页/授权设置插件；选区菜单/命令面板/Agent动作；既有本书聊天 | 实现关闭条件已满足，接通（待 E2E）：四动作共用生产 ReadingAiActions，选区 More 菜单、命令面板和双 scope Agent 工具随对应开关显示/移除，缓存工具执行再次复核，独立于 askConversation。固定当前书/会话/来源版本与选区或章节；完整选区最多32000 UTF-16，预览不足按版本化 Range 续读，缺失/过长拒绝不截断。章节意图固定提取章节序号，要求既有 Agent 读取整章而非视口；翻译跟随界面语言。原生/全局入口打开本书聊天，等待真实 send 接受；loading 可等，忙/取消/目标失效/关闭不可迟到发送，同步已接受回执不被晚取消覆盖。书内 Agent 返回上下文并在当前回合回答，不递归发送。文本隐私和 localOnly 同一执行门控，等待中 off/on 永久作废旧调用，已接受推理沿用聊天停止/错误/重试。定向服务、实际端口、挂载 UI 和 Agent 请求间发现测试通过，不是模型效果证明；正式插件设置持久/撤权、真实 Tauri 菜单/命令/对话、全格式整章/长选区、失败并发和 packaged 依 host-capability-stage-three.md 验收。 | [SETTINGS](../apps/web/src/domain/settings/catalog.ts) [SETDOMAIN](../apps/web/src/domain/settings/domain.ts) [SETTOOLS](../packages/agent/src/tools/settings-tools.ts) [READINGAIACTIONS](../apps/web/src/services/reading-ai-actions.ts) [READINGAIRUNTIME](../apps/web/src/services/reading-ai-runtime.ts) [READINGAITOOLS](../packages/agent/src/tools/reading-ai-tools.ts) [READINGAIPROOF](../apps/web/src/services/reading-ai-actions.test.ts) [READINGAISURFACES](../apps/web/src/features/ai/hooks/reading-ai-surfaces.test.tsx) [READINGAIPORTPROOF](../apps/web/src/features/ai/agent/ports/reading-ai-port.test.ts) | 新增盘点 |
| <a id="SET21"></a>SET21 | ai.preferences.features.summarizeChapter | 实装 | **接通（待 E2E）**：get_settings/update_settings + summarize_chapter（双scope）<br>[设计] 类型化设置工具 | **接通（待 E2E）**：settings discover/read/update（需路径授权）<br>[设计] 类型化设置领域 | 设置页/授权设置插件；选区菜单/命令面板/Agent动作；既有本书聊天 | 实现关闭条件已满足，接通（待 E2E）：四动作共用生产 ReadingAiActions，选区 More 菜单、命令面板和双 scope Agent 工具随对应开关显示/移除，缓存工具执行再次复核，独立于 askConversation。固定当前书/会话/来源版本与选区或章节；完整选区最多32000 UTF-16，预览不足按版本化 Range 续读，缺失/过长拒绝不截断。章节意图固定提取章节序号，要求既有 Agent 读取整章而非视口；翻译跟随界面语言。原生/全局入口打开本书聊天，等待真实 send 接受；loading 可等，忙/取消/目标失效/关闭不可迟到发送，同步已接受回执不被晚取消覆盖。书内 Agent 返回上下文并在当前回合回答，不递归发送。文本隐私和 localOnly 同一执行门控，等待中 off/on 永久作废旧调用，已接受推理沿用聊天停止/错误/重试。定向服务、实际端口、挂载 UI 和 Agent 请求间发现测试通过，不是模型效果证明；正式插件设置持久/撤权、真实 Tauri 菜单/命令/对话、全格式整章/长选区、失败并发和 packaged 依 host-capability-stage-three.md 验收。 | [SETTINGS](../apps/web/src/domain/settings/catalog.ts) [SETDOMAIN](../apps/web/src/domain/settings/domain.ts) [SETTOOLS](../packages/agent/src/tools/settings-tools.ts) [READINGAIACTIONS](../apps/web/src/services/reading-ai-actions.ts) [READINGAIRUNTIME](../apps/web/src/services/reading-ai-runtime.ts) [READINGAITOOLS](../packages/agent/src/tools/reading-ai-tools.ts) [READINGAIPROOF](../apps/web/src/services/reading-ai-actions.test.ts) [READINGAISURFACES](../apps/web/src/features/ai/hooks/reading-ai-surfaces.test.tsx) [READINGAIPORTPROOF](../apps/web/src/features/ai/agent/ports/reading-ai-port.test.ts) | 新增盘点 |
...
| <a id="AI02"></a>AI02 | 创建/切换/清空全局线程和书内聊天 | 实装 | **接通**：get_conversation_state 双 scope；manage_conversation 全局<br>[设计] 创建/选择及经批准清空其他线程 | **接通**：conversations 1.1 commands.createThread/selectThread/clear<br>[设计] conversations:write 受控命令 | AgentWorkspace；ChatPanel；Agent 工具；插件正式入口 | 创建持久选择的空全局草稿，首次消息才创建 transcript 行；选择不等于导航/可见，未知线程拒绝。clear 先停止并等在途回合最后写入，再清 transcript 与隐藏线程/insights；不删除长期记忆、事件历史或已完成工具副作用，多步清空不承诺原子回滚。Agent 不通过执行中的工具停止/清空自身，以免死锁，当前线程仍由原生控件管理。权限/持久选择失败/排空定向测试通过；新组合插件与 Tauri E2E 待集中验收。 Memory Desk 0.9 已组合线程创建/选择、运行态观察、宿主确认请求、自有请求状态/取消、停止和确认清空，并消费Agent页头线程目标；41项插件测试、编译回调与受控宿主请求状态机通过。原生批准点击、升级授权、持久化与真实模型留集中桌面验收。2026-09-11 Memory Desk 0.9 命令入口的真实 Worker/Tauri/SQLite 组合已验未勾选清空不写、勾选后清空自有两条消息；测试摘要和线程亦清理，原选择恢复。不冒充新建/切换菜单、生成中停止或完整模型验收。 | [CHAT](../apps/web/src/features/ai/lib/conversation-store.ts) [CHATDOMAIN](../apps/web/src/domain/conversations.ts) [CONVCONTROL](../apps/web/src/domain/conversation-control.ts) [CONVRUNTIME](../apps/web/src/domain/conversation-runtime.ts) [CONVTOOLS](../packages/agent/src/tools/conversation-control-tools.ts) [AGENTUI](../apps/web/src/features/agent/components/AgentWorkspace.tsx) [CHATCONTROL](../apps/web/src/features/ai/hooks/useBookConversation.ts) [REGISTRY](../packages/agent/src/tools/registry.ts) [MEMORYCONVERSATIONS](../plugins/memory-desk/src/conversation-controls.ts) [MEMORYREQUESTS](../plugins/memory-desk/src/turn-requests.ts) [MEMORYCONVERSATIONPROOF](../plugins/memory-desk/tests/conversation-controls.test.ts) [IMAGECONVERSATIONPROOF](../docs/evidence/image-conversation-composition-2026-09-11.json) | L04 |
| <a id="AI03"></a>AI03 | 发送/流式生成/停止/重试聊天回合 | 实装 | **接通**：request_conversation_turn 双 scope；get_conversation_state；manage_conversation stop<br>[设计] 宿主确认后采用/发送/重试，非递归调用自身 | **接通**：conversations 1.2 requestTurn/cancelTurnRequest + queries.turnRequests/runtime/observeRuntime<br>[设计] 受权提交意图、查询结果与停止排空 | 书内/全局聊天确认栏；Agent/插件正式入口 | draft/send 文本 1–65536 字符，retry 不接替代正文或 role；只向已挂载聊天提交，每个目标最多一个待确认请求，五分钟过期，卸载/关闭/停止/清空取消。宿主原生按钮才可采用草稿或启动回合，草稿不覆盖已输入文本；send/retry 要空闲且沿用请求时的 transcript generation，retry 复用原消息/附件。请求不自动导航；pending 不等于接受，adopted 不等于已发，started 不等于生成/持久化完成；流式输出仍由原生聊天承载，不提供任意伪造历史入口。最多保留全宿主 128 个状态记录，queries 仅返回自有无正文摘要，Agent 只看本 scope 最近 20 项并报截断。取消不能撤回已采用草稿/已启动回合或工具副作用；运行态不暴露正文，stop 等最后写入。定向权限/状态/生命周期测试通过，组合插件与 Tauri E2E 待集中验收。 Memory Desk 0.9 已组合线程创建/选择、运行态观察、宿主确认请求、自有请求状态/取消、停止和确认清空，并消费Agent页头线程目标；41项插件测试、编译回调与受控宿主请求状态机通过。原生批准点击、升级授权、持久化与真实模型留集中桌面验收。2026-09-11 Memory Desk 0.9 真实 Worker/Tauri 原生 Use draft 使请求 adopted、草稿进入输入框但未发送；原生 Dismiss request 使 send 请求 dismissed，插件历史正确呈现两状态，消息数均保持2。未调用推理；重试、发送执行、流式/停止与Agent页头未验。 | [THREAD](../packages/agent/src/runtime/thread.ts) [CHATCONTROL](../apps/web/src/features/ai/hooks/useBookConversation.ts) [CHATUI](../apps/web/src/features/ai/components/ChatPanel.tsx) [RUNTIME](../packages/agent/src/runtime/runtime.ts) [API](../packages/plugin-types/src/index.ts) [CONVCONTROL](../apps/web/src/domain/conversation-control.ts) [CONVRUNTIME](../apps/web/src/domain/conversation-runtime.ts) [CONVTOOLS](../packages/agent/src/tools/conversation-control-tools.ts) [CONVREQUESTS](../apps/web/src/domain/conversation-turn-requests.ts) [CONVREQUESTUI](../apps/web/src/features/ai/components/ChatTurnRequest.tsx) [MEMORYCONVERSATIONS](../plugins/memory-desk/src/conversation-controls.ts) [MEMORYREQUESTS](../plugins/memory-desk/src/turn-requests.ts) [MEMORYCONVERSATIONPROOF](../plugins/memory-desk/tests/conversation-controls.test.ts) [IMAGECONVERSATIONPROOF](../docs/evidence/image-conversation-composition-2026-09-11.json) | L05 |
| <a id="AI04"></a>AI04 | 提问、选项澄清、批准/拒绝高风险动作 | 实装 | **接通**：ask_user + InteractionPort<br>[设计] 宿主交互工具 | **部分**：agentTools 1.2 approval=required；普通表单无通用批准票据<br>[设计] 宿主拥有的逐次工具确认 | Agent permission cards；Dictionary/RSS | 声明式工具确认已接：宿主展示来源与完整 JSON 参数，执行获准快照；拒绝/取消/过期/退休/禁用不调用，五分钟截止，参数16384/整个说明24576字符上限。确认不增加权限，不是任意插件UI批准票据，也不保证调用期间数据版本或已派发工作回滚。基础验证通过，真实Worker/Tauri集中验收待做 | [INTERACTION](../packages/agent/src/tools/interaction-tools.ts) [ANNTOOLS](../packages/agent/src/tools/annotation-tools.ts) [SHELFTOOLS](../packages/agent/src/tools/shelf-tools.ts) [API](../packages/plugin-types/src/index.ts) [TOOLAPPROVAL](../apps/web/src/features/plugins/runtime/plugin-tool-confirmation.ts) [TOOLAPPROVALPROOF](../apps/web/src/features/plugins/runtime/plugin-tools.test.ts) | L05 |
| <a id="AI05"></a>AI05 | Agent 展示可点击书卡与词典卡 | 实装 | **接通**：present_books[全局]；插件wordCards/bookCards结果<br>[设计] 结构化结果呈现 | **接通**：agentTools1.3 PluginToolBookCards及既有wordCards<br>[设计] 宿主解析书籍ID并渲染 | Agent书卡；Dictionary词卡；正式插件上下文基础探针 | 新书卡要求注册插件library读/写授权；1..24个仅含bookId的引用，每ID非空至多256字符，去重并用同源书库的真实title/author，拒伪造resolver/标题/封面/URL和混合wordCards。global可用书库，book仅当前书；未知/超scope跳过，读故障拒绝。模型仅收gist及ID回执，宿主details.reference复用书卡/封面/点击导航，批准details保留。Thread与present_books共享本轮ID去重，下一轮可重现；prepared引用不是绘制保证，不自动开书/读取正文，不抹掉已展示卡。16项基础/脚本线程检查通过；compiledWorker/业务插件/真实Tauri点击集中后置。固定工具结果卡与PluginView不同，不能任意互换，计数非总内存上限。 | [PRESENT](../packages/agent/src/tools/present-tools.ts) [EXTOOLS](../apps/web/src/features/plugins/runtime/plugin-tools.ts) [API](../packages/plugin-types/src/index.ts) [DICTTOOLS](../plugins/dictionary/src/agent-tools.ts) [PLUGINBOOKCARDS](../apps/web/src/features/plugins/runtime/plugin-book-cards.ts) [PLUGINBOOKCARDSPROOF](../apps/web/src/features/plugins/runtime/plugin-book-cards.test.ts) [PRESENTFLOW](../packages/agent/src/runtime/present-flow.test.ts) [CTX](../apps/web/src/features/plugins/runtime/plugin-context.ts) | 新增盘点 |
| <a id="AI06"></a>AI06 | 一次性文本/结构化/流式 LLM 推理 | 实装 | **自动**：Runtime ask/askDetailed + 线程推理<br>[设计] 运行时服务 | **接通**：llm 1.4 ask/askDetailed：readingContext/schema/onText/signal/timeoutMs/maxOutputTokens/requestId<br>[设计] 受预算约束的推理服务 | Dictionary 1.3；Agent 后台管线；推理基础探针 | readingContext仍由宿主过滤，收紧立即取消；必需正文禁止报ai/context-withheld，非法结构报ai/invalid-reading-context。两接口信号穿Worker到普通/结构化/流式源；结构化最多两次共用截止，流式等待每次onText回调，取消后不发旧增量。1.3成功metadata含每次尝试，原ask仍只返原结果。任意prompt不做来源推断，字典本地缓存策略不变。已有隐私macOS证据不覆盖本批新路径；基础测试通过，真实Tauri/服务商组合留集中验收。 | [RUNTIME](../packages/agent/src/runtime/runtime.ts) [CTX](../apps/web/src/features/plugins/runtime/plugin-context.ts) [API](../packages/plugin-types/src/index.ts) [DICT](../plugins/dictionary/src/index.ts) [STRUCTUREDREADING](../packages/agent/src/runtime/one-shot.ts) [STRUCTUREDREADINGPROOF](../docs/evidence/structured-reading-context-2026-09-09.json) [PLUGINLLM](../apps/web/src/features/plugins/runtime/plugin-llm.ts) | L01 |
| <a id="AI07"></a>AI07 | 推理取消、超时、用量/预算/成本可见性 | 部分 | **部分**：线程abort；共享Runtime.ask/askDetailed支持终局onAttempt元数据，含失败/晚结算<br>[设计] 统一任务/预算 | **部分**：llm 1.4 requestId/getRequest/listRequests/cancelRequest + askDetailed/policy<br>[设计] 命名请求的失败/取消/超时及晚用量已接；总预算/耐久历史仍缺 | 聊天Stop；插件推理基础探针 | 默认60秒、整数1..110000ms，整次ask含结构化重试；每plugin ID2/全App8个插件推理，跨激活共享，原提供者terminal promise未结算不退槽。askDetailed成功返回value/attempts，每次记录解析模型、停止原因、SDK token计数和USD费用估算，含首次无效结构化尝试，不含原文/密钥/端点；未知或全零用量及零价格目录返回null，不等于免费。两接口可请求每次1..65536输出token，模型/账户/relay可更严，结构化两次各请求一次，不是总token或账单预算。Worker取消保留本地reason，宿主RPC为plugin/cancelled，内部ai/request-cancelled；超时ai/request-timeout，忙ai/busy。1.4可选requestId为1..64 ASCII字母/数字/_/-，每激活保留64个命名请求，只逐出最早已settled项；保留期间重名拒绝，不是幂等重试。getRequest/listRequests返回running/completed/failed/cancelled/timed-out、稳定errorCode、revision与attempts，不保留prompt/output/error prose；未知/逐出为null。cancelRequest独立取消，不依赖已取消RPC；其当次回执可仍running。settled等待原SDK promises，晚用量继续补入，SDK拒绝无message则计数null；共享AgentRuntime的宿主onAttempt同源交付，不新增模型工具或聊天账本。退休清空，不跨激活/进程。Worker本地预取消不派发，因此无宿主条目。不保证SDK网络背压或输出内存上限；总token/费用预算、耐久历史和统一TaskRef仍缺，settled也不是远端账单结算，取消不能保证未收费。定向类型/服务/真实Bun Worker传输已验，真实Tauri/服务商与业务插件组合集中后置。 | [THREAD](../packages/agent/src/runtime/thread.ts) [RUNTIME](../packages/agent/src/runtime/runtime.ts) [CTX](../apps/web/src/features/plugins/runtime/plugin-context.ts) [WIRE](../apps/web/src/features/plugins/runtime/plugin-worker-host.ts) [PLUGINLLM](../apps/web/src/features/plugins/runtime/plugin-llm.ts) [INFERENCERECEIPTS](../apps/web/src/features/plugins/runtime/plugin-inference-receipts.ts) | L02 |
| <a id="AI08"></a>AI08 | 书内 scope/游标/选区自动 grounding | 实装 | **自动**：thread + grounding-context<br>[设计] 自动且有来源的上下文 | **接通**：agentContextProviders / agentRetrievalProviders 补充有来源的数据块，不能覆写核心<br>[设计] 受限上下文贡献 | 书内 Agent；插件上下文/检索提供者 | 自动注入不等于独立工具；不允许插件注入更高优先级系统策略，插件块受同一文本隐私策略过滤 | [THREAD](../packages/agent/src/runtime/thread.ts) [GROUND](../packages/agent/src/runtime/grounding-context.ts) [EXTOOLS](../apps/web/src/features/plugins/runtime/plugin-tools.ts) | L07 |
| <a id="AI09"></a>AI09 | 剧透边界、请求允许超前内容 | 实装 | **接通**：read_chapter/search_book_text/query_book_graph + 批准<br>[设计] 宿主策略 | **接通**：library 域授权读取正文；不存在模型剧透审批（有意）<br>[设计] 数据授权与 Agent 剧透策略分开 | 书内 Agent；插件正文读取 | 访问权限不是剧透许可；插件提供上下文需要 provenance/fence 政策，宿主在装配时按 Agent 围栏过滤而非信任插件声明 | [SPOILER](../packages/agent/src/tools/spoiler-permission.ts) [TEXTTOOLS](../packages/agent/src/tools/book-text-tools.ts) [GRAPHTOOLS](../packages/agent/src/tools/graph-tools.ts) [LIB](../apps/web/src/domain/library.ts) | L07 |
| <a id="AI10"></a>AI10 | 注册供模型使用的插件工具 | 实装 | **扩展**：extraTools(scope) 进入真实 registry<br>[设计] 受 scope/授权控制工具 | **接通**：agentTools.register<br>[设计] 工具贡献 | Dictionary 5 个；RSS 5 个 | 插件安装/启用后才存在；不能将拥有插件 UI 视为已经有 Agent 工具 | [REGISTRY](../packages/agent/src/tools/registry.ts) [PORTS](../apps/web/src/features/ai/agent/ports/index.ts) [EXTOOLS](../apps/web/src/features/plugins/runtime/plugin-tools.ts) [DICTTOOLS](../plugins/dictionary/src/agent-tools.ts) [RSSTOOLS](../plugins/rss-reader/src/agent-tools.ts) | L06 |
| <a id="AI11"></a>AI11 | 每轮上下文 provider | 实装 | **自动**：plugin context provider 注入线程<br>[设计] 受预算/来源约束消费 | **接通**：agentContextProviders.register<br>[设计] 上下文贡献 | Reading Goals 按请求书籍 scope 提供阅读目标 | 真实 Worker 到受控推理服务已验；此前没有专属目标编辑工具；0.4接线如下，不等于任意上下文语义已验收 Reading Goals0.4新增双scope get/set/clear_reading_goal，写入经宿主确认并比较所见revision；UI共用goals文档条件写，下一轮上下文/候选读取同一数据。schema2首次按书提升旧KV，文档/空目标墓碑优先，旧KV清理失败拒绝并可重试。新增agent:tools，沿用Storage2.1而非新宿主API；新协议原生升级/批准/持久化与模型组合待集中验收，不等于直接写入或删除已提升记忆。 | [EXTOOLS](../apps/web/src/features/plugins/runtime/plugin-tools.ts) [THREAD](../packages/agent/src/runtime/thread.ts) [API](../packages/plugin-types/src/index.ts) [READINGGOALS](../plugins/reading-goals/src/index.ts) [MEMORYPOLICYPROOF](../docs/evidence/memory-build-policy-2026-09-09.json) [READINGGOALTOOLS](../plugins/reading-goals/src/tools.ts) [READINGGOALSTORE](../plugins/reading-goals/src/goals.ts) [READINGGOALTOOLPROOF](../plugins/reading-goals/tests/tools.test.ts) | L06 |
...
| <a id="EXT03"></a>EXT03 | 插件页面/对话框/视图结果与栈导航 | 实装 | **未接**：模型不渲染 PluginView 树<br>[设计] 通过工具结构化输出，由宿主呈现 | **部分**：视图栈/嵌套对话框 lease；views 1.9 自有整组关闭<br>[设计] 宿主声明式视图 | Dictionary/RSS/Jumper/Annotation Desk；隔离桌面视图探针 | push/back 保留父回调，replace/reset/关闭释放离栈回调；显式导航按 frame key 重置嵌套表单值/错误，普通根数据刷新保持草稿协调。views 1.3 onClose 对已接受 frame 至多一次报告 closed/back/replaced/reset/refreshed/unmounted；遮盖、StrictMode 重放与 live 更新不触发，live 不能替换通知。通知不阻止关闭、不消费返回导航，异步通知句柄最长十秒保留，退休不保证交付；外层直接移除只报 unmounted。新回调已过序列化/栈/嵌套/live/React 定向测试，真实 Worker/桌面组合留集中验收。旧 Annotation Desk 实机复验冲突留草稿、显式刷新加载新值并清错误，不覆盖新回调，也不等于全部视觉与焦点验收。迟到 UI 淘汰不取消已发起业务副作用，GAP06/10 未整体关闭。Views1.9 新增 close:all，关闭动作所属根呈现及嵌套插件对话框，true仍只关当前层；不能关另一插件/原生图片，退休子级与旧根request不能关闭替代界面。序列化嵌套/迟到回调定向测试与 Text Desk0.11.1 实际 Worker/Tauri 图片交接通过，不外推全部焦点/布局验收。 | [API](../packages/plugin-types/src/index.ts) [VIEWS](../apps/web/src/features/plugins/lib/plugin-view.ts) [RENDER](../apps/web/src/features/plugins/components/PluginViewRenderer.tsx) [WIRE](../apps/web/src/features/plugins/runtime/plugin-worker-host.ts) [VIEWSESSION](../apps/web/src/features/plugins/lib/plugin-view-session.ts) [VIEWSOURCE](../apps/web/src/features/plugins/hooks/usePluginViewSource.ts) [VIEWPROBE](../apps/web/tests/desktop/desktop-view-probe.ts) [DESKPROBE](../apps/web/tests/desktop/desktop-annotation-desk.ts) [IMAGECONVERSATIONPROOF](../docs/evidence/image-conversation-composition-2026-09-11.json) | I06, I08, J10 |
...
| <a id="EXT05"></a>EXT05 | 表单输入、动态选项、验证与提交 | 实装 | **接通（待 E2E）**：ask_user 固定 question + ask_user_form 1–8 字段（text/textarea/number/select/checkbox）<br>[设计] 结构化交互 schema | **接通（待 E2E）**：PluginFormView.onSubmit + submitMode=explicit/change；全部字段种类 fieldErrors<br>[设计] 声明式表单 | RSS 订阅/OPML粘贴；Dictionary；Annotation Desk 选中/确认/编辑；Agent 结构化表单 | text/textarea/time/number/secret/select/choice/checkbox/toggle 的 fieldErrors 均接到宿主控件 error 属性（选择/布尔控件有可访问错误描述），Dialog 项不能返回字段错误。Agent ask_user_form 收集非敏感结构化答案，答案是数据不是批准票据，不授权破坏性动作或剧透。OPML 是文本粘贴而非通用选择文件服务（SYS11）。真实 Worker/Tauri 表单与焦点属第三段。 | [API](../packages/plugin-types/src/index.ts) [RENDER](../apps/web/src/features/plugins/components/PluginViewRenderer.tsx) [PLUGINFORM](../apps/web/src/features/plugins/components/PluginFormViewBody.tsx) [DICTVIEWS](../plugins/dictionary/src/views.ts) [RSSVIEWS](../plugins/rss-reader/src/views.ts) [DESKBATCH](../plugins/annotation-desk/src/batch.ts) [INTERACTIONFORM](../packages/agent/src/tools/interaction-form-tool.ts) [INTERACTIONFORMPROOF](../packages/agent/src/tools/interaction-tools.test.ts) | J02 |
| <a id="EXT06"></a>EXT06 | 大列表分页/虚拟化、Tree/Table/编辑器/图像资源 | 部分 | **未接**：无正式入口<br>[设计] 有界工具结果/资源引用 | **部分**：views 1.7 资源位图；1.6 树形视图；list/table/tree 分页与数据源排序；list 虚拟行<br>[设计] 编辑器 schema 仍缺 | 插件声明数据，宿主绘制表格/树形/资源图片与键盘页控件 | 分页支持未知总页数和空页返回；翻页/排序默认替换整帧、复用错误与退休。表格1..16列/200行，文本/有限数/null；排序归插件完整数据源。列表筛选仅当前页。树500节点/12层，全树唯一 ID；展开不加载，宿主拥有方向键/首尾/typeahead，live 保留展开并处理焦点移除，不是虚拟树/跨帧焦点协议。图片根/块/live 仅消费本激活已封存资源，沿用取得授权，拒绝原书/外部URL；Rust 嗅探 PNG/JPEG/GIF/BMP/WebP，输入16 MiB/8192边/16MP，静态PNG预览不放大、最长边2048；SVG/HTML/AVIF不支持。私有URL共享16份/64 MiB PNG负载，末个组件卸载/退休释放，晚结果不分配URL；原资源释放不擦除已生成副本，不是总内存限额或Agent视觉。边界、序列化身份、挂载React与原生字节解码测试通过，真实 Worker/Tauri、打包CSP、窄窗/读屏器留集中验收；编辑器未接，不开放 React/HTML/DOM。 | [API](../packages/plugin-types/src/index.ts) [RENDER](../apps/web/src/features/plugins/components/PluginViewRenderer.tsx) [TABLEVIEW](../apps/web/src/features/plugins/components/PluginTableViewBody.tsx) [VIEWPAGING](../apps/web/src/features/plugins/components/PluginViewPagination.tsx) [DATAVIEWPROOF](../apps/web/src/features/plugins/components/plugin-data-views.test.tsx) [TREEVIEW](../apps/web/src/features/plugins/components/PluginTreeViewBody.tsx) [TREEVIEWPROOF](../apps/web/src/features/plugins/components/plugin-tree-view.test.tsx) [IMAGEVIEW](../apps/web/src/features/plugins/components/PluginImageViewBody.tsx) [IMAGEVIEWOWNER](../apps/web/src/features/plugins/lib/plugin-image-owner.ts) [IMAGEVIEWPROOF](../apps/web/src/features/plugins/components/plugin-image-view.test.tsx) [RESOURCEIMAGES](../apps/desktop/src-tauri/src/resource_images.rs) | J03, J06, J07 |
| <a id="EXT07"></a>EXT07 | Toast、持久错误、进度/取消/确认交互 | 部分 | **部分**：工具 error/interaction + streaming<br>[设计] 统一失败/任务呈现 | **部分**：views 1.8 progress 等待/取消；类型化 toast/error(code)<br>[设计] 统一 progress/approval | 插件 toast；Agent 工具输出；Reading Goals 活跃视图读取失败 | 内联 error 与结构化 toast 均由宿主本地化稳定 code，未知码通用失败，不透传 message。toast 只有宿主判定 retryable 且提供回调时才显示重试，用户单次触发，无自动重试或额外权限。错误提示六秒/全局16条，关闭与退休释放回调；点击后最多十秒保留执行句柄，不取消已派发工作。定向序列化/React 测试通过，真实 Worker/Tauri 留集中验收。agentTools 1.2 已接声明式工具宿主确认；views 1.8在已有确定进度块上增加value:null未知进度与cancel动作；max有限且>0、数值0..max，未知时不显示百分比/aria-valuenow，减少动画偏好生效。取消独立于主操作busy，单按钮在途去重，仍经当前帧/回调所有权执行；新取消意图淘汰旧动作的迟到返回，不自动宣称工作已停止或回滚。Jumper0.3/Text Desk0.9已组合未知搜索进度/取消/live/关闭清理，失败稳定code不冒充无命中；仅已知可恢复故障提供重试，取消后明确重搜，不新增宿主接口。Memory Desk 0.7以现有图谱任务状态呈现queued/running/cancelling并调用cancelGraphTask，不把阶段上限伪装进度。定向序列化/挂载/插件测试通过，真实Worker/Tauri留集中验收；通用UI批准票据和TaskRef调度/截止/重试/持久协议仍缺，保留部分。 | [API](../packages/plugin-types/src/index.ts) [RENDER](../apps/web/src/features/plugins/components/PluginViewRenderer.tsx) [ERRORS](../packages/core/src/errors.ts) [INTERACTION](../packages/agent/src/tools/interaction-tools.ts) [TIMEVIEW](../plugins/reading-goals/src/time-view.ts) [TIMEPROOF](../docs/evidence/reading-time-snapshot-2026-09-09.json) [PROGRESSVIEW](../apps/web/src/features/plugins/components/PluginProgressBlock.tsx) [PROGRESSPROOF](../apps/web/src/features/plugins/components/plugin-progress-block.test.tsx) [MEMORYTASKVIEW](../plugins/memory-desk/src/tasks.ts) [JUMPERSEARCH](../plugins/jumper/src/text-search.ts) [TEXTDESKSEARCH](../plugins/text-desk/src/search-task.ts) [JUMPERSEARCHPROOF](../plugins/jumper/tests/jumper.test.ts) [TEXTDESKSEARCHPROOF](../plugins/text-desk/src/search-views.test.ts) | J09 |
| <a id="EXT08"></a>EXT08 | 应用/阅读主题和字体贡献 | 实装 | **接通**：get_setting_options/update_settings 选择 appearance.theme / reading.fontFamily 等含插件声明项<br>[设计] 选择工具，不注册代码资产 | **接通**：manifest themes/fonts<br>[设计] 静态主题/字体贡献 | editorial-themes；Agent/插件设置选择 | 能力 catalog 含 themes/fonts，ctx 无 register 是声明式设计而非漏实现；选项目录随插件安装/停用刷新（CFG10） | [THEMES](../plugins/editorial-themes/manifest.json) [API](../packages/plugin-types/src/index.ts) [SETTINGS](../apps/web/src/domain/settings/catalog.ts) | N01 |
| <a id="EXT09"></a>EXT09 | 词典查询/收藏/复习列表/CSV 导出 | 实装 | **扩展**：lookup_word/get_vocabulary/save_word；delete_saved_word/export_vocabulary[全局]<br>[设计] 插件工具 | **接通**：Dictionary 私有 storage + LLM + views/export<br>[设计] 现有原语组合 | Dictionary | Dictionary 1.4 已补经宿主确认按ID删词及系统保存对话框CSV导出，不返回CSV正文到模型；读列表暴露准确ID。不是文档版本CAS；基础检查通过，真实保存/批准/删除留集中验收 | [DICT](../plugins/dictionary/src/index.ts) [DICTTOOLS](../plugins/dictionary/src/agent-tools.ts) [DICTVIEWS](../plugins/dictionary/src/views.ts) [DICTEXPORT](../plugins/dictionary/src/export.ts) | 新增盘点 |
| <a id="EXT10"></a>EXT10 | RSS 订阅/刷新/退订/OPML/阅读文章 | 实装 | **扩展**：list_feeds/subscribe_feed/refresh_feed/unsubscribe_feed/import_opml[全局]<br>[设计] 插件工具 | **接通**：RSS 私有 collection + content provider<br>[设计] 现有原语组合 | RSS 0.12 | 私有正文缓存、稳定文章身份、更新通知及显式源重载已接；退订经宿主确认URL/bookId并在同URL队列内重验绑定；OPML本地UTF-8文件预填与经确认的Agent文本分批导入已接，默认10/最大20、并发4，区分新增/已有/失败；总文档1MiB/1000源，Agent文本8000字符另受批准预算。既有订阅不刷新，失败不保证回滚，不自动跑下一批；位置迁移边界仍见 LIB14；OPML/Agent 集中原生验收未完成 2026-09-11 真实 RSS0.11 Worker/Tauri 已验订阅/刷新/稳定文章导航、503保留数据、源关闭且Worker重启后缓存阅读、退订清理；自有SQL计数0，900/600宽前台画面已验。无宿主API改动，不覆盖应用重启/远程媒体/OPML或Agent模型。 RSS0.12已组合自动刷新实时页、暂停/恢复/立即运行，沿用Schedules1.1/UI1.2和既有全局Agent工具；缺绑定移除操作，离开退订，重复运行不假报完成。修正计划回调吞掉部分/全部刷新失败：四并发批次完成后抛首个捕获错误，宿主不推进lastSuccessAt；普通Refresh all仍汇总部分成功，旧记录不改。插件27项基础测试、编译入口/类型/构建通过，新调度原生Worker/Tauri/Agent及重启组合留集中验收。 | [RSS](../plugins/rss-reader/src/index.ts) [RSSTOOLS](../plugins/rss-reader/src/agent-tools.ts) [RSSVIEWS](../plugins/rss-reader/src/views.ts) [RSSFEED](../plugins/rss-reader/src/feed.ts) [RSSLIB](../plugins/rss-reader/src/feed-library.ts) [RSSCACHE](../plugins/rss-reader/src/content-cache.ts) [RSSCACHEPROOF](../plugins/rss-reader/tests/feed-library.test.ts) [RSSOPML](../plugins/rss-reader/src/opml-import.ts) [RSSOPMLFILE](../plugins/rss-reader/src/opml-file.ts) [RSSOPMLPROOF](../plugins/rss-reader/tests/opml-file.test.ts) [RSSCOMPOSITIONPROOF](../docs/evidence/rss-composition-2026-09-11.json) [RSSREFRESH](../plugins/rss-reader/src/refresh.ts) [RSSSCHEDULEVIEW](../plugins/rss-reader/src/schedule-view.ts) [RSSSCHEDULEPROOF](../plugins/rss-reader/tests/schedule-view.test.ts) | 新增盘点 |
| <a id="EXT11"></a>EXT11 | 本地 marketplace 插件清单与启用状态 | 实装 | **接通**：list_installed_plugins / list_plugin_contributions[双域]<br>[设计] 安装目录与注册条目发现 | **接通**：services.plugins1.1 list/observe/contributions/observeContributions<br>[设计] 自有与受控公共目录 | 插件管理页；双端公共目录；Maintenance Desk 0.2 | 安装元数据不变；另查15种已注册扩展point/pluginId/key，注册态与enabled配置、健康、上下文可用性分开。条目发现不返回提供者数据、回调、设置、路径、凭据或原始错误；不执行插件。条目页默认50/最多100及16000字符JSON条目预算，变化需重查；最多64串行合并观察随退休释放。通用跨插件调用另列MORE06。Maintenance Desk 0.2已组合200字符搜索、40项分页、实时观察、选中元数据与贡献身份详情；刷新回首屏，非版本快照，不调用贡献项。20项插件测试及构建/类型通过；真实Worker/Tauri新路径集中后置。 | [HOST](../apps/web/src/features/plugins/runtime/plugin-host.ts) [MARKET](../apps/web/src/features/plugins/runtime/marketplace.ts) [API](../packages/plugin-types/src/index.ts) [PLUGINDIRECTORY](../apps/web/src/services/plugin-directory.ts) [HOSTIOTOOLS](../packages/agent/src/tools/host-io-tools.ts) [CONTRIBUTIONDISCOVERY](../apps/web/src/services/plugin-contributions.ts) [CONTRIBUTIONDISCOVERYPROOF](../apps/web/src/services/plugin-contributions.test.ts) [MAINTENANCEADMIN](../plugins/maintenance-desk/src/plugin-directory.ts) [MAINTENANCEADMINPROOF](../plugins/maintenance-desk/tests/admin.test.ts) | 新增盘点 |
| <a id="EXT12"></a>EXT12 | 安装/授权/启停/更新/回滚/卸载插件 | 实装 | **接通**：open_maintenance_settings(plugins)[双域]<br>[设计] 打开宿主管理入口 | **接通**：maintenance 1.1 openSettings(plugins)<br>[设计] 打开宿主管理入口 | Plugins settings，真实挂载标题定位；Maintenance Desk 0.2交接 | 只打开并定位宿主插件管理页，不预选目标/点击/安装/授权/启停/卸载；不新增宿主尚无的回滚实现。用户继续使用既有宿主控件及安装授权确认，不开放静默控制自己或其他插件。Maintenance Desk 0.2等待opened后关闭自身弹窗，不把opened当操作成功；最终任务回执和实际 Tauri 管理组合待集中验收。 | [HOST](../apps/web/src/features/plugins/runtime/plugin-host.ts) [MARKET](../apps/web/src/features/plugins/runtime/marketplace.ts) [RUST](../apps/desktop/src-tauri/src/lib.rs) [HOSTMAINTENANCE](../apps/web/src/services/maintenance.ts) [MAINTENANCETOOLS](../packages/agent/src/tools/maintenance-tools.ts) [MAINTENANCEADMIN](../plugins/maintenance-desk/src/plugin-directory.ts) [MAINTENANCEADMINPROOF](../plugins/maintenance-desk/tests/admin.test.ts) | O05, R05 |

### 存储、网络与原生资源

| ID | 宿主能力 | 宿主现状 | Agent 当前与目标 | 插件当前与目标 | 实际消费者 | 缺口/边界 | 来源 | 旧基线 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| <a id="SYS01"></a>SYS01 | 插件隔离 KV 同步读与异步持久写 | 实装 | **扩展**：通过插件工具间接使用，无 KV 工具<br>[设计] 不得读任意私有 KV | **接通（待 E2E）**：storage2.4 getDurable及get/set/remove/flush/onChange<br>[设计] 带 durable ack 的隔离 KV | 设置/状态插件；Reading Goals迁移和bundle来源 | 顺序持久写、flush、镜像失败重基与remote origin已接。getDurable等待已接受namespace写，固定前缀单键SQLite查询，1024 UTF-16键/禁NUL，路径形状仍为字面KV后缀；坏JSON/读失败不当空，退休丢弃迟到读并排空底层工作。Worker走RPC不读镜像，migration storage同步开放；Reading Goals legacy迁移改读真实持久值。定向原生/宿主/Bun Worker通过，实际Tauri持久化与生命周期仍第三段，不据此关闭全部GAP02/03 | [CTX](../apps/web/src/features/plugins/runtime/plugin-context.ts) [WORKER](../apps/web/src/features/plugins/runtime/plugin-sandbox.worker.ts) [KV](../apps/web/src/platform/local-store.ts) [DICT](../plugins/dictionary/src/index.ts) [DURABLEKV](../apps/web/src/features/plugins/runtime/plugin-durable-kv.ts) [GOALCONTEXTSOURCE](../plugins/reading-goals/src/context-source.ts) | O01, O02 |
...
| <a id="SYS07"></a>SYS07 | 网络域名授权、预算、下载流和离线重试 | 部分 | **部分**：download_resource 全局批准/有界下载；无累计预算或重试<br>[设计] 用途受限任务 | **部分**：network 2.1 policy/openStream/readStream/closeStream；累计额度/重试仍缺<br>[设计] 授权/任务/缓存原语 | 安装/更新授权；请求所有权；Worker分块探针 | 最多32个精确HTTP(S) origin或单独*，缺省拒绝；范围安装/更新展示且旧宿主不能忽略。响应句柄仅本激活，先头后按offset拉取，默认64 KiB/最大1 MiB，单流单个在途读，不可重放；总下载1 GiB，上传和fetch正文64 MiB。8/32并发与120秒绝对寿命共用，EOF/close/到期/停用释放，晚原生结果先收尾再退槽；open的signal只取消开头，取得句柄后显式close。已交付/保存字节不回滚，丢失读回执不能盲重试；不承诺严格总内存预算，原生分块/IPC有额外缓冲且单块最多64 MiB。第一方自定义地址仍显式全网包含本地服务，不是IP/DNS隔离。Agent已接每次批准HTTPS下载到会话资源：64 MiB/120秒、1/4并发、重定向重新批准，不自动导入/保存；取消清理可能超过截止。累计速率/字节额度与重试协调仍缺；socket/耐久离线队列非当前宿主能力。实际大下载/慢网/桌面组合留集中验收。 | [API](../packages/plugin-types/src/index.ts) [HTTP](../apps/web/src/platform/http-client.ts) [WIRE](../apps/web/src/features/plugins/runtime/plugin-worker-host.ts) [CATALOG](../packages/core/src/capabilities.ts) [NETWORKPOLICY](../apps/web/src/features/plugins/lib/plugin-network-policy.ts) [NETWORKSERVICE](../apps/web/src/features/plugins/runtime/plugin-network.ts) [NETWORKPROOF](../apps/web/src/features/plugins/runtime/plugin-network.test.ts) [NETWORKREQUESTS](../apps/web/src/features/plugins/runtime/plugin-network-requests.ts) [NETWORKSTREAMPROOF](../apps/web/src/features/plugins/runtime/plugin-network-requests.test.ts) [DOWNLOAD](../apps/web/src/services/resource-download.ts) [DOWNLOADTOOLS](../packages/agent/src/tools/download-tools.ts) [DOWNLOADPROOF](../apps/web/src/services/resource-download.test.ts) | P04, P06 |
...
| ID | 宿主能力 | 宿主现状 | Agent 当前与目标 | 插件当前与目标 | 实际消费者 | 缺口/边界 | 来源 | 旧基线 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| <a id="OPS01"></a>OPS01 | 同步连接/断开/立即同步/状态与积压 | 实装 | **接通**：get_sync_status；manage_sync now/settings/connect/disconnect 双 scope<br>[设计] 状态、后端发现、批准同步与宿主连接流程 | **接通**：services.sync 1.1 connectionOptions/requestFlow及既有同步接口<br>[设计] 独立 service:sync 控制面 | Data & Sync；Agent 工具；Maintenance Desk 0.3 | 共享 scheduler，初始/变化串行观察最多64；fresh outbox与cycleStartBacklog分开，不泄漏书籍/blobKey/游标。同步未连接/凭据失效/管理中拒绝；already-running不是完成。connectionOptions仅注册后端ref/名称；connect省略ref为relay，登录仍确认身份和宿主口令，断开仍宿主确认。requestFlow等本地完成或取消；一次一个流程，不替换已有原生对话框，换账号拒绝旧确认。单次signal/退休/Worker超时关闭未开始流程，确认后不强杀原生操作，实际源结束才释放。无持久TaskRef/重启恢复，不回滚共享同步。Maintenance Desk 0.3已组合实时状态、独立积压、显式同步与后端选择；20条激活期日志区分completed/already-running，取消不停止共享同步。30项插件测试及构建/类型通过；真实Worker/Tauri跨设备集中后置。 | [SYNC](../apps/web/src/platform/sync/sync-scheduler.ts) [SYNCCONNECT](../apps/web/src/platform/sync/connect.ts) [ACCOUNTUI](../apps/web/src/features/settings/sections/SyncAccountGroup.tsx) [API](../packages/plugin-types/src/index.ts) [SYNCSERVICE](../apps/web/src/services/sync.ts) [SYNCCONTROLLER](../apps/web/src/services/sync-controller.ts) [SYNCFLOWS](../apps/web/src/services/sync-flow-controller.ts) [SYNCFLOWUI](../apps/web/src/features/settings/hooks/useSyncAccountFlows.ts) [SYNCTOOLS](../packages/agent/src/tools/sync-tools.ts) [MAINTENANCESYNC](../plugins/maintenance-desk/src/sync.ts) [MAINTENANCESYNCPROOF](../plugins/maintenance-desk/tests/sync.test.ts) | R04 |
...
| <a id="OPS06"></a>OPS06 | 账号登录、连接 token、退出、删除账号 | 实装 | **接通**：get_sync_status；manage_sync connect/disconnect/delete-account<br>[设计] 宿主身份及危险确认，最终本地回执 | **接通**：sync 1.1 snapshot/requestFlow<br>[设计] 独立授权的宿主账户流程 | SyncAccountGroup；双端流程请求；Maintenance Desk 0.3 | 不返回email/账号ID/token/主密钥。请求直接进入现有宿主登录/退出/删除确认，不能自批准或填入凭据；connect失败可在原对话框重试，其他操作失败拒绝并由宿主本地化反馈。completed必须等真实宿主动作，cancelled不写；删除远端和本地断开持有共同连接锁，保留本地书籍/批注。换账号使旧确认失效；确认后取消不回滚。Maintenance Desk 0.3已组合流程review、关闭浮层后等待、近期回执/取消；连接固定后端ref，删除告知远端影响及本地保留，不自批准。基础与编译入口通过，真实Worker/Tauri登录/删除集中后置。 | [ACCOUNTUI](../apps/web/src/features/settings/sections/SyncAccountGroup.tsx) [SYNCCONNECT](../apps/web/src/platform/sync/connect.ts) [RUST](../apps/desktop/src-tauri/src/lib.rs) [EXTERNAL](../apps/web/src/platform/external-link.ts) [SYNCSERVICE](../apps/web/src/services/sync.ts) [SYNCFLOWS](../apps/web/src/services/sync-flow-controller.ts) [SYNCFLOWUI](../apps/web/src/features/settings/hooks/useSyncAccountFlows.ts) [SYNCTOOLS](../packages/agent/src/tools/sync-tools.ts) [MAINTENANCESYNC](../plugins/maintenance-desk/src/sync.ts) [MAINTENANCESYNCPROOF](../plugins/maintenance-desk/tests/sync.test.ts) | 新增盘点 |
| <a id="OPS07"></a>OPS07 | 套餐/用量/购买/账单管理 | 实装 | **接通**：get_sync_status includeAccount；manage_sync upgrade/billing<br>[设计] 套餐用量与外部流程交接回执 | **接通**：sync 1.1 account/requestFlow<br>[设计] 按需脱敏读及宿主购买入口 | 购买/账单portal；双端配额与流程；Maintenance Desk 0.3 | account仅返回tier/hasBilling/三项用量及四项额度，null是非relay/未连接而非零用量；失败和换代拒绝。upgrade/billing直接复用宿主外链操作，保留平台购买限制与账号资格，打开前复查取消/连接代；不返回URL/ticket/keys/email/accountId。external-opened只证明外部交接，不是支付或订阅变更完成；购买仍由用户在外部页面完成，不开放自动付款。Maintenance Desk 0.3已组合按需联网用量查询、null/0额度、资格账单入口、原生交接与真实状态日志；不轮询账户或把外链当付款完成。基础与编译入口通过，真实远端套餐、系统浏览器和购买结果留集中Tauri验收。 | [ACCOUNTUI](../apps/web/src/features/settings/sections/SyncAccountGroup.tsx) [EXTERNAL](../apps/web/src/platform/external-link.ts) [SYNCSERVICE](../apps/web/src/services/sync.ts) [SYNCCONTROLLER](../apps/web/src/services/sync-controller.ts) [SYNCFLOWS](../apps/web/src/services/sync-flow-controller.ts) [SYNCFLOWUI](../apps/web/src/features/settings/hooks/useSyncAccountFlows.ts) [SYNCTOOLS](../packages/agent/src/tools/sync-tools.ts) [MAINTENANCESYNC](../plugins/maintenance-desk/src/sync.ts) [MAINTENANCESYNCPROOF](../plugins/maintenance-desk/tests/sync.test.ts) | 新增盘点 |
| <a id="OPS08"></a>OPS08 | 备份导出与合并导入 | 部分 | **部分**：request_backup / open_maintenance_settings[双域]<br>[设计] 原生备份请求及最终回执已接 | **部分**：maintenance 1.2 requestBackup / openSettings<br>[设计] 原生备份请求及最终回执已接 | DataSyncPanel 原有按钮、原生文件服务与 HostActionFlow | requestBackup 只定位，用户点击及选择文件后返回 imported/exported/cancelled；openSettings 的 opened 仍非完成。不交出字节/路径/计数，无新增授权或自我批准。单次 signal 已接，未开始可取消，已开始合并不回滚；无耐久回执，imported 不证明重载/genesis完成。v1仅KV/books/collections/annotations/本地files，独立聊天/记忆/插件文档/密钥库/event-log未枚举；KV可能含个人数据，整份JSON驻内存、验证不完整、可覆盖及部分写入，故仍部分。八语言已纠正全量备份保证。基础调用/受控文件/挂载通过；Maintenance Desk真实Worker/Tauri已完成另存为临时文件和exported回执，实际Open选择取消返回cancelled。八语言成功文案不再误称默认文件名或全量。合并恢复/跨重启与完整Agent组合仍待验。 | [BACKUP](../apps/web/src/features/settings/lib/backup-io.ts) [BACKUPFLOW](../apps/web/src/features/settings/lib/backup-file-actions.ts) [BACKUPFLOWUI](../apps/web/src/features/settings/hooks/useBackupActions.ts) [BACKUPFLOWPROOF](../apps/web/src/features/settings/hooks/useBackupActions.test.tsx) [DATAUI](../apps/web/src/features/settings/sections/DataSyncPanel.tsx) [RUST](../apps/desktop/src-tauri/src/lib.rs) [HOSTMAINTENANCE](../apps/web/src/services/maintenance.ts) [MAINTENANCETOOLS](../packages/agent/src/tools/maintenance-tools.ts) [MAINTENANCEDESKNATIVE](../docs/evidence/maintenance-desk-2026-09-11.json) | O06, R05 |
...
| <a id="CON06"></a>CON06 | 长任务进度、取消、超时、并发与幂等 | 部分 | **部分**：局部 thread abort/工具 sequential<br>[设计] 统一任务原语 | **部分**：PluginCallOptions覆盖10正文查询/16阅读控制；仍无通用TaskRef<br>[设计] 统一任务服务 | 搜索/导航/LLM等同RPC取消；其他任务仍分域 | Library1.17/Reading2.18共用末参数options.signal和26项位置表；本地预取消不派发，宿主请求控制器合并realm，查询取消先结束等待、源清理继续。lifecycle.read限每realm32个未结算源操作，满则plugin/busy，不因取消早退槽；不是全App配额或全宿主TaskRef/进度/幂等/重试协议。资源获取、持久写及未列入方法没有自动新增此options，不能宣称所有宿主操作都可单次取消。 | [THREAD](../packages/agent/src/runtime/thread.ts) [WIRE](../apps/web/src/features/plugins/runtime/plugin-worker-host.ts) [HOST](../apps/web/src/features/plugins/runtime/plugin-host.ts) [API](../packages/plugin-types/src/index.ts) [CALLOPTIONS](../apps/web/src/features/plugins/runtime/plugin-call-options.ts) [CALLOPTIONSPROOF](../apps/web/src/features/plugins/runtime/plugin-call-options.test.ts) | K06, L07, N05, Q04, Q06 |
| <a id="CON07"></a>CON07 | 领域事件的本地/远端/外部变化一致性 | 部分 | **自动**：端口每轮读；runtime 配置失效<br>[设计] 有版本快照/读后写 | **部分**：domain subscribe；library/conversations observeInvalidation<br>[设计] 业务事件与失效通知分开 | 本地domain broadcasts；授权投影失效 | library1.18/conversations1.3读授权初始及变化通知已接，来源initial/local/host/remote/restore/mixed，无业务payload/actor；订阅序号非CAS或数据库版本。串行合并与每领域64观察，失败callback日志隔离、后续仍可交付，退休不取消已经开始的callback。sync投影提交即发而非等待整轮成功，备份书籍/集合行恢复同样通知；保留旧subscribe本地语义，不能当远端可重放事件流或自动重做写操作。基础授权/IPC/生命周期检查通过，外部原始DB修改、所有历史恢复入口和GAP09/11其余项、真实组合Tauri仍缺。 | [EVENTROSTER](../apps/web/src/domain/events.ts) [APPEVENTS](../apps/web/src/platform/app-events.ts) [CTX](../apps/web/src/features/plugins/runtime/plugin-context.ts) [SYNC](../apps/web/src/platform/sync/sync-scheduler.ts) [PROJECTIONINVALIDATION](../apps/web/src/domain/projection-invalidation.ts) [PROJECTIONINVALIDATIONPROOF](../apps/web/src/domain/projection-invalidation.test.ts) [SYNCSTORE](../apps/web/src/platform/sync/sync-store.ts) | A06, Q01, Q02 |
...
| <a id="MORE03"></a>MORE03 | 环境 locale/platform/timezone/在线/ready 快照 | 部分 | **部分**：get_host_environment（全局/书内）+ 自动语言/日期上下文<br>[设计] 环境查询与真实 availability | **部分**：session 2.0 environment/observeEnvironment + ctx.appVersion/capabilities<br>[设计] 统一环境与 availability 快照 | Agent 查询；零权限 Worker 观察；Listening Desk 0.10实时网络提示 | 共享 revision、runtime/platform/locale/timeZone/utcOffsetMinutes/networkHint；首次立即快照，语言/网络/焦点变化刷新，时区每 30 秒复核且每次查询刷新，末个观察者释放监听与 timer。网络仅 OS/WebView 提示，不证明 endpoint 可达、账号/模型就绪或格式可用；这些 availability 仍缺。无阅读/账号字段，不借内置服务绕过 reading 权限。Listening Desk 0.10独立实时页观察网络提示，不阻止本地朗读；原根视图仍按需刷新。基础/编译入口通过，新实时页真实Worker/Tauri后置。隔离 macOS debug 双端与真实 Worker 已验既有环境接口；旧 session 四阅读事件旁路已移除；packaged/跨平台/真实系统时区和网络切换未验。 | [CTX](../apps/web/src/features/plugins/runtime/plugin-context.ts) [ENVIRONMENT](../apps/web/src/platform/host-environment.ts) [ENVTOOLS](../packages/agent/src/tools/environment-tools.ts) [ENVPROOF](../docs/evidence/host-environment-2026-09-09.json) [SESSIONBOUNDARY](../docs/evidence/reading-session-boundary-2026-09-09.json) [API](../packages/plugin-types/src/index.ts) [LISTENINGDESK](../plugins/listening-desk/src/views.ts) [LISTENINGMONITOR](../plugins/listening-desk/src/monitor.ts) [LISTENINGMONITORPROOF](../plugins/listening-desk/tests/monitor.test.ts) | A07 |
| <a id="MORE04"></a>MORE04 | 书籍/集合上下文菜单与 Agent header 插槽 | 实装 | **未接**：无正式入口<br>[设计] 语义命令，不操作菜单 DOM | **接通**：contextActions 1.0 book/collection；headerActions 1.2 agent<br>[设计] 语义对象动作及页头视图 | 书籍网格/列表、集合卡片/详情操作菜单；Agent header 与窄窗 overflow | 接通待集中验收。书籍 id/title/author、集合 id/name 和当前全局线程 id 是点击上下文，不授予领域读取/写入权限，不披露原始路径、封面 URL、消息或成员清单。context action 返回既有 view/toast/void，复用注册级动态状态、退休和 Dialog 生命周期；Agent header 强制 popup，换线程重建视图，不加入书架导航/命令面板。集合/书籍列表为显式操作菜单，不声称系统级原生右键菜单或批量选择贡献；逐目标条件、菜单自定义布局不是本批范围。 Memory Desk 0.9 已组合线程创建/选择、运行态观察、宿主确认请求、自有请求状态/取消、停止和确认清空，并消费Agent页头线程目标；41项插件测试、编译回调与受控宿主请求状态机通过。原生批准点击、升级授权、持久化与真实模型留集中桌面验收。 | [SHELFUI](../apps/web/src/features/shelf/components/Shelf.tsx) [AGENTUI](../apps/web/src/features/agent/components/AgentWorkspace.tsx) [MENU](../apps/web/src/features/menus/lib/menu-registry.tsx) [API](../packages/plugin-types/src/index.ts) [CTX](../apps/web/src/features/plugins/runtime/plugin-context.ts) [CONTEXTACTIONS](../apps/web/src/features/plugins/lib/context-action-items.tsx) [AGENTHEADER](../apps/web/src/features/plugins/hooks/usePluginAgentHeaderEntries.tsx) [CONTEXTACTIONPROOF](../apps/web/src/features/plugins/runtime/context-actions.test.ts) [MEMORYCONVERSATIONS](../plugins/memory-desk/src/conversation-controls.ts) [MEMORYREQUESTS](../plugins/memory-desk/src/turn-requests.ts) [MEMORYCONVERSATIONPROOF](../plugins/memory-desk/tests/conversation-controls.test.ts) | I05 |
...
| <a id="MORE06"></a>MORE06 | 发现/复用类型化提供者与跨插件权限交集 | 部分 | **部分**：list_plugin_contributions[双域]；host聚合tool/retrieval<br>[设计] 所有已注册扩展点身份发现 | **部分**：plugins1.1 contributions/observeContributions<br>[设计] 15种注册条目查询/观察，不增权 | 宿主消费voices/content/modes/tools/transports；双端注册身份目录；Maintenance Desk 0.2 | 共享registry自动覆盖selection/header/context动作、commands、settingsOptions、voice/content/readerModes、Agent tools/context/retrieval/memory candidates、themes/fonts，另合并syncTransports。仅point/pluginId/key，查询不调用插件或打开传输会话；注册非健康/可用/本轮可调用保证，key不是通用调用句柄或设置值。按扩展点/插件/搜索分页，nextOffset按实际页长，更新后重读；观察覆盖注册/替换/更新/注销，单次初始与串行合并回调，退休清理。现有宿主条目发现已接；settings1.10另通过精确设置路径委托动态选项读取，源仅接自己的已保存非秘密表单，非任意invoke；跨插件任意invoke与权限交集broker原本未建，仍缺，不把注册权当调用权或读取他人storage。Maintenance Desk 0.2已组合全目录或固定插件贡献查询/分页/观察，只读身份；基础与编译入口测试通过，真实Worker/Tauri集中后置。 | [EXTOOLS](../apps/web/src/features/plugins/runtime/plugin-tools.ts) [CTX](../apps/web/src/features/plugins/runtime/plugin-context.ts) [SYNCTRANSPORT](../apps/web/src/platform/sync/transport-registry.ts) [API](../packages/plugin-types/src/index.ts) [CONTRIBUTIONDISCOVERY](../apps/web/src/services/plugin-contributions.ts) [CONTRIBUTIONDISCOVERYPROOF](../apps/web/src/services/plugin-contributions.test.ts) [MAINTENANCEADMIN](../plugins/maintenance-desk/src/plugin-directory.ts) [MAINTENANCEADMINPROOF](../plugins/maintenance-desk/tests/admin.test.ts) [DYNAMICOPTIONS](../apps/web/src/domain/settings/dynamic-options.ts) [PLUGINOPTIONSOURCE](../apps/web/src/domain/settings/plugin-option-source.ts) | N04, N06 |
...
| 顺序 | 行号 | 必须先解决的原因 |
| --- | --- | --- |
| P0 | SET04/SET05/SET18–SET21、SET24–SET26 | 0 个只保存值的设置；SET24/25 已接 Agent 自动输入和插件结构化推理与取消，任意文本通路仍缺；SET26 localOnly 的宿主推理已接，插件网络边界仍缺。隐私/记忆开关需要真实效果，不是只补 API。 |
...
| Reading Goals | 版本化书内目标、上下文/记忆候选、宿主记忆开关 | get_reading_goal；set_reading_goal/clear_reading_goal逐次批准；global/book | 0.4新存储/升级/批准待集中验收；非release内置；候选公共回执仍缺 |
| Workspace Profiles | 十路径预设、旧七路径兼容、分页与条件应用/删除 | workspace_profiles只读；save_workspace_profile/manage_workspace_profile逐次批准；global/book | 保留本书覆盖；设置与私有文档非联合事务；0.6原生确认/条件写待集中验收 |
...
[代码] Reading Goals 同时注册 agentContextProviders 与 memoryCandidateProviders：每轮按请求书籍提供私有阅读目标，用户选择后提出书内偏好；实际宿主裁决、入库与 buildMemory 取消已在隔离 Tauri 验证。Dictionary 提供检索贡献，运行时会生成一个额外 retrieve 工具。具体带命名空间的 21 个插件 Agent 入口在库存表中列出。
...
- Plugin Agent contribution：21 个。
- Plugin setting declaration：24 个。
- Native bundled plugin：6 个。

以下“已映射”只保证注册项可追到矩阵行，不意味着目标已实现。Plugin ctx 是授予当前全部 manifest 权限后的 233 个顶层可调用路径；返回的 collection/session 方法单列。Settings 74 路径是在 custom + 主/快模型配置的完整条件快照中生成，不表示未配置 AI 时也显示全部路径。Native command 包含 cfg/no-op 历史项，见 SYS18，不能算桌面能力全部对插件开放。

### Agent global

| 当前注册项 | 矩阵行 | 说明 |
| --- | --- | --- |
| `explain_selection` | [SET18](#SET18) | [代码] 注册库存已映射，不表示产品 E2E 通过 |
| `define_term` | [SET19](#SET19) | [代码] 注册库存已映射，不表示产品 E2E 通过 |
| `translate_selection` | [SET20](#SET20) | [代码] 注册库存已映射，不表示产品 E2E 通过 |
| `summarize_chapter` | [SET21](#SET21) | [代码] 注册库存已映射，不表示产品 E2E 通过 |
...
| `list_book_images` | [TXT12](#TXT12) | [代码] 注册库存已映射，不表示产品 E2E 通过 |
| `open_book_image_resource` | [TXT12](#TXT12) | [代码] 注册库存已映射，不表示产品 E2E 通过 |
| `show_book_image` | [TXT12](#TXT12) | [代码] 注册库存已映射，不表示产品 E2E 通过 |
...
| `list_plugin_contributions` | [EXT11](#EXT11) [MORE06](#MORE06) | [代码] 注册库存已映射，不表示产品 E2E 通过 |
...
| `open_maintenance_settings` | [SYS15](#SYS15) [SYS16](#SYS16) [EXT12](#EXT12) [OPS08](#OPS08) [OPS09](#OPS09) [OPS10](#OPS10) [CFG08](#CFG08) | [代码] 注册库存已映射，不表示产品 E2E 通过 |
...
| `download_resource` | [SYS06](#SYS06) [SYS07](#SYS07) [SYS13](#SYS13) | [代码] 注册库存已映射，不表示产品 E2E 通过 |
...
| `get_book_content_state` | [LIB14](#LIB14) | [代码] 注册库存已映射，不表示产品 E2E 通过 |
...
| `inspect_user_profile` | [MEM08](#MEM08) | [代码] 注册库存已映射，不表示产品 E2E 通过 |
| `update_user_profile` | [MEM07](#MEM07) | [代码] 注册库存已映射，不表示产品 E2E 通过 |
| `manage_memory` | [MEM01](#MEM01) [MEM04](#MEM04) [MEM05](#MEM05) | [代码] 注册库存已映射，不表示产品 E2E 通过 |
| `query_entities` | [MEM08](#MEM08) | [代码] 注册库存已映射，不表示产品 E2E 通过 |
| `manage_entity` | [MEM08](#MEM08) | [代码] 注册库存已映射，不表示产品 E2E 通过 |
| `classify_book` | [MEM09](#MEM09) | [代码] 注册库存已映射，不表示产品 E2E 通过 |
| `capture_context_bundle` | [MEM13](#MEM13) | [代码] 注册库存已映射，不表示产品 E2E 通过 |
| `list_context_bundles` | [MEM13](#MEM13) | [代码] 注册库存已映射，不表示产品 E2E 通过 |
| `read_context_bundle` | [MEM13](#MEM13) | [代码] 注册库存已映射，不表示产品 E2E 通过 |
| `export_context_bundle` | [MEM13](#MEM13) | [代码] 注册库存已映射，不表示产品 E2E 通过 |
...
| `ask_user_form` | [EXT05](#EXT05) | [代码] 注册库存已映射，不表示产品 E2E 通过 |
...
| `get_model_catalog` | [CFG08](#CFG08) | [代码] 注册库存已映射，不表示产品 E2E 通过 |
| `refresh_model_catalog` | [CFG08](#CFG08) | [代码] 注册库存已映射，不表示产品 E2E 通过 |
...
| 当前注册项 | 矩阵行 | 说明 |
| --- | --- | --- |
| `domains.settings.queries.modelCatalog` | [CFG08](#CFG08) | [代码] 注册库存已映射，不表示产品 E2E 通过 |
...
| `domains.settings.commands.refreshModelCatalog` | [CFG08](#CFG08) | [代码] 注册库存已映射，不表示产品 E2E 通过 |
...
| `domains.library.queries.books.listNavigationTargets` | [READ05](#READ05) | [代码] 注册库存已映射，不表示产品 E2E 通过 |
...
| `domains.library.queries.books.getContentState` | [LIB14](#LIB14) | [代码] 注册库存已映射，不表示产品 E2E 通过 |
...
| `domains.library.queries.books.listImages` | [TXT12](#TXT12) | [代码] 注册库存已映射，不表示产品 E2E 通过 |
...
| `domains.library.queries.books.openImageResource` | [TXT12](#TXT12) | [代码] 注册库存已映射，不表示产品 E2E 通过 |
...
| `domains.library.events.observeInvalidation` | [OPS05](#OPS05) [CON07](#CON07) | [代码] 注册库存已映射，不表示产品 E2E 通过 |
...
| `domains.library.events.observeContentState` | [LIB14](#LIB14) | [代码] 注册库存已映射，不表示产品 E2E 通过 |
...
| `domains.library.commands.books.invalidateVirtualBook` | [LIB14](#LIB14) | [代码] 注册库存已映射，不表示产品 E2E 通过 |
...
| `domains.reading.commands.reload` | [LIB14](#LIB14) | [代码] 注册库存已映射，不表示产品 E2E 通过 |
...
| `domains.conversations.queries.getInsights` | [MEM12](#MEM12) | [代码] 注册库存已映射，不表示产品 E2E 通过 |
...
| `domains.conversations.events.observeInvalidation` | [OPS05](#OPS05) [CON07](#CON07) | [代码] 注册库存已映射，不表示产品 E2E 通过 |
...
| `domains.memory.queries.page` | [MEM01](#MEM01) | [代码] 注册库存已映射，不表示产品 E2E 通过 |
...
| `domains.memory.queries.profileContext` | [MEM08](#MEM08) | [代码] 注册库存已映射，不表示产品 E2E 通过 |
| `domains.memory.queries.entities` | [MEM08](#MEM08) | [代码] 注册库存已映射，不表示产品 E2E 通过 |
...
| `domains.memory.queries.context.history` | [MEM13](#MEM13) | [代码] 注册库存已映射，不表示产品 E2E 通过 |
| `domains.memory.queries.context.read` | [MEM13](#MEM13) | [代码] 注册库存已映射，不表示产品 E2E 通过 |
| `domains.memory.queries.context.export` | [MEM13](#MEM13) | [代码] 注册库存已映射，不表示产品 E2E 通过 |
| `domains.memory.events.observe` | [MEM01](#MEM01) [MEM04](#MEM04) [MEM05](#MEM05) [MEM06](#MEM06) [MEM08](#MEM08) [MEM09](#MEM09) [MEM10](#MEM10) [MEM11](#MEM11) | [代码] 注册库存已映射，不表示产品 E2E 通过 |
| `domains.memory.commands.mutate` | [MEM04](#MEM04) [MEM05](#MEM05) | [代码] 注册库存已映射，不表示产品 E2E 通过 |
| `domains.memory.commands.context.capture` | [MEM13](#MEM13) | [代码] 注册库存已映射，不表示产品 E2E 通过 |
| `domains.memory.commands.updateProfile` | [MEM07](#MEM07) | [代码] 注册库存已映射，不表示产品 E2E 通过 |
| `domains.memory.commands.decideEntity` | [MEM08](#MEM08) | [代码] 注册库存已映射，不表示产品 E2E 通过 |
...
| `services.storage.observeDocuments` | [SYS02](#SYS02) [CON07](#CON07) | [代码] 注册库存已映射，不表示产品 E2E 通过 |
| `services.storage.applyDocuments` | [SYS02](#SYS02) | [代码] 注册库存已映射，不表示产品 E2E 通过 |
...
| `services.ui.reader.image.open` | [TXT12](#TXT12) | [代码] 注册库存已映射，不表示产品 E2E 通过 |
...
| `services.ui.reader.focus` | [READ11](#READ11) | [代码] 注册库存已映射，不表示产品 E2E 通过 |
...
| `services.plugins.observeContributions` | [EXT11](#EXT11) [MORE06](#MORE06) | [代码] 注册库存已映射，不表示产品 E2E 通过 |
...
| `services.maintenance.openSettings` | [SYS15](#SYS15) [SYS16](#SYS16) [EXT12](#EXT12) [OPS08](#OPS08) [OPS09](#OPS09) [OPS10](#OPS10) [CFG08](#CFG08) | [代码] 注册库存已映射，不表示产品 E2E 通过 |
...
| `services.sync.connectionOptions` | [OPS01](#OPS01) | [代码] 注册库存已映射，不表示产品 E2E 通过 |
| `services.sync.requestFlow` | [OPS01](#OPS01) [OPS06](#OPS06) [OPS07](#OPS07) | [代码] 注册库存已映射，不表示产品 E2E 通过 |
...
| `services.network.openStream` | [SYS07](#SYS07) | [代码] 注册库存已映射，不表示产品 E2E 通过 |
| `services.network.readStream` | [SYS07](#SYS07) | [代码] 注册库存已映射，不表示产品 E2E 通过 |
| `services.network.closeStream` | [SYS07](#SYS07) | [代码] 注册库存已映射，不表示产品 E2E 通过 |
| `services.llm.ask` | [AI06](#AI06) | [代码] 注册库存已映射，不表示产品 E2E 通过 |
| `services.llm.askDetailed` | [AI07](#AI07) | [代码] 注册库存已映射，不表示产品 E2E 通过 |
| `services.llm.getRequest` | [AI07](#AI07) | [代码] 注册库存已映射，不表示产品 E2E 通过 |
| `services.llm.listRequests` | [AI07](#AI07) | [代码] 注册库存已映射，不表示产品 E2E 通过 |
| `services.llm.cancelRequest` | [AI07](#AI07) | [代码] 注册库存已映射，不表示产品 E2E 通过 |
| `services.llm.policy` | [AI07](#AI07) | [代码] 注册库存已映射，不表示产品 E2E 通过 |
...
| `contributions.agentTools.register` | [AI05](#AI05) [AI10](#AI10) | [代码] 注册库存已映射，不表示产品 E2E 通过 |
...
| `storage.collection().page` | [SYS02](#SYS02) | [代码] 注册库存已映射，不表示产品 E2E 通过 |
...
| `agentTools` | [AI05](#AI05) [AI10](#AI10) | [代码] 注册库存已映射，不表示产品 E2E 通过 |
...
| 当前注册项 | 矩阵行 | 说明 |
| --- | --- | --- |
| `views` | [EXT03](#EXT03) [EXT04](#EXT04) [EXT05](#EXT05) [EXT06](#EXT06) | [代码] 注册库存已映射，不表示产品 E2E 通过 |
...
| `storage::load_kv_all` | [SYS01](#SYS01) [CFG01](#CFG01) [SET04](#SET04) [SET05](#SET05) | [代码] 内部 IPC 能力证据；不是插件或模型授权入口 |
| `storage::get_kv` | [SYS01](#SYS01) [MEM13](#MEM13) | [代码] 内部 IPC 能力证据；不是插件或模型授权入口 |
| `storage::set_kv` | [SYS01](#SYS01) [CFG01](#CFG01) [SET04](#SET04) [SET05](#SET05) | [代码] 内部 IPC 能力证据；不是插件或模型授权入口 |
| `storage::set_kv_batch` | [SYS01](#SYS01) [CFG01](#CFG01) [SET04](#SET04) [SET05](#SET05) | [代码] 内部 IPC 能力证据；不是插件或模型授权入口 |
| `storage::delete_kv` | [SYS01](#SYS01) [CFG01](#CFG01) [SET04](#SET04) [SET05](#SET05) | [代码] 内部 IPC 能力证据；不是插件或模型授权入口 |
| `storage::replace_kv_prefix` | [SYS01](#SYS01) [CFG01](#CFG01) [SET04](#SET04) [SET05](#SET05) | [代码] 内部 IPC 能力证据；不是插件或模型授权入口 |
...
| `storage::profile_inspect` | [MEM06](#MEM06) [MEM07](#MEM07) [MEM08](#MEM08) | [代码] 内部 IPC 能力证据；不是插件或模型授权入口 |
| `storage::profile_context` | [MEM06](#MEM06) [MEM08](#MEM08) | [代码] 内部 IPC 能力证据；不是插件或模型授权入口 |
| `storage::context_bundle_source_revision` | [MEM13](#MEM13) | [代码] 内部 IPC 能力证据；不是插件或模型授权入口 |
| `storage::context_bundle_publish` | [MEM13](#MEM13) | [代码] 内部 IPC 能力证据；不是插件或模型授权入口 |
| `storage::conversation_insights_snapshot` | [MEM13](#MEM13) | [代码] 内部 IPC 能力证据；不是插件或模型授权入口 |
| `storage::book_context_snapshot` | [MEM13](#MEM13) | [代码] 内部 IPC 能力证据；不是插件或模型授权入口 |
| `storage::context_bundle_history` | [MEM13](#MEM13) | [代码] 内部 IPC 能力证据；不是插件或模型授权入口 |
| `storage::context_bundle_read` | [MEM13](#MEM13) | [代码] 内部 IPC 能力证据；不是插件或模型授权入口 |
| `storage::profile_initialize` | [MEM06](#MEM06) [MEM07](#MEM07) [MEM08](#MEM08) | [代码] 内部 IPC 能力证据；不是插件或模型授权入口 |
| `storage::profile_commit` | [MEM06](#MEM06) [MEM07](#MEM07) [MEM08](#MEM08) | [代码] 内部 IPC 能力证据；不是插件或模型授权入口 |
| `storage::profile_restore` | [MEM07](#MEM07) [MEM08](#MEM08) [OPS11](#OPS11) | [代码] 内部 IPC 能力证据；不是插件或模型授权入口 |
| `storage::entity_query` | [MEM08](#MEM08) | [代码] 内部 IPC 能力证据；不是插件或模型授权入口 |
| `storage::entity_commit` | [MEM08](#MEM08) | [代码] 内部 IPC 能力证据；不是插件或模型授权入口 |
| `storage::identity_consolidation_snapshot` | [MEM08](#MEM08) | [代码] 内部 IPC 能力证据；不是插件或模型授权入口 |
| `storage::identity_consolidation_commit` | [MEM08](#MEM08) | [代码] 内部 IPC 能力证据；不是插件或模型授权入口 |
...
| `storage::plugin_docs_page` | [SYS02](#SYS02) | [代码] 内部 IPC 能力证据；不是插件或模型授权入口 |
| `storage::plugin_docs_apply` | [SYS02](#SYS02) | [代码] 内部 IPC 能力证据；不是插件或模型授权入口 |
...
| `external_open::external_open_take` | [SYS12](#SYS12) [SET05](#SET05) | [代码] 内部 IPC 能力证据；不是插件或模型授权入口 |
| `external_open::external_open_is_current` | [SYS12](#SYS12) [SET05](#SET05) | [代码] 内部 IPC 能力证据；不是插件或模型授权入口 |
...
| `resource_images::resource_image_preview` | [EXT06](#EXT06) | [代码] 内部 IPC 能力证据；不是插件或模型授权入口 |
...
| `resources::resource_commit_context` | [MEM13](#MEM13) | [代码] 内部 IPC 能力证据；不是插件或模型授权入口 |
...
| `desktop_startup::desktop_startup_enabled` | [SET04](#SET04) | [代码] 内部 IPC 能力证据；不是插件或模型授权入口 |
| `exit_coordination::app_exit_confirm` | [SYS17](#SYS17) | [代码] 内部 IPC 能力证据；不是插件或模型授权入口 |
...
| `autostart` | [SET04](#SET04) | [代码] 注册库存已映射，不表示产品 E2E 通过 |
...
| 当前注册项 | 矩阵行 | 说明 |
| --- | --- | --- |
| `projections-invalidated` | [CON07](#CON07) [OPS05](#OPS05) | [代码] 注册库存已映射，不表示产品 E2E 通过 |
...
| `context.bundlePublished` | [MEM13](#MEM13) | [代码] 注册库存已映射，不表示产品 E2E 通过 |
...
| 当前注册项 | 矩阵行 | 说明 |
| --- | --- | --- |
| `annotation-desk` | [ANN01](#ANN01) [ANN04](#ANN04) [ANN05](#ANN05) [ANN08](#ANN08) [ANN09](#ANN09) [EXT02](#EXT02) [EXT05](#EXT05) [SYS10](#SYS10) | [代码] 源码版本 0.4.0；源码存在不等于打包、安装、启用或模型可调用 |
...
| `jumper` | [TXT02](#TXT02) [TXT07](#TXT07) [READ01](#READ01) [READ06](#READ06) [READ07](#READ07) [READ13](#READ13) [EXT02](#EXT02) [SYS02](#SYS02) [AI05](#AI05) | [代码] 源码版本 0.8.0；源码存在不等于打包、安装、启用或模型可调用 |
| `library-desk` | [LIB01](#LIB01) [LIB02](#LIB02) [LIB03](#LIB03) [LIB05](#LIB05) [LIB06](#LIB06) [LIB07](#LIB07) [LIB08](#LIB08) [LIB09](#LIB09) [LIB10](#LIB10) [LIB11](#LIB11) [LIB15](#LIB15) [LIB16](#LIB16) [LIB17](#LIB17) [LIB18](#LIB18) [READ01](#READ01) [UI01](#UI01) [UI02](#UI02) [UI03](#UI03) [EXT02](#EXT02) [EXT03](#EXT03) [EXT05](#EXT05) [EXT06](#EXT06) [SYS09](#SYS09) [SYS11](#SYS11) [SYS13](#SYS13) [MORE05](#MORE05) | [代码] 源码版本 0.8.0；源码存在不等于打包、安装、启用或模型可调用 |
| `listening-desk` | [READ16](#READ16) [READ18](#READ18) [READ06](#READ06) [EXT02](#EXT02) [MORE03](#MORE03) | [代码] 源码版本 0.10.0；源码存在不等于打包、安装、启用或模型可调用 |
| `maintenance-desk` | [CFG08](#CFG08) [SYS15](#SYS15) [OPS03](#OPS03) [OPS08](#OPS08) [OPS11](#OPS11) [EXT02](#EXT02) [EXT05](#EXT05) | [代码] 源码版本 0.3.0；源码存在不等于打包、安装、启用或模型可调用 |
...
| `rss-reader` | [EXT10](#EXT10) | [代码] 源码版本 0.12.1；源码存在不等于打包、安装、启用或模型可调用 |
| `sentence-reader` | [READ15](#READ15) [READ16](#READ16) | [代码] 源码版本 1.1.0；源码存在不等于打包、安装、启用或模型可调用 |
| `text-desk` | [TXT04](#TXT04) [TXT05](#TXT05) [TXT06](#TXT06) [TXT07](#TXT07) [TXT10](#TXT10) [TXT11](#TXT11) [TXT12](#TXT12) [TXT13](#TXT13) [LIB01](#LIB01) [READ01](#READ01) [READ13](#READ13) [EXT01](#EXT01) [EXT02](#EXT02) [EXT05](#EXT05) [EXT06](#EXT06) [SYS09](#SYS09) [SYS13](#SYS13) | [代码] 源码版本 0.11.1；源码存在不等于打包、安装、启用或模型可调用 |
...
| `workspace-profiles` | [UI02](#UI02) [UI04](#UI04) [CFG01](#CFG01) [CFG10](#CFG10) [EXT02](#EXT02) [EXT05](#EXT05) [SYS02](#SYS02) | [代码] 源码版本 0.6.0；源码存在不等于打包、安装、启用或模型可调用 |

### Plugin Agent contribution

| 当前注册项 | 矩阵行 | 说明 |
| --- | --- | --- |
| `plugin_dictionary_delete_saved_word` | [EXT09](#EXT09) [AI12](#AI12) [READ07](#READ07) [LIB01](#LIB01) | [代码] global/book；插件启用后才进入工具集；来源 plugins/dictionary/src/agent-tools.ts |
| `plugin_dictionary_export_vocabulary` | [EXT09](#EXT09) [AI12](#AI12) [READ07](#READ07) [LIB01](#LIB01) | [代码] global/book；插件启用后才进入工具集；来源 plugins/dictionary/src/agent-tools.ts |
...
| `plugin_jumper_list_bookmarks` | [TXT02](#TXT02) [TXT07](#TXT07) [READ01](#READ01) [READ06](#READ06) [READ07](#READ07) [READ13](#READ13) [EXT02](#EXT02) [SYS02](#SYS02) [AI05](#AI05) | [代码] 仅 global；插件启用后才进入工具集；来源 plugins/jumper/src/bookmark-tools.ts |
| `plugin_jumper_inspect_bookmark_location` | [TXT02](#TXT02) [TXT07](#TXT07) [READ01](#READ01) [READ06](#READ06) [READ07](#READ07) [READ13](#READ13) [EXT02](#EXT02) [SYS02](#SYS02) [AI05](#AI05) | [代码] 仅 global；插件启用后才进入工具集；来源 plugins/jumper/src/bookmark-tools.ts |
| `plugin_jumper_save_bookmark` | [TXT02](#TXT02) [TXT07](#TXT07) [READ01](#READ01) [READ06](#READ06) [READ07](#READ07) [READ13](#READ13) [EXT02](#EXT02) [SYS02](#SYS02) [AI05](#AI05) | [代码] 仅 global；插件启用后才进入工具集；来源 plugins/jumper/src/bookmark-tools.ts |
| `plugin_jumper_manage_bookmark` | [TXT02](#TXT02) [TXT07](#TXT07) [READ01](#READ01) [READ06](#READ06) [READ07](#READ07) [READ13](#READ13) [EXT02](#EXT02) [SYS02](#SYS02) [AI05](#AI05) | [代码] 仅 global；插件启用后才进入工具集；来源 plugins/jumper/src/bookmark-tools.ts |
...
| `jumper` | [TXT02](#TXT02) [TXT07](#TXT07) [READ01](#READ01) [READ06](#READ06) [READ07](#READ07) [READ13](#READ13) [EXT02](#EXT02) [SYS02](#SYS02) [AI05](#AI05) | [代码] Rust BUNDLED 编译内置清单；不是用户当前安装/启用状态 |
