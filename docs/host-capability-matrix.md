...
| <a id="READ04"></a>READ04 | 前后翻页、章节、书首书尾 | 实装 | **部分**：navigate_reading(next/previous/next-section/previous-section/start/end)<br>[设计] 导航步骤工具 | **部分**：reading 2.12 commands.step(page/section/start/end,guard?)<br>[设计] 导航步骤命令 | 键盘/页面按钮/滚轮；桌面探针 | 已接页面、相邻线性源分节与书首书尾；分节跳过 linear=no，定位相邻分节开头，边界保持实际位置。首尾复用引擎 fraction 0/1；不是 TOC 节点或抽取章节步进。分节/首尾跳转记共享历史，普通页步进不记；复用会话 guard、队列、超时与固定版式 paint 等待。定向测试通过，Tauri/Worker 多格式待集中验收；印刷页定位和全部原生入口收敛仍缺 | [NAV](../apps/web/src/domain/reading-session-controller.ts) [NAVADAPTER](../apps/web/src/features/reader/lib/reading-engine-adapter.ts) [NAVADAPTERPROOF](../apps/web/src/features/reader/lib/reading-engine-adapter.test.ts) [SHORTCUT](../apps/web/src/features/settings/lib/shortcuts.ts) [API](../packages/plugin-types/src/index.ts) [REGISTRY](../packages/agent/src/tools/registry.ts) | D04 |
| <a id="READ05"></a>READ05 | 按进度/固定版式页索引定位 | 实装 | **部分**：open_book(fraction 或 sectionIndex+contentVersion)<br>[设计] 进度/页位置工具 | **部分**：reading 2.12 goTo({fraction} 或 {sectionIndex,contentVersion})<br>[设计] seek 命令 | 阅读进度拖动；两端桌面探针 | 0–1 进度与零起始源分节索引已接；sectionIndex 必须带版本、不可混用 CFI/href/fraction/textQuote，非法索引/旧版本/缺失源节分别拒绝。PDF/漫画每节一源页，固定版式双页展示不改索引；其他格式不将源节冒充页面或目录章节。返回原有真实位置并进入共享历史，定向测试通过，真实多格式组合待集中验收。印刷页标签和重排屏幕页数仍缺 | [READTOOLS](../packages/agent/src/tools/reader-tools.ts) [NAV](../apps/web/src/domain/reading-session-controller.ts) [NAVADAPTER](../apps/web/src/features/reader/lib/reading-engine-adapter.ts) [NAVADAPTERPROOF](../apps/web/src/features/reader/lib/reading-engine-adapter.test.ts) [API](../packages/plugin-types/src/index.ts) | D05 |
...
以下“已映射”只保证注册项可追到矩阵行，不意味着目标已实现。Plugin ctx 是授予当前全部 manifest 权限后的 190 个顶层可调用路径；返回的 collection/session 方法单列。Settings 74 路径是在 custom + 主/快模型配置的完整条件快照中生成，不表示未配置 AI 时也显示全部路径。Native command 包含 cfg/no-op 历史项，见 SYS18，不能算桌面能力全部对插件开放。
