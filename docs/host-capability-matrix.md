...
| <a id="EXT06"></a>EXT06 | 大列表分页/虚拟化、Tree/Table/编辑器/图像资源 | 部分 | **未接**：无正式入口<br>[设计] 有界工具结果/资源引用 | **部分**：views 1.6 树形视图；list/table/tree 分页、语义表格与数据源排序；list 虚拟行<br>[设计] 编辑器/图像 schema 仍缺 | 插件声明数据，宿主绘制表格/树形语义、键盘导航与本地化页控件 | 分页支持未知总页数，空页仍能返回；翻页/排序默认替换整帧，不累积历史，复用错误与回调退休。表格 1..16 列/每页最多200行，文本/有限数/null，行打开可 push/dialog；宿主不只排序当前页。列表搜索/时间过滤只处理当前页。树最多500节点/12层，全树唯一 ID、初始展开和节点动作；宿主拥有展开、方向键/首尾/typeahead，live 保留存续展开、移除焦点退到祖先且不抢外部焦点。展开仅显示已提供子节点，不加载/调用插件；换帧重新初始化，不是虚拟树或惰性子节点/跨帧焦点协议。基础、序列化回调及挂载 React 已验，真实 Worker/Tauri 大数据/窄窗/读屏器留集中验收；资源图片及编辑器未接，不开放 React/HTML/DOM。 | [API](../packages/plugin-types/src/index.ts) [RENDER](../apps/web/src/features/plugins/components/PluginViewRenderer.tsx) [TABLEVIEW](../apps/web/src/features/plugins/components/PluginTableViewBody.tsx) [VIEWPAGING](../apps/web/src/features/plugins/components/PluginViewPagination.tsx) [DATAVIEWPROOF](../apps/web/src/features/plugins/components/plugin-data-views.test.tsx) [TREEVIEW](../apps/web/src/features/plugins/components/PluginTreeViewBody.tsx) [TREEVIEWPROOF](../apps/web/src/features/plugins/components/plugin-tree-view.test.tsx) | J03, J06, J07 |
