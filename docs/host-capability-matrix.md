...
| ID | 宿主能力 | 宿主现状 | Agent 当前与目标 | 插件当前与目标 | 实际消费者 | 缺口/边界 | 来源 | 旧基线 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| <a id="OPS01"></a>OPS01 | 同步连接/断开/立即同步/状态与积压 | 实装 | **接通**：get_sync_status；manage_sync now/settings/connect/disconnect 双 scope<br>[设计] 状态、后端发现、批准同步与宿主连接流程 | **接通**：services.sync 1.1 connectionOptions/requestFlow及既有同步接口<br>[设计] 独立 service:sync 控制面 | Data & Sync；Agent 工具；插件正式入口 | 共享 scheduler，初始/变化串行观察最多64；fresh outbox与cycleStartBacklog分开，不泄漏书籍/blobKey/游标。同步未连接/凭据失效/管理中拒绝；already-running不是完成。connectionOptions仅注册后端ref/名称；connect省略ref为relay，登录仍确认身份和宿主口令，断开仍宿主确认。requestFlow等本地完成或取消；一次一个流程，不替换已有原生对话框，换账号拒绝旧确认。单次signal/退休/Worker超时关闭未开始流程，确认后不强杀原生操作，实际源结束才释放。无持久TaskRef/重启恢复，不回滚共享同步。控制器/双端/受控挂载基础检查通过，真实组合插件和Tauri跨设备验收集中后置。 | [SYNC](../apps/web/src/platform/sync/sync-scheduler.ts) [SYNCCONNECT](../apps/web/src/platform/sync/connect.ts) [ACCOUNTUI](../apps/web/src/features/settings/sections/SyncAccountGroup.tsx) [API](../packages/plugin-types/src/index.ts) [SYNCSERVICE](../apps/web/src/services/sync.ts) [SYNCCONTROLLER](../apps/web/src/services/sync-controller.ts) [SYNCFLOWS](../apps/web/src/services/sync-flow-controller.ts) [SYNCFLOWUI](../apps/web/src/features/settings/hooks/useSyncAccountFlows.ts) [SYNCTOOLS](../packages/agent/src/tools/sync-tools.ts) | R04 |
...
| `maintenance-desk` | [CFG08](#CFG08) [SYS15](#SYS15) [OPS03](#OPS03) [OPS08](#OPS08) [OPS11](#OPS11) [EXT02](#EXT02) [EXT05](#EXT05) | [代码] 源码版本 0.2.0；源码存在不等于打包、安装、启用或模型可调用 |
