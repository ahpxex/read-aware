...
| <a id="READ16"></a>READ16 | 启停模式/上下一单元/跟随/回当前 | 实装 | **部分**：configure_reading_mode + mode.position + navigate_reading(return-to-unit)<br>[设计] 模式配置/返回；缺步进/跟随 | **部分**：reading 2.3 configureMode/returnToMode + session.mode/observeSession<br>[设计] 模式配置/返回；缺步进/跟随 | 文本单元工具栏；Agent；Listening Desk 模式表单与返回按钮 | 发现、unit 选择、启停、状态与版本化 resting Location 已共享。返回等待页面及异步分段恢复，成功才提交历史；模式变更/更新导航/取消阻止迟到成功。恢复验证 book/contentVersion/modeKey/unitId 并重解 CFI，不再钳制旧 ordinal；无版本旧位置丢弃，但保留启用/单位偏好。modeKey 仍是前置条件，不是任意 provider 选择。慢分段、拒绝、取消与真实插件返回见实机证据。仍缺公共单元步进/跟随、跨节/全书末尾完成；短时 sessionTimer 不要求耐久调度 | [MODECONTROL](../apps/web/src/features/reader/lib/reading-mode-controller.ts) [MODEOWNER](../apps/web/src/features/reader/hooks/useReadingModeControl.ts) [UNITS](../apps/web/src/features/reader/hooks/useTextUnitNavigator.ts) [WORKSPACE](../apps/web/src/features/reader/components/ReaderWorkspace.tsx) [SHORTCUT](../apps/web/src/features/settings/lib/shortcuts.ts) [MODESTATE](../apps/web/src/features/reader/lib/text-unit-mode-state.ts) [MODETIMER](../apps/web/src/features/reader/hooks/useSessionTimer.ts) [MODEPROOF](../docs/evidence/reading-mode-2026-09-09.json) [MODERETURN](../docs/evidence/reading-mode-return-2026-09-09.json) | K02 |
...
| <a id="READ18"></a>READ18 | 开始/停止朗读、播放位置与 fallback 状态 | 实装 | **接通**：control_read_aloud + get_reading_session.playback<br>[设计] 朗读控制工具 | **接通**：reading 2.1 controlPlayback + session.playback/observeSession<br>[设计] 共享朗读状态与命令 | 文本单元朗读按钮；Agent；Listening Desk | 实际音频开始才回执；owner 取消/切书阻断迟到合成，系统 fallback 与单元 CFI 可观察。隔离 macOS Tauri 已验证；未验 release/Windows/Linux/远端 TTS。READ16 已接启停/单位配置/返回，公共步进仍缺；非逐字播放时间/暂停恢复，6 秒无下一单元报 timeout，不冒充已读完。Listening Desk 为按需刷新快照。 | [PLAYBACK](../apps/web/src/features/reader/lib/read-aloud-controller.ts) [AUDIO](../apps/web/src/features/reader/hooks/useReadAloud.ts) [READTOOLS](../packages/agent/src/tools/reader-tools.ts) [API](../packages/plugin-types/src/index.ts) [LISTENINGDESK](../plugins/listening-desk/src/views.ts) [PLAYBACKPROOF](../docs/evidence/reading-playback-2026-09-09.json) | K04, K05 |
...
以下“已映射”只保证注册项可追到矩阵行，不意味着目标已实现。Plugin ctx 是授予当前全部 manifest 权限后的 84 个顶层可调用路径；返回的 collection/session 方法单列。Settings 46 路径是在 custom + 主/快模型配置的完整条件快照中生成，不表示未配置 AI 时也显示全部路径。Native command 包含 cfg/no-op 历史项，见 SYS18，不能算桌面能力全部对插件开放。
...
| `listening-desk` | [READ16](#READ16) [READ18](#READ18) [READ06](#READ06) [EXT02](#EXT02) | [代码] 源码版本 0.3.0；源码存在不等于打包、安装、启用或模型可调用 |
