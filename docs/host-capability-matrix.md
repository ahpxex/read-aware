...
| <a id="AI06"></a>AI06 | 一次性文本/结构化/流式 LLM 推理 | 实装 | **自动**：Runtime ask + 线程推理<br>[设计] 运行时服务 | **接通**：llm 1.2 ask：readingContext/schema/onText/signal/timeoutMs<br>[设计] 受预算约束的推理服务 | Dictionary 1.3；Agent 后台管线；推理基础探针 | readingContext仍由宿主过滤，收紧立即取消；必需正文禁止报ai/context-withheld，非法结构报ai/invalid-reading-context。1.2信号穿Worker到普通/结构化/流式源；结构化最多两次共用截止，流式等待每次onText回调，取消后不发旧增量。任意prompt不做来源推断，字典本地缓存策略不变。已有隐私macOS证据不覆盖本批新取消/超时路径；基础测试通过，真实Tauri/服务商组合留集中验收。 | [RUNTIME](../packages/agent/src/runtime/runtime.ts) [CTX](../apps/web/src/features/plugins/runtime/plugin-context.ts) [API](../packages/plugin-types/src/index.ts) [DICT](../plugins/dictionary/src/index.ts) [STRUCTUREDREADING](../packages/agent/src/runtime/one-shot.ts) [STRUCTUREDREADINGPROOF](../docs/evidence/structured-reading-context-2026-09-09.json) [PLUGINLLM](../apps/web/src/features/plugins/runtime/plugin-llm.ts) | L01 |
| <a id="AI07"></a>AI07 | 推理取消、超时、用量/预算/成本可见性 | 部分 | **部分**：线程abort；共享Runtime.ask可接调用者signal<br>[设计] 统一任务/预算 | **部分**：llm 1.2 ask signal/timeoutMs + policy<br>[设计] 推理取消/截止/并发已接，用量成本仍缺 | 聊天Stop；插件推理基础探针 | 默认60秒、整数1..110000ms，整次ask含结构化重试；每plugin ID2/全App8个插件推理，拒绝不排队，跨激活共享，原提供者terminal promise未结算不退槽，退休继续排空。Worker取消保留本地reason，宿主RPC为plugin/cancelled，推理服务内部ai/request-cancelled；超时ai/request-timeout，忙ai/busy，八语言有文案。onText逐条等待不等于服务商网络背压或输出内存上限；SDK不响应取消时清理仍等待。用量、token/费用预算、成本回执、全宿主统一TaskRef仍缺；取消不能保证远端未处理/未收费。新路径真实Tauri验收集中后置。 | [THREAD](../packages/agent/src/runtime/thread.ts) [RUNTIME](../packages/agent/src/runtime/runtime.ts) [CTX](../apps/web/src/features/plugins/runtime/plugin-context.ts) [WIRE](../apps/web/src/features/plugins/runtime/plugin-worker-host.ts) [PLUGINLLM](../apps/web/src/features/plugins/runtime/plugin-llm.ts) | L02 |
...
以下“已映射”只保证注册项可追到矩阵行，不意味着目标已实现。Plugin ctx 是授予当前全部 manifest 权限后的 206 个顶层可调用路径；返回的 collection/session 方法单列。Settings 74 路径是在 custom + 主/快模型配置的完整条件快照中生成，不表示未配置 AI 时也显示全部路径。Native command 包含 cfg/no-op 历史项，见 SYS18，不能算桌面能力全部对插件开放。
