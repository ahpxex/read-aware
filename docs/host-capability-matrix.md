...
| <a id="READ13"></a>READ13 | 读取/建立/清除文本选区 | 实装 | **部分**：get_reading_session.selection；自动附件<br>[设计] Range 查询/受控选择 | **部分**：reading 2.9 session.selection / observeSession；selectionActions 1.2 range<br>[设计] 选区查询/命令 | 选择菜单；Agent；Dictionary；Text Desk 0.6 | 快照有 capture id、最多 12000 UTF-16 预览、全文长度、版本化 Range 或明确不可用原因。真实 FB2/PDF 选区、零/读/写 Worker、双端读范围、观察和 Escape/切书清空已验；PDF 使用页 CFI+quote 而非临时文本层路径。已有原生手势可建立/清除，公共 set/clear 命令未建；迟到非空旧文档选区、coarse-pointer 定时器、长选区与全部格式未验。旧标注范围仍为空，不用当前版本伪造来源。 | [READER](../apps/web/src/features/reader/components/FoliateReaderView.tsx) [ENGINE](../apps/web/foliate-js/src/view.ts) [TEXTACTIONS](../apps/web/src/features/reader/hooks/useReaderTextActions.ts) [API](../packages/plugin-types/src/index.ts) [NAV](../apps/web/src/domain/reading-session-controller.ts) [SELECTIONRANGE](../apps/web/src/features/reader/lib/selection-range.ts) [SELECTIONPROOF](../docs/evidence/selection-range-2026-09-10.json) | E01, E02 |
...
| `text-desk` | [TXT04](#TXT04) [TXT05](#TXT05) [TXT06](#TXT06) [TXT07](#TXT07) [TXT10](#TXT10) [TXT13](#TXT13) [LIB01](#LIB01) [READ01](#READ01) [READ13](#READ13) [EXT01](#EXT01) [EXT02](#EXT02) [EXT05](#EXT05) | [代码] 源码版本 0.6.0；源码存在不等于打包、安装、启用或模型可调用 |
