...
export const units: Unit[] = [
  ...
  unit("D6", "Domain", "memory", "记忆、画像与书内知识的受限读模型", "MEM01-11 MEM13", "domains.memory",
    "memory 1.2 已开放 search/bookGraph/inspect 与有版本条件的 correct/setPinned/forget。个人/跨书/本书记忆复用 Agent 存储检索；人物概念图共用先过滤再合并的纯查询，元数据读取失败不跳过边界。纠错/置顶/遗忘通过正式事件与原子条件提交，Agent manage_memory 每次要求批准；后台现有记录维护同样使用模型前快照和整批条件事务，旧判断不能静默覆盖用户纠错。空闲检查读取真实版本与衰减期限，事务内回执不吞掉新增/迟到写，判断失败保留重试；仍是五分钟空闲轮询，非即时观察或耐久任务。字段来源、任务状态、分类纠正与重建仍需契约。",
    ...
    "叙事图按阅读位置在合并别名/关系前过滤，未知位置不披露；说明性/已读完书可全图，重分类后排除旧 flavor。Agent 书内仍用回合 fence/global 保留既有全图政策，并非两端所有上下文权限相同。原生 macOS debug 已验 Worker、Agent 工具、编译插件、SQLite 读故障/恢复和来源导航。chapterHref 不是版本化来源；摘要内容版本、细粒度授权、完整格式及跨平台仍缺。画像与 bundle 单列，不成为 Jumper 的前置条件。"),
