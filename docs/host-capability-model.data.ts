...
export const units: Unit[] = [
  ...
  unit("D6", "Domain", "memory", "记忆、画像与书内知识的受限读模型", "MEM01-11 MEM13", "domains.memory",
    "memory 1.1 已开放 search/bookGraph/inspect 与有版本条件的 correct/setPinned/forget。个人/跨书/本书记忆复用 Agent 存储检索；人物概念图共用先过滤再合并的纯查询，元数据读取失败不跳过边界。纠错/置顶/遗忘通过正式事件与原子条件提交，Agent manage_memory 每次要求批准；后台现有记录维护同样使用模型前快照和整批条件事务，旧判断不能静默覆盖用户纠错。空闲检查读取真实版本与衰减期限，事务内回执不吞掉新增/迟到写，判断失败保留重试；仍是五分钟空闲轮询，非即时观察或耐久任务。字段来源、任务状态、分类纠正与重建仍需契约。",
    "保留 search_memory/remember/query_book_graph 与自动抽取、巩固、digest；不把内部端口函数存在计作模型工具或成熟产品入口。",
    "Memory Desk 0.2 已组合记忆读写、书库与导航；条件反馈需 memory:write，额外字段与过期版本拒绝。memory:read 目前授予全部记忆 scope，查询 scopes 只是过滤，不是按书授权；图谱按宿主阅读位置保护，插件不能自行声明确认剧透。更细授权与观察仍缺；新增候选由 C4 memoryCandidateProviders 处理，现有记忆通过受控反馈管理，不能写权重、强化次数或画像投影。",
    ...
    "叙事图按阅读位置在合并别名/关系前过滤，未知位置不披露；说明性/已读完书可全图，重分类后排除旧 flavor。Agent 书内仍用回合 fence/global 保留既有全图政策，并非两端所有上下文权限相同。原生 macOS debug 已验 Worker、Agent 工具、编译插件、SQLite 读故障/恢复和来源导航。chapterHref 不是版本化来源；摘要内容版本、观察、细粒度授权、完整格式及跨平台仍缺。画像与 bundle 单列，不成为 Jumper 的前置条件。"),
...
export const scenarios = [
  ...
  ["记忆与人物关系", "D6 C4 Q1", "memory 1.1 与 Memory Desk 0.2 已接检索/保护图谱及有版本条件的纠错/置顶/遗忘，Agent 共享检索与合并逻辑；后台巩固/强化已接模型前快照与整批条件提交，空闲检查覆盖外部写/时间变化，事务回执与失败重试避免漏掉下一轮；按书授权、观察、摘要来源版本、新事实去重/遗忘后再抽取与跨设备协调仍缺。画像投影与正式 bundle 尚未实现；候选来源和拒绝结果必须可辨，不开放原始投影写。"],
