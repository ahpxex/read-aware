...
export const units: Unit[] = [
  ...
  unit("S11", "Service", "session（现有桥，目标收敛）", "消除重复的会话真相", "READ07-08 MORE03", "services.session",
    "当前 session.subscribe 桥的四种阅读事件仍待收敛到 D2.reading 的快照/观察。session 1.1 已共享环境快照与观察：revision/runtime/platform/locale/timeZone/utcOffsetMinutes/networkHint，Listening Desk 0.6 消费离线提示；格式可用性和服务真实就绪仍缺。",
    "get_host_environment 在全局/书内查询同一快照；自动日期/语言上下文与按需查询分清，不为每个环境字段新建工具。networkHint 不是 endpoint 可达性。",
    "environment 每次刷新，observeEnvironment 立即快照后观察 revision；语言/网络/焦点事件与 30 秒时区复核，末个观察者清理监听。读当前书仍须 reading 授权，元数据不含阅读或账号状态。旧四事件迁移仍需协调版本更新，不保留两个会话 API 互相兜底。",
