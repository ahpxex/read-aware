...
export const units: Unit[] = [
  ...
  unit("D5", "Domain", "settings", "全部真实偏好及生效语义", "UI02 UI04-05 CFG01-13 SET01-58", "domains.settings",
  ...
  unit("C1", "Contribution", "actions/options", "动作、命令与动态选项", "UI03-05 EXT01-02 MORE04-05 CFG09", "contributions.selectionActions contributions.headerActions contributions.commands contributions.settingsOptions",
    ...
    "Jumper 用现有 reader headerActions；新增插件自己的命令不需要新增宿主分支。声明无额外 permission 不代表动作能绕过它调用的领域权限。",
  ...
  unit("C4", "Contribution", "agent extensions", "工具、上下文、检索与记忆候选", "AI04-05 AI08-12 MEM02-03 EXT09-10 MORE06", "contributions.agentTools contributions.agentContextProviders contributions.agentRetrievalProviders contributions.memoryCandidateProviders",
    "工具显式声明 scope/schema/风险；上下文每轮有界注入；检索按需返回有来源片段；记忆候选由宿主去重/政策裁决。四个通道保留不同语义和生命周期，不合并成万能 agent.invoke。",
    "只有插件 installed + enabled + scope/grant/availability 满足时才消费。上下文与候选没有插件实例不等于宿主 API 未实现。",
  ...
  unit("V2", "Schema", "settings", "字段声明而非行为授权", "CFG01-13 SET01-58", "schemas.settings",
