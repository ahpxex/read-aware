...
export const units: Unit[] = [
  ...
  unit("D6", "Domain", "memory", "记忆、画像与书内知识的受限读模型", "MEM01-11 MEM13", "domains.memory",
    ...
    "Memory Desk 0.5 已组合记忆读写、书库、导航和生成任务；条件反馈和分类纠正需 memory:write，额外字段与过期版本拒绝。分类详情自动观察，表单确认分类及剧透边界变化并冻结版本；该确认是插件 UX，不是宿主新授权票据。memory:read 目前授予全部记忆 scope，查询 scopes 只是过滤，不是按书授权；图谱按宿主阅读位置保护，插件不能自行声明确认剧透。events.observe 与自有 live views 已接变化/错误/恢复，表单版本冻结；更细授权仍缺；新增候选由 C4 memoryCandidateProviders 处理，现有记忆通过受控反馈管理，不能写权重、强化次数或画像投影。",
...
export const scenarios = [
  ...
  ["记忆与人物关系", "D6 C4 Q1", "memory 1.4 与 Memory Desk 0.5 已接检索/保护图谱及有版本条件的纠错/置顶/遗忘，Agent 共享检索与合并逻辑，双端任务控制与观察已接、失败重建目标保留；后台巩固/强化已接模型前快照与整批条件提交，空闲检查覆盖外部写/时间变化，事务回执与失败重试避免漏掉下一轮；按书授权、摘要来源版本、新事实去重/遗忘后再抽取与跨设备协调仍缺。画像投影与正式 bundle 尚未实现；候选来源和拒绝结果必须可辨，不开放原始投影写。"],
