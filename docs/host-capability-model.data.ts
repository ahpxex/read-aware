...
export const units: Unit[] = [
  ...
  unit("D6", "Domain", "memory", "记忆、画像与书内知识的受限读模型", "MEM01-11 MEM13", "domains.memory",
    "memory 1.3 已开放 search/bookGraph/inspect、条件反馈与 classification/classify/分类观察。个人/跨书/本书记忆复用 Agent 存储检索；人物概念图共用先过滤再合并的纯查询，元数据读取失败不跳过边界。纪要提示词现共用同一策略，分类/状态/坐标变化时下一轮刷新且保留同章对话，不回收已发内容。纠错/置顶/遗忘通过正式事件与原子条件提交，Agent manage_memory 每次要求批准；后台现有记录维护同样使用模型前快照和整批条件事务，旧判断不能静默覆盖用户纠错。空闲检查读取真实版本与衰减期限，事务内回执不吞掉新增/迟到写，判断失败保留重试；仍是五分钟空闲轮询，非即时观察或耐久任务。分类纠正已接双端条件写，自动分类只填空、不覆盖新选择；字段来源、图谱任务状态与重建仍需契约。",
    ...
    "Memory Desk 0.4 已组合记忆读写、书库与导航；条件反馈和分类纠正需 memory:write，额外字段与过期版本拒绝。分类详情自动观察，表单确认分类及剧透边界变化并冻结版本；该确认是插件 UX，不是宿主新授权票据。memory:read 目前授予全部记忆 scope，查询 scopes 只是过滤，不是按书授权；图谱按宿主阅读位置保护，插件不能自行声明确认剧透。events.observe 与自有 live views 已接变化/错误/恢复，表单版本冻结；更细授权仍缺；新增候选由 C4 memoryCandidateProviders 处理，现有记忆通过受控反馈管理，不能写权重、强化次数或画像投影。",
...
export const scenarios = [
  ...
  ["记忆与人物关系", "D6 C4 Q1", "memory 1.3 与 Memory Desk 0.4 已接检索/保护图谱及有版本条件的纠错/置顶/遗忘，Agent 共享检索与合并逻辑；后台巩固/强化已接模型前快照与整批条件提交，空闲检查覆盖外部写/时间变化，事务回执与失败重试避免漏掉下一轮；按书授权、摘要来源版本、新事实去重/遗忘后再抽取与跨设备协调仍缺。画像投影与正式 bundle 尚未实现；候选来源和拒绝结果必须可辨，不开放原始投影写。"],
