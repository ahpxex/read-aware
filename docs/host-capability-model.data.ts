...
export const units: Unit[] = [
  ...
  unit("S4", "Service", "resources", "文件、书籍资产与私有二进制资源", "LIB08-09 TXT11-12 SYS09-11 SYS13 MORE07", "services.resources",
    ...
    "读写文件必须有来源和用户意图，不接受模型猜出的任意路径；Agent 已接原生选文件、批准原书导出引用、UTF-8 分页读取、二进制保存和释放，按 conversation 隔离。原书引用在端口强制只导出，不能绕过防剧透读入模型；字节在宿主保存。目录/拖放接管和 FileRef 直接导入仍待接。",
    "resources 1.0 已接 pick/openBook/create/stat/read/append/commit/save/release。原书需 library 授权；快照与自建文件为匿名临时文件，1 小时引用、不漫游/不备份/不入 blob outbox。每 owner 16 引用/1 GiB，宿主 64 文件/2 GiB，1 MiB 分块、32 个串行排队上限；offset 校验防重复追加，commit 封口，release 兼 abort，退出激活排空清理。选中源的后续变化不修改快照。书内图片/封面、包内资产、可跨激活持久化资源与 transferable 桥优化仍缺；旧 ui.exportFile 便利接口尚未迁移。定向检查与原生文件单测不替代 Tauri E2E。",
