...
export const units: Unit[] = [
  ...
  unit("S10", "Service", "plugins（目标新增只读目录）", "插件可用性与受控自管理", "EXT11-12 MORE06", "",
    ...
    "六个源码插件与五个 bundled 的库存独立核对；实际 installed/enabled 需运行态证据；失效依赖、禁用 provider 不返回可用。"),
  unit("S11", "Service", "session（现有桥，目标收敛）", "消除重复的会话真相", "READ07-08 MORE03", "services.session",
    "当前 session.subscribe 桥的四种阅读事件收敛到 D2.reading 的快照/观察。ctx 的 locale/appVersion/capabilities 保留环境元数据；增加确有消费者的 platform/timezone/availability 只读描述与失效通知。",
    "自动日期/语言上下文与按需环境查询分清，不为每个环境字段新建工具。",
    "当前 locale 不是持续订阅；读当前书必须通过 reading 授权而不是借无 permission 的 session metadata 偷渡。迁移采用协调版本更新，不保留两个会话 API 互相兜底。",
...
export const refinements = [
  ...
  ["R4", "细化现有契约", "READ16", "modeKey/unitId/content version 决定恢复是否合法；模式私有位置不得由插件直接写 read-aware-navigator-state:*。mode sessionTimer 从零计时，不持久化是设计，不是 durable scheduler 缺口。"],
  ...
  ["R8", "校正库存口径", "EXT11", "源码有六个第一方插件，Rust BUNDLED 编译清单只有五个，不含 WebDAV；此前把源码目录称为 Bundled plugin 不准确。本轮分开枚举，不推断用户安装态。"],
...
export const scenarios = [
  ...
  ["句读与 TTS", "D2 C2 C3 D5 Q2", "reading 2.2 已共享朗读和当前模式快照、启停、句段单位配置；Listening Desk 组合模式表单、朗读和导航历史。隔离 macOS Tauri 验证真实分段完成、拒绝、Agent 取消、插件停用、用户接管、关闭、只读及 PDF 不支持；朗读互控/续读/回退见此前证据。READ16 步进/回当前、内容版本化恢复、任意 provider 选择仍缺；release、其他系统、远端 TTS 未验。"],
