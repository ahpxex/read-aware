...
export const units: Unit[] = [
  ...
  unit("D6", "Domain", "memory", "记忆、画像与书内知识的受限读模型", "MEM01-11 MEM13", "domains.memory",
    "memory 1.0 已开放 search/bookGraph 两个只读查询。个人/跨书/本书记忆复用 Agent 存储检索；人物概念图共用先过滤再合并的纯查询，元数据读取失败不跳过边界。字段来源、任务状态、记住用户确认的事实、纠错/遗忘反馈、分类纠正和重建请求仍需受控业务契约。",
    "保留 search_memory/remember/query_book_graph 与自动抽取、巩固、digest；不把内部端口函数存在计作模型工具或成熟产品入口。",
    "Memory Desk 0.1 已组合只读记忆、书库和导航。memory:read 目前授予全部记忆 scope，查询 scopes 只是过滤，不是按书授权；图谱按宿主阅读位置保护，插件不能自行声明确认剧透。更细授权与观察仍缺；提议写由 C4 memoryCandidateProviders 或受控反馈处理，不能写权重、强化次数或画像投影。",
...
export const scenarios = [
  ...
  ["记忆与人物关系", "D6 C4 Q1", "memory 1.0 与 Memory Desk 已接只读检索/保护图谱，Agent 共享检索与合并逻辑；按书授权、观察、摘要来源版本仍缺。画像投影与正式 bundle 尚未实现；候选来源和拒绝结果必须可辨，不开放原始投影写。"],
