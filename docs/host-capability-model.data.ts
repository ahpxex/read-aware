...
export const units: Unit[] = [
  ...
  unit("S4", "Service", "resources", "文件、书籍资产与私有二进制资源", "LIB08-09 TXT11-12 SYS09-11 SYS13 MORE07", "services.resources",
    ...
    "resources 1.1 已接 pick/openBook/openCover/create/stat/read/append/commit/save/release；原书和封面需 library 授权，封面仅查询本地 ready 快照，不生成/下载；Agent open_book_cover 只返回元数据。library 1.8 importResource 消费本激活封口引用，同源原生导入与内容去重，已受理阶段不因取消而中断落库，导入数据遵循既有同步设置。快照与自建文件为匿名临时文件，1 小时引用、不漫游/不备份/不入 blob outbox。每 owner 16 引用/1 GiB，宿主 64 文件/2 GiB，1 MiB 分块、32 个串行排队上限；offset 校验防重复追加，commit 封口，release 兼 abort，退出激活排空清理。选中源的后续变化不修改快照。桌面 ui.exportFile 与灯箱复制已共用原生资源后端；封面详细状态/提取控制/插件资源呈现、书内图片、包内资产、持久化资源与 transferable 优化仍缺。定向检查与原生文件单测不替代 Tauri E2E。",
