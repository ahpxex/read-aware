...
| READ10 | 目录/注释/外观/聊天面板开关 / 实装 | 未接：无正式入口 | 未接：无正式入口 | Reader header；参数化面板操作，不开放 Jotai atom。宿主目录/聊天偏好已改为共享 KV 镜像，SQLite 成功才运行焦点续接；保存失败回滚并提示，排队意图基于前次结算状态，换书取消未派发写，旧意图不抢焦点。窄窗互斥切换单次保存，隔离 macOS debug 已验真实数据库拒绝/重试、重开和外部更新。此为双端面板服务的前置修复，未新增 Agent/插件面板 API；外观/注释仍为 UI 临时状态，DOM 完成回执/隐藏菜单入口与完整跨平台验收仍待接。原生重开观察到 wheel-phase 退订异常，记录为未解决生命周期问题。 | [S3](#S3) | [WORKSPACE](../apps/web/src/features/reader/components/ReaderWorkspace.tsx) [UI](../apps/web/src/state/ui.ts) [MENU](../apps/web/src/features/menus/lib/menu-registry.tsx) [API](../packages/plugin-types/src/index.ts) [PANELLAYOUT](../apps/web/src/features/reader/lib/reader-panel-layout.ts) [PANELHOOK](../apps/web/src/features/reader/hooks/useReaderPanelLayout.ts) [PANELPROOF](../docs/evidence/reader-panel-persistence-2026-09-09.json) |
