...
| 句读与 TTS | [D2](#D2) · [C2](#C2) · [C3](#C3) · [D5](#D5) · [Q2](#Q2) | reading 2.1 已将朗读 start/stop、实际播放状态、单元 CFI 与 fallback 接到 Agent/插件；Listening Desk 组合朗读和导航历史。隔离 macOS Tauri 验证双端互控、自动续读、系统回退、发起插件停用与关闭时的迟到合成隔离。模式启停/步进/恢复 READ16 仍未接；release、其他系统、远端 TTS 未验。 |
...
| READ07 | 统一当前书/位置/加载/历史快照 / 实装 | 接通：get_reading_session（书内 scope 不泄露其他书） | 接通：reading.queries.session() | 共享控制器；Agent；可中途启用的插件；revision/sessionId/bookId/status/location/history 已共享；reading 2.1 新增 playback，详见 READ18；此行不代表 selection/mode 已实现 | [D2](#D2) · [S11](#S11) | [NAV](../apps/web/src/domain/reading-session-controller.ts) [READING](../apps/web/src/domain/reading.ts) [READTOOLS](../packages/agent/src/tools/reader-tools.ts) [API](../packages/plugin-types/src/index.ts) [NAVPROBE](../apps/web/src/features/plugins/runtime/fixtures/desktop-reading-probe.ts) |
