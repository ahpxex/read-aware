...
| 句读与 TTS | [D2](#D2) · [C2](#C2) · [C3](#C3) · [D5](#D5) · [Q2](#Q2) | reading 2.3 已共享朗读、模式快照/启停/单位配置及版本化位置返回；Listening Desk 组合模式表单、朗读、导航历史和 Current passage。返回等待导航及分段恢复，失败/取消不提交成功历史；无版本旧地址不恢复。隔离 macOS Tauri 验证慢分段返回、拒绝、模式变更取消，其他启停/朗读边界见此前证据。READ16 公共单元步进/跟随、跨节/全书末尾完成、任意 provider 选择仍缺；release、其他系统、远端 TTS 未验。 |
...
| READ16 | 启停模式/上下一单元/跟随/回当前 / 实装 | 部分：configure_reading_mode + mode.position + navigate_reading(return-to-unit) | 部分：reading 2.3 configureMode/returnToMode + session.mode/observeSession | 文本单元工具栏；Agent；Listening Desk 模式表单与返回按钮；发现、unit 选择、启停、状态与版本化 resting Location 已共享。返回等待页面及异步分段恢复，成功才提交历史；模式变更/更新导航/取消阻止迟到成功。恢复验证 book/contentVersion/modeKey/unitId 并重解 CFI，不再钳制旧 ordinal；无版本旧位置丢弃，但保留启用/单位偏好。modeKey 仍是前置条件，不是任意 provider 选择。慢分段、拒绝、取消与真实插件返回见实机证据。仍缺公共单元步进/跟随、跨节/全书末尾完成；短时 sessionTimer 不要求耐久调度 | [D2](#D2) · [C2](#C2) | [MODECONTROL](../apps/web/src/features/reader/lib/reading-mode-controller.ts) [MODEOWNER](../apps/web/src/features/reader/hooks/useReadingModeControl.ts) [UNITS](../apps/web/src/features/reader/hooks/useTextUnitNavigator.ts) [WORKSPACE](../apps/web/src/features/reader/components/ReaderWorkspace.tsx) [SHORTCUT](../apps/web/src/features/settings/lib/shortcuts.ts) [MODESTATE](../apps/web/src/features/reader/lib/text-unit-mode-state.ts) [MODETIMER](../apps/web/src/features/reader/hooks/useSessionTimer.ts) [MODEPROOF](../docs/evidence/reading-mode-2026-09-09.json) [MODERETURN](../docs/evidence/reading-mode-return-2026-09-09.json) |
