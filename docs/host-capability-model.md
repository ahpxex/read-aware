...
- [代码] 224 条现状证据映射到 30 个责任单元；30 个当前 catalog 成员、129 个旧验收项、32 个旧场景全部反查。单元不是新 API 数，字段行不是独立产品功能。
