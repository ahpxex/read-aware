...
| 句读与 TTS | [D2](#D2) · [C2](#C2) · [C3](#C3) · [D5](#D5) · [Q2](#Q2) | reading 2.2 已共享朗读和当前模式快照、启停、句段单位配置；Listening Desk 组合模式表单、朗读和导航历史。隔离 macOS Tauri 验证真实分段完成、拒绝、Agent 取消、插件停用、用户接管、关闭、只读及 PDF 不支持；朗读互控/续读/回退见此前证据。READ16 步进/回当前、内容版本化恢复、任意 provider 选择仍缺；release、其他系统、远端 TTS 未验。 |
...
| READ07 | 统一当前书/位置/加载/历史快照 / 实装 | 接通：get_reading_session（书内 scope 不泄露其他书） | 接通：reading.queries.session() | 共享控制器；Agent；可中途启用的插件；revision/sessionId/bookId/status/location/history 已共享；2.1 新增 playback、2.2 新增 mode（READ18/READ16）。此行不代表 selection 或完整模式步进/恢复已接通 | [D2](#D2) · [S11](#S11) | [NAV](../apps/web/src/domain/reading-session-controller.ts) [READING](../apps/web/src/domain/reading.ts) [READTOOLS](../packages/agent/src/tools/reader-tools.ts) [API](../packages/plugin-types/src/index.ts) [NAVPROBE](../apps/web/src/features/plugins/runtime/fixtures/desktop-reading-probe.ts) |
