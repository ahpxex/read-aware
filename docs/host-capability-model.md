...
| READ16 | 启停模式/上下一单元/跟随/回当前 / 实装 | 未接：无正式入口 | 未接：无正式入口 | 文本单元工具栏；快捷键；兼容模式位置恢复；底层已有 building/ready/empty/error，失败/构建时禁用步进与朗读，但双端状态和命令尚未接。恢复需 modeKey/unitId/contentVersion 匹配；当前仍缺内容版本校验和跨节完成语义。短时 sessionTimer 不持久化是设计，不要求耐久调度 | [D2](#D2) · [C2](#C2) | [UNITS](../apps/web/src/features/reader/hooks/useTextUnitNavigator.ts) [WORKSPACE](../apps/web/src/features/reader/components/ReaderWorkspace.tsx) [SHORTCUT](../apps/web/src/features/settings/lib/shortcuts.ts) [MODESTATE](../apps/web/src/features/reader/lib/text-unit-mode-state.ts) [MODETIMER](../apps/web/src/features/reader/hooks/useSessionTimer.ts) [UNITPROOF](../docs/evidence/reading-segmentation-2026-09-09.json) |
