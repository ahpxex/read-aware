...
- [代码] 215 条现状证据映射到 30 个责任单元；30 个当前 catalog 成员、129 个旧验收项、32 个旧场景全部反查。单元不是新 API 数，字段行不是独立产品功能。
...
- [设计] 通过条件：六个源码插件与五个 bundled 的库存独立核对；实际 installed/enabled 需运行态证据；失效依赖、禁用 provider 不返回可用。
...
### <a id="S11"></a>S11 · Service · session（现有桥，目标收敛）

**消除重复的会话真相**

- 当前 catalog 身份：`services.session`。
- [设计] 操作：当前 session.subscribe 桥的四种阅读事件收敛到 D2.reading 的快照/观察。ctx 的 locale/appVersion/capabilities 保留环境元数据；增加确有消费者的 platform/timezone/availability 只读描述与失效通知。
- [设计] Agent：自动日期/语言上下文与按需环境查询分清，不为每个环境字段新建工具。
- [设计] 插件：当前 locale 不是持续订阅；读当前书必须通过 reading 授权而不是借无 permission 的 session metadata 偷渡。迁移采用协调版本更新，不保留两个会话 API 互相兜底。
...
| 裁决 | 分类 | 证据 | 具体结论 |
| --- | --- | --- | --- |
| R1 | 补真实遗漏 | [CFG11](./host-capability-matrix.md#CFG11) | 内容字体四字段 followReader/fontFamily/fontSize/lineSpacing，独立尺寸 x-small/small/medium/large/x-large，行距 compact/comfortable/relaxed；默认 true/null/medium/comfortable。跟随全局 reader，null 是应用 sans。目标路径建议 appearance.contentTypography.*，不是当前已注册路径。 |
| R2 | 补真实遗漏 | [CFG12](./host-capability-matrix.md#CFG12) | 默认标注色 yellow/green/blue/pink，默认 yellow；recolor 影响之后的一键标注。目标 annotations.defaultColor 属于 settings 路径，不是 annotations 领域方法。 |
| R3 | 补真实遗漏 | [CFG13](./host-capability-matrix.md#CFG13) | 更新通道 stable/beta，默认 stable，设备本地且不漫游。目标 general.updateChannel 属于 settings；不是批准升级/重启。 |
| R4 | 细化现有契约 | [READ16](./host-capability-matrix.md#READ16) | modeKey/unitId/content version 决定恢复是否合法；模式私有位置不得由插件直接写 read-aware-navigator-state:*。mode sessionTimer 从零计时，不持久化是设计，不是 durable scheduler 缺口。 |
...
| R8 | 校正库存口径 | [EXT11](./host-capability-matrix.md#EXT11) | 源码有六个第一方插件，Rust BUNDLED 编译清单只有五个，不含 WebDAV；此前把源码目录称为 Bundled plugin 不准确。本轮分开枚举，不推断用户安装态。 |
...
[设计] CFG11/CFG12/CFG13 的建议路径只是目标命名，当前 catalog 中不存在；实现需沿 settings 定义、权限、默认值、验证、持久化与实际消费者接入，不能只修改文档或 schema。

## 组合场景

| 场景 | 责任单元 | 当前缺口与验收重点 |
| --- | --- | --- |
| Jumper | [D1](#D1) · [D2](#D2) · [C1](#C1) · [S3](#S3) · [V1](#V1) · [Q2](#Q2) · [Q3](#Q3) | 已有 reader header 入口；缺有位置 TOC/精确搜索、session 快照、完成回执、统一历史和异步结果失效。编号规则/搜索匹配策略留插件。不存在章节不派发；A 查询迟到不能覆盖 B；back 失败不移动指针。 |
...
| 句读与 TTS | [D2](#D2) · [C2](#C2) · [C3](#C3) · [D5](#D5) · [Q2](#Q2) | 贡献分段/合成已存在，启停/步进/恢复/实际播放查询未开放；宿主补 reading 控制，不新增单独播放器域。换书和禁用 provider 后不播放迟到音频。 |
| 主题与定时主题 | [D5](#D5) · [C5](#C5) · [S8](#S8) · [V3](#V3) | 主题/字体可组合，已有 settings 足以切换；短 clock 不要求耐久工作流。新增主题无需宿主改动，缺字体时回退。 |
| WebDAV | [C5](#C5) · [S2](#S2) · [S5](#S5) · [S9](#S9) · [Q2](#Q2) | 贡献密文传输已有；缺 session close 和受控同步状态/连接面。连接失败不伪装已同步，撤权不能继续使用旧 session。 |
...
| TXT02 | 读取原书分层导航目录及 href / 实装 | 部分：BookTextPort.hrefs 内部可供 open_book | 未接：无正式入口 | 阅读目录；Agent 内部映射不等于完整目录工具输出；索引不等于印刷章号 | [D1](#D1) | [READER](../apps/web/src/features/reader/components/FoliateReaderView.tsx) [WORKSPACE](../apps/web/src/features/reader/components/ReaderWorkspace.tsx) [READTOOLS](../packages/agent/src/tools/reader-tools.ts) [TEXTPORT](../apps/web/src/features/ai/agent/ports/book-text-port.ts) |
...
| TXT04 | 查询无文本/未抽取/就绪 / 实装 | 部分：get_toc/read_chapter 内部 getTextStatus | 未接：无正式入口 | Agent 正文工具；port 只有 ok/unextracted/textless，缺处理中/部分失败的正式模型 | [D1](#D1) | [TEXT](../apps/web/src/features/library/lib/book-text-store.ts) [TEXTPORT](../apps/web/src/features/ai/agent/ports/book-text-port.ts) [TEXTTOOLS](../packages/agent/src/tools/book-text-tools.ts) |
| TXT05 | 启动、重建、暂停让路正文抽取 / 实装 | 未接：无正式入口 | 未接：无正式入口 | 书籍导入/阅读需求优先调度；任务控制仍在宿主；reader-demand-activity 非公开插件事件 | [D1](#D1) | [TEXT](../apps/web/src/features/library/lib/book-text-store.ts) [APPEVENTS](../apps/web/src/platform/app-events.ts) [SESSION](../apps/web/src/features/reader/hooks/useReaderSession.ts) |
...
| TXT07 | 引擎全文精确搜索并返回 CFI / 引擎 | 未接：无正式入口 | 未接：无正式入口 | Foliate search 已有；未见内置全文搜索 UI 接入；不能把引擎已有说成产品 UI 已有；Agent 文本检索不可替代位置搜索 | [D1](#D1) | [ENGINE](../apps/web/foliate-js/src/view.ts) [READER](../apps/web/src/features/reader/components/FoliateReaderView.tsx) [READTOOLS](../packages/agent/src/tools/reader-tools.ts) |
...
| TXT09 | 读取当前可见文本/阅读游标 / 实装 | 自动：ReadingCursor 进入书内每轮 grounding | 部分：选择/动作回调仅给局部上下文 | 书内 Agent；文本单元导航；Agent 自动上下文并无独立快照工具；插件不能随时查询视口 | [D2](#D2) | [READER](../apps/web/src/features/reader/components/FoliateReaderView.tsx) [GROUND](../packages/agent/src/runtime/grounding-context.ts) [THREAD](../packages/agent/src/runtime/thread.ts) [API](../packages/plugin-types/src/index.ts) |
...
| TXT13 | 统一位置/范围解析、校验、版本与失效 / 部分 | 部分：open_book 接受 anchor/chapterHref/index | 部分：goTo 接受 cfi/href | 目录/标注/Agent/RSS；缺稳定跨来源引用和 content generation；插件不能靠自行造 CFI 补齐 | [D1](#D1) | [ENGINE](../apps/web/foliate-js/src/view.ts) [NAV](../apps/web/src/features/plugins/state/reader-nav.ts) [READTOOLS](../packages/agent/src/tools/reader-tools.ts) [API](../packages/plugin-types/src/index.ts) |
| READ01 | 打开书/恢复保存位置 / 实装 | 部分：open_book 返回 opened:true 仅表示派发 | 部分：reading.commands.openBook void | 书架；Agent；RSS；两端共同缺实际开书成功回执，不只是插件缺口 | [D2](#D2) | [SESSION](../apps/web/src/features/reader/hooks/useReaderSession.ts) [READERPORT](../apps/web/src/features/ai/agent/ports/reader-port.ts) [READTOOLS](../packages/agent/src/tools/reader-tools.ts) [CTX](../apps/web/src/features/plugins/runtime/plugin-context.ts) |
| READ02 | 关闭当前书并返回书架 / 实装 | 未接：无正式入口 | 未接：无正式入口 | 阅读关闭/完成页；开与关应成对；失效 SessionRef 不可关闭新书 | [D2](#D2) | [SESSION](../apps/web/src/features/reader/hooks/useReaderSession.ts) [WORKSPACE](../apps/web/src/features/reader/components/ReaderWorkspace.tsx) [APP](../apps/web/src/App.tsx) [API](../packages/plugin-types/src/index.ts) |
| READ03 | 按章节/标注/href/CFI 跳转 / 实装 | 部分：open_book(章节索引/标注/anchor/href) | 部分：reading.commands.goTo(cfi/href) | 阅读目录/标注；Agent；RSS 文章；Agent 比插件多索引解析，但共同缺 ready/completion；无位置标注可退化成仅开书 | [D2](#D2) | [READTOOLS](../packages/agent/src/tools/reader-tools.ts) [READERPORT](../apps/web/src/features/ai/agent/ports/reader-port.ts) [CTX](../apps/web/src/features/plugins/runtime/plugin-context.ts) [NAV](../apps/web/src/features/plugins/state/reader-nav.ts) |
| READ04 | 前后翻页、章节、书首书尾 / 实装 | 未接：无正式入口 | 未接：无正式入口 | 键盘/页面按钮/滚轮；不得以 open_book 近似所有翻页语义 | [D2](#D2) | [READER](../apps/web/src/features/reader/components/FoliateReaderView.tsx) [SHORTCUT](../apps/web/src/features/settings/lib/shortcuts.ts) [API](../packages/plugin-types/src/index.ts) [REGISTRY](../packages/agent/src/tools/registry.ts) |
| READ05 | 按进度/固定版式页索引定位 / 实装 | 未接：无正式入口 | 未接：无正式入口 | 阅读进度拖动；页索引/印刷页标签/重排页数必须区分 | [D2](#D2) | [READER](../apps/web/src/features/reader/components/FoliateReaderView.tsx) [WORKSPACE](../apps/web/src/features/reader/components/ReaderWorkspace.tsx) [ENGINE](../apps/web/foliate-js/src/view.ts) [API](../packages/plugin-types/src/index.ts) |
| READ06 | 导航历史 back/forward 及可用性 / 引擎 | 未接：无正式入口 | 未接：无正式入口 | Foliate history；未见完整共享 UI 命令接入；引擎指针移动不等于导航成功；公共历史所有权未建立 | [D2](#D2) | [HISTORY](../apps/web/foliate-js/src/history.ts) [ENGINE](../apps/web/foliate-js/src/view.ts) [API](../packages/plugin-types/src/index.ts) [READTOOLS](../packages/agent/src/tools/reader-tools.ts) |
| READ07 | 统一当前书/位置/加载/历史快照 / 部分 | 自动：书内 scope+cursor 注入，非可调用查询 | 部分：session 未来事件 + stats 持久读 | ReaderWorkspace；Agent 每轮上下文；宿主状态分散在组件/引擎，尚无统一快照；中途启用插件无法重建全部当前状态 | [D2](#D2) · [S11](#S11) | [SESSION](../apps/web/src/features/reader/hooks/useReaderSession.ts) [THREAD](../packages/agent/src/runtime/thread.ts) [API](../packages/plugin-types/src/index.ts) |
| READ08 | 会话开关/章节/进度事件 / 实装 | 自动：宿主向线程传递 cursor/scope | 部分：services.session.subscribe 四事件 | Dictionary 缓存当前书标题；四事件存在，但并无 ready/精确位置/历史/模式事件 | [D2](#D2) · [S11](#S11) | [APPEVENTS](../apps/web/src/platform/app-events.ts) [THREAD](../packages/agent/src/runtime/thread.ts) [API](../packages/plugin-types/src/index.ts) [DICTTOOLS](../plugins/dictionary/src/agent-tools.ts) |
| READ09 | 阅读沉浸/显示隐藏控制层 / 实装 | 未接：无正式入口 | 未接：无正式入口 | 空格/内容点击/滚动隐藏；属于宿主运行态，不因不持久化而拒绝抽象 | [D2](#D2) | [WORKSPACE](../apps/web/src/features/reader/components/ReaderWorkspace.tsx) [READER](../apps/web/src/features/reader/components/FoliateReaderView.tsx) [SHORTCUT](../apps/web/src/features/settings/lib/shortcuts.ts) |
| READ10 | 目录/注释/外观/聊天面板开关 / 实装 | 未接：无正式入口 | 未接：无正式入口 | Reader header；参数化面板操作，不是允许访问 Jotai atom | [S3](#S3) | [WORKSPACE](../apps/web/src/features/reader/components/ReaderWorkspace.tsx) [UI](../apps/web/src/state/ui.ts) [MENU](../apps/web/src/features/menus/lib/menu-registry.tsx) [API](../packages/plugin-types/src/index.ts) |
...
| READ15 | 贡献句子/段落等分段模式 / 实装 | 未接：无算法注册工具 | 接通：readerModes.register(text-unit-navigator) | sentence-reader；只开放分段策略，不是任意渲染器/格式解码器 | [D2](#D2) · [C2](#C2) | [API](../packages/plugin-types/src/index.ts) [CTX](../apps/web/src/features/plugins/runtime/plugin-context.ts) [SENTENCE](../plugins/sentence-reader/src/index.ts) [UNITS](../apps/web/src/features/reader/hooks/useTextUnitNavigator.ts) |
| READ16 | 启停模式/上下一单元/跟随/回当前 / 实装 | 未接：无正式入口 | 未接：无正式入口 | 文本单元工具栏；快捷键；兼容模式位置恢复；贡献模式不等于能控制模式；恢复需 modeKey/unitId 匹配，旧 ordinal 不可强套新分段；短时 sessionTimer 不持久化是设计，不要求耐久调度 | [D2](#D2) · [C2](#C2) | [UNITS](../apps/web/src/features/reader/hooks/useTextUnitNavigator.ts) [WORKSPACE](../apps/web/src/features/reader/components/ReaderWorkspace.tsx) [SHORTCUT](../apps/web/src/features/settings/lib/shortcuts.ts) [MODESTATE](../apps/web/src/features/reader/lib/text-unit-mode-state.ts) [MODETIMER](../apps/web/src/features/reader/hooks/useSessionTimer.ts) |
...
| READ18 | 开始/停止朗读、播放位置与 fallback 状态 / 实装 | 未接：无正式入口 | 未接：无正式入口 | 文本单元朗读按钮；宿主拥有音频播放/预取/回退；两端都无直接控制 | [D2](#D2) · [C3](#C3) | [AUDIO](../apps/web/src/features/reader/hooks/useReadAloud.ts) [UNITS](../apps/web/src/features/reader/hooks/useTextUnitNavigator.ts) [API](../packages/plugin-types/src/index.ts) |
...
| READ20 | 跨书/并发导航的取消、序列化与回执 / 待建 | 部分：open_book sequential 只约束工具调度 | 部分：requestId 派发，不是完成协议 | 现有局部防重；用户/Agent/插件竞争、失败不推进历史仍缺统一裁决 | [D2](#D2) · [Q2](#Q2) | [READTOOLS](../packages/agent/src/tools/reader-tools.ts) [NAV](../apps/web/src/features/plugins/state/reader-nav.ts) [APP](../apps/web/src/App.tsx) [HISTORY](../apps/web/foliate-js/src/history.ts) |
| ANN01 | 列出/按书按词按类型检索标注 / 实装 | 接通：get_annotations；类型在工具层过滤 | 接通：annotations.queries.list | 阅读注释；全局 Agent 注释；Agent；列表已接通，分页/稳定游标另列 | [D3](#D3) | [ANNOT](../apps/web/src/domain/annotations.ts) [ANNDB](../apps/web/src/features/annotations/lib/annotation-db.ts) [LIBTOOLS](../packages/agent/src/tools/library-tools.ts) [CTX](../apps/web/src/features/plugins/runtime/plugin-context.ts) |
...
| ANN03 | 创建下划线样式 / 实装 | 未接：create_annotation 未传 style | 接通：createHighlight(style=underline) | 选择菜单；同一 host 命令支持 style，但 Agent adapter/tool 丢了该参数 | [D3](#D3) | [ANNOT](../apps/web/src/domain/annotations.ts) [ANNTOOLS](../packages/agent/src/tools/annotation-tools.ts) [TEXTACTIONS](../apps/web/src/features/reader/hooks/useReaderTextActions.ts) [API](../packages/plugin-types/src/index.ts) |
| ANN04 | 高亮改色/删除 / 实装 | 接通：edit_annotation / delete_annotation+批准 | 接通：recolorHighlight/removeHighlight | 标注菜单；Agent；修改高亮文本/改样式不是当前命令的已支持语义 | [D3](#D3) | [ANNOT](../apps/web/src/domain/annotations.ts) [ANNTOOLS](../packages/agent/src/tools/annotation-tools.ts) [CTX](../apps/web/src/features/plugins/runtime/plugin-context.ts) |
| ANN05 | 创建/编辑/删除笔记 / 实装 | 接通：create/edit/delete_annotation | 接通：createNote/updateNote/removeNote | NoteEditor；Agent；可无位置笔记；删除 Agent 批准和插件授权不是同一种策略 | [D3](#D3) | [ANNOT](../apps/web/src/domain/annotations.ts) [ANNTOOLS](../packages/agent/src/tools/annotation-tools.ts) [CTX](../apps/web/src/features/plugins/runtime/plugin-context.ts) |
| ANN06 | 读取/删除 ask 问题轨迹 / 实装 | 接通：get_annotations / delete_annotation+批准 | 部分：可 list ask，未导出 removeAsk | Agent；注释视图；同一领域 removeAsk 有实现但 plugin-context 省略 | [D3](#D3) | [ANNOT](../apps/web/src/domain/annotations.ts) [ANNTOOLS](../packages/agent/src/tools/annotation-tools.ts) [CTX](../apps/web/src/features/plugins/runtime/plugin-context.ts) |
...
| ANN08 | 按 ID 读取、分页、批量/版本冲突标注操作 / 部分 | 部分：全量 list 后找 ID；可循环单项 | 部分：list+单项命令，无 ID getter/游标/CAS | 现有列表与单项动作；底层 annotation_get 已有；整个行要求的批次/版本协议未有 | [D3](#D3) | [ANNDB](../apps/web/src/features/annotations/lib/annotation-db.ts) [ANNOT](../apps/web/src/domain/annotations.ts) [ANNTOOLS](../packages/agent/src/tools/annotation-tools.ts) [API](../packages/plugin-types/src/index.ts) |
...
| UI02 | 书架搜索/布局/排序/分组/多选 / 实装 | 部分：list_books 查数据，不改变视图 | 未接：无正式入口 | 书架；命令面板；数据查询不是切换用户当前列表；多选不是用户授权批量删除 | [D5](#D5) · [S3](#S3) | [SHELF](../apps/web/src/features/shelf/lib/shelf-view.ts) [SHELFUI](../apps/web/src/features/shelf/components/Shelf.tsx) [COMMAND](../apps/web/src/features/command/lib/build-commands.tsx) |
...
| UI04 | 快捷键查询、重绑、冲突与重置 / 实装 | 未接：无正式入口 | 部分：贡献默认 shortcut，无重绑查询 | 快捷键设置页；插件命令；默认快捷键不是用户实际绑定；菜单快捷键与原生快捷键需一致 | [D5](#D5) · [C1](#C1) | [SHORTCUT](../apps/web/src/features/settings/lib/shortcuts.ts) [SHORTUI](../apps/web/src/features/settings/sections/ShortcutsPanel.tsx) [API](../packages/plugin-types/src/index.ts) |
...
| CFG01 | 设置 discover/read/update 与动态选项 / 实装 | 接通：get_settings/update_settings | 接通：settings.queries.discover/read + commands.update | Agent；Theme Schedule；TTS options；路径、scope、可写性运行时发现；设置 API 接通不证明值有消费者 | [D5](#D5) · [V2](#V2) | [SETTINGS](../apps/web/src/domain/settings/catalog.ts) [SETDOMAIN](../apps/web/src/domain/settings/domain.ts) [SETTOOLS](../packages/agent/src/tools/settings-tools.ts) [CTX](../apps/web/src/features/plugins/runtime/plugin-context.ts) |
...
| CFG04 | 阅读对齐 reading.textAlign / 实装 | 未接：无正式入口 | 未接：无正式入口 | 阅读设置/渲染；实际字段存在但 settings catalog 缺失 | [D5](#D5) · [V2](#V2) | [PREFS](../apps/web/src/features/settings/lib/reader-settings.ts) [SETTINGS](../apps/web/src/domain/settings/catalog.ts) [READER](../apps/web/src/features/reader/components/FoliateReaderView.tsx) |
| CFG05 | 固定版式颜色 reading.fixedLayoutColor / 实装 | 未接：无正式入口 | 未接：无正式入口 | 固定版式外观；不是 reading.theme 的同义项；catalog 缺失 | [D5](#D5) · [V2](#V2) | [PREFS](../apps/web/src/features/settings/lib/reader-settings.ts) [SETTINGS](../apps/web/src/domain/settings/catalog.ts) [READER](../apps/web/src/features/reader/components/FoliateReaderView.tsx) |
| CFG06 | 更新内容弹窗 general.whatsNewDialog / 实装 | 未接：无正式入口 | 未接：无正式入口 | GeneralPanel / useWhatsNewDialog；开关实际有效，但 catalog 漏项 | [D5](#D5) · [V2](#V2) | [GENERAL](../apps/web/src/features/settings/lib/general-settings.ts) [WHATSNEW](../apps/web/src/features/update/hooks/useWhatsNewDialog.ts) [SETTINGS](../apps/web/src/domain/settings/catalog.ts) |
...
| CFG10 | 设置变化事件/外部写入刷新 / 部分 | 自动：每次读当前目录/值 | 部分：settings events + storage.onChange | Theme Schedule；插件设置视图；GAP03/09/11：失败回滚、远端写与异步 callback 仍未闭合 | [D5](#D5) · [V2](#V2) · [Q3](#Q3) | [SETDOMAIN](../apps/web/src/domain/settings/domain.ts) [CTX](../apps/web/src/features/plugins/runtime/plugin-context.ts) [WORKER](../apps/web/src/features/plugins/runtime/plugin-sandbox.worker.ts) [KV](../apps/web/src/platform/local-store.ts) |
| CFG11 | 聊天/笔记内容字体：跟随阅读或独立字号/字体/行距 / 实装 | 未接：无正式入口 | 未接：无正式入口 | AppearancePanel；聊天、笔记、插件 Markdown 与 composer；followReader/fontFamily/fontSize/lineSpacing 未进 catalog；跟随全局 reader 偏好而非本书 override；fontFamily=null 是应用字体选项，不是删除覆盖 | [D5](#D5) · [V2](#V2) | [TYPOGRAPHY](../apps/web/src/features/settings/lib/content-typography.ts) [TYPOGRAPHYUI](../apps/web/src/features/settings/sections/AppearancePanel.tsx) [TYPOGRAPHYEFFECT](../apps/web/src/features/settings/hooks/useContentTypography.ts) [SETTINGS](../apps/web/src/domain/settings/catalog.ts) |
| CFG12 | 新标注默认颜色 / 实装 | 未接：无正式入口 | 未接：无正式入口 | 一键高亮/下划线；recolor 更新后续默认色；createHighlight 指定当前标注颜色不等于修改下一次默认色；yellow/green/blue/pink，默认 yellow | [D5](#D5) · [V2](#V2) | [MARKPREFS](../apps/web/src/features/annotations/lib/annotation-prefs.ts) [TEXTACTIONS](../apps/web/src/features/reader/hooks/useReaderTextActions.ts) [SETTINGS](../apps/web/src/domain/settings/catalog.ts) |
| CFG13 | 软件更新通道 stable/beta / 实装 | 未接：无正式入口 | 未接：无正式入口 | AboutPanel；软件更新查询；设备本地，不漫游；修改通道不等于批准下载、安装或重启；当前 catalog 缺失 | [D5](#D5) · [V2](#V2) | [UPDATECHANNEL](../apps/web/src/features/update/lib/update-channel.ts) [ABOUT](../apps/web/src/features/settings/sections/AboutPanel.tsx) [UPDATE](../apps/web/src/features/update/lib/software-update.ts) [SETTINGS](../apps/web/src/domain/settings/catalog.ts) |
...
| SET23 | ai.preferences.buildMemory / 部分 | 部分：get_settings/update_settings | 部分：settings discover/read/update（需路径授权） | 仅设置页/设置存储/目录；效果未接；保存值有实现；全生产源码扫描未找到对应效果消费者。不能算行为已实现或端到端覆盖。 | [D5](#D5) · [V2](#V2) | [SETTINGS](../apps/web/src/domain/settings/catalog.ts) [SETDOMAIN](../apps/web/src/domain/settings/domain.ts) [SETTOOLS](../packages/agent/src/tools/settings-tools.ts) [AIPREFS](../apps/web/src/features/settings/lib/ai-preferences.ts) |
...
| AI06 | 一次性文本/结构化/流式 LLM 推理 / 实装 | 自动：Runtime ask + 线程推理 | 接通：services.llm.ask fast/smart + schema/onText | Dictionary；Agent 后台管线；流式回调存在，但无公开 AbortSignal/任务预算/用量回执 | [S6](#S6) | [RUNTIME](../packages/agent/src/runtime/runtime.ts) [CTX](../apps/web/src/features/plugins/runtime/plugin-context.ts) [API](../packages/plugin-types/src/index.ts) [DICT](../plugins/dictionary/src/index.ts) |
...
| AI11 | 每轮上下文 provider / 实装 | 自动：plugin context provider 注入线程 | 接通：agentContextProviders.register | 宿主已接线；本仓六插件未注册该类；无消费者是 adoption 缺口，不是 host 未实现 | [C4](#C4) | [EXTOOLS](../apps/web/src/features/plugins/runtime/plugin-tools.ts) [THREAD](../packages/agent/src/runtime/thread.ts) [API](../packages/plugin-types/src/index.ts) |
...
| MEM03 | 轮后抽取/去重/强化记忆 / 实装 | 自动：thread 轮后抽取与 reinforce | 部分：memoryCandidateProviders.propose | Agent 后台；本仓无候选 provider 消费者；自动管线已实装；buildMemory 开关未接消费，见逐项设置 | [D6](#D6) · [C4](#C4) | [THREAD](../packages/agent/src/runtime/thread.ts) [MEMORYPORT](../apps/web/src/features/ai/agent/ports/memory-port.ts) [API](../packages/plugin-types/src/index.ts) |
...
| EXT02 | 书架与阅读 header menu 入口 / 实装 | 未接：无正式入口 | 接通：headerActions.register(surface=shelf/reader) | Dictionary/RSS 书架入口；Jumper 阅读入口可用；Jumper 缺口不是 header 插槽，而是 TXT/D/任务语义 | [C1](#C1) | [API](../packages/plugin-types/src/index.ts) [CTX](../apps/web/src/features/plugins/runtime/plugin-context.ts) [MENU](../apps/web/src/features/menus/lib/menu-registry.tsx) [RSS](../plugins/rss-reader/src/index.ts) [DICT](../plugins/dictionary/src/index.ts) |
| EXT03 | 插件页面/对话框/视图结果与栈导航 / 实装 | 未接：模型不渲染 PluginView 树 | 部分：PluginViewResult push/pop/replace/close | Dictionary/RSS；当前可用，但 GAP06/10 的 callback 生命周期和过期结果未闭合 | [S3](#S3) · [V1](#V1) | [API](../packages/plugin-types/src/index.ts) [VIEWS](../apps/web/src/features/plugins/lib/plugin-view.ts) [RENDER](../apps/web/src/features/plugins/components/PluginViewRenderer.tsx) [WIRE](../apps/web/src/features/plugins/runtime/plugin-worker-host.ts) |
| EXT04 | 列表、搜索、详情、Markdown、blocks 组合 / 实装 | 未接：无正式入口 | 接通：PluginView list/detail/markdown/blocks | Dictionary/RSS；宿主组件有 Table/Tree 不等于插件 schema 有；复杂数据视图另列 | [S3](#S3) · [V1](#V1) | [API](../packages/plugin-types/src/index.ts) [RENDER](../apps/web/src/features/plugins/components/PluginViewRenderer.tsx) [DICTVIEWS](../plugins/dictionary/src/views.ts) [RSSVIEWS](../plugins/rss-reader/src/views.ts) |
| EXT05 | 表单输入、动态选项、验证与提交 / 实装 | 部分：ask_user 固定 question 交互 | 接通：PluginFormView.onSubmit + submitMode=explicit/change | RSS 订阅/OPML粘贴；Dictionary；OPML 是文本粘贴而非通用选择文件服务 | [S3](#S3) · [V1](#V1) | [API](../packages/plugin-types/src/index.ts) [RENDER](../apps/web/src/features/plugins/components/PluginViewRenderer.tsx) [DICTVIEWS](../plugins/dictionary/src/views.ts) [RSSVIEWS](../plugins/rss-reader/src/views.ts) |
...
| SYS01 | 插件隔离 KV 同步读写镜像 / 部分 | 扩展：通过插件工具间接使用，无 KV 工具 | 部分：storage.get/set/remove/onChange | 全部有设置/状态插件；GAP02/03：void 写和 worker mirror 不能证明持久成功 | [S1](#S1) | [CTX](../apps/web/src/features/plugins/runtime/plugin-context.ts) [WORKER](../apps/web/src/features/plugins/runtime/plugin-sandbox.worker.ts) [KV](../apps/web/src/platform/local-store.ts) [DICT](../plugins/dictionary/src/index.ts) |
...
| SYS03 | 插件 schema 迁移/快照/更新回滚 / 部分 | 未接：宿主管理安装生命周期 | 部分：migrate storage-only；host snapshot/restore | RSS legacy feeds 迁移；插件更新；GAP01/02：快照时序与写持久屏障不完整 | [S1](#S1) | [HOST](../apps/web/src/features/plugins/runtime/plugin-host.ts) [WIRE](../apps/web/src/features/plugins/runtime/plugin-worker-host.ts) [WORKER](../apps/web/src/features/plugins/runtime/plugin-sandbox.worker.ts) [RUST](../apps/desktop/src-tauri/src/lib.rs) |
...
| SYS06 | 原生网络 HTTP 请求与响应 / 实装 | 内部：推理端口/插件工具，无通用 fetch 工具 | 部分：services.network.fetch | RSS/TTS/WebDAV；GAP04/05：Request 语义丢失、AbortSignal 不端到端；headers/二进制也需契约测试 | [S5](#S5) | [HTTP](../apps/web/src/platform/http-client.ts) [CTX](../apps/web/src/features/plugins/runtime/plugin-context.ts) [WIRE](../apps/web/src/features/plugins/runtime/plugin-worker-host.ts) [WORKER](../apps/web/src/features/plugins/runtime/plugin-sandbox.worker.ts) [RSS](../plugins/rss-reader/src/index.ts) [TTS](../plugins/tts/src/index.ts) |
...
| SYS10 | 保存文本/二进制文件与取消回执 / 实装 | 未接：无正式入口 | 接通：ui.exportFile(filename,content,mimeType) → boolean | Dictionary CSV；原生保存；已有取消 boolean，不应误报完全缺文件输出；没有流式 FileRef | [S4](#S4) | [EXPORT](../apps/web/src/platform/export-file.ts) [CTX](../apps/web/src/features/plugins/runtime/plugin-context.ts) [API](../packages/plugin-types/src/index.ts) [DICTEXPORT](../plugins/dictionary/src/export.ts) |
...
| OPS04 | WebDAV 等自定义密文 transport / 实装 | 部分：可改非敏感插件设置，不能连接 | 接通：syncTransports.register/open/session methods | webdav-sync；GAP14：session 缺 close/dispose，不能据 register.dispose 推断连接释放 | [C5](#C5) · [S9](#S9) | [API](../packages/plugin-types/src/index.ts) [WEBDAV](../plugins/webdav-sync/src/index.ts) [SYNCTRANSPORT](../apps/web/src/platform/sync/transport-registry.ts) |
...
| CON03 | 生命周期 staging/activate/deactivate 与资源释放 / 部分 | 自动：runtime invalidation/flush background | 部分：lifecycle staged contributions/disposables | 插件启停/升级；Agent 运行时重建；GAP06/08/14：callback、已发出的 host effect、transport session 不能全被关闭 | [Q2](#Q2) | [HOST](../apps/web/src/features/plugins/runtime/plugin-host.ts) [CTX](../apps/web/src/features/plugins/runtime/plugin-context.ts) [WIRE](../apps/web/src/features/plugins/runtime/plugin-worker-host.ts) [THREAD](../packages/agent/src/runtime/thread.ts) |
| CON04 | 跨 Worker RPC 的类型、错误与资源额度 / 部分 | 扩展：插件 tool 也经过同一 worker bridge | 部分：describeContext/encode/decode/invoke | 所有 Worker 插件及其 Agent 工具；GAP07/12/13/17：超时/崩溃、__fn 碰撞、解码配额、错误码漂移 | [Q2](#Q2) | [WORKER](../apps/web/src/features/plugins/runtime/plugin-sandbox.worker.ts) [WIRE](../apps/web/src/features/plugins/runtime/plugin-worker-host.ts) [API](../packages/plugin-types/src/index.ts) [ERRORS](../packages/core/src/errors.ts) |
...
| CON09 | 沙箱、权限撤销和 packaged CSP 验证 / 部分 | 扩展：插件工具间接承受同样沙箱风险 | 部分：Worker + API gate + CSP | 安装信任边界；GAP16 仍需真实 packaged Tauri 验证；既不宣称已越狱，也不宣称完全隔离 | [Q1](#Q1) | [WORKER](../apps/web/src/features/plugins/runtime/plugin-sandbox.worker.ts) [WIRE](../apps/web/src/features/plugins/runtime/plugin-worker-host.ts) [RUST](../apps/desktop/src-tauri/src/lib.rs) [HOST](../apps/web/src/features/plugins/runtime/plugin-host.ts) |
...
| MORE03 | 环境 locale/platform/timezone/在线/ready 快照 / 部分 | 自动：prompt 语言/日期与运行态分别注入 | 部分：ctx.locale/appVersion/manifest/capabilities | 插件本地化；Agent prompt；无统一平台/时区/在线/支持格式/就绪信息；读取 locale 并不等于订阅全部环境变化 | [S11](#S11) | [CTX](../apps/web/src/features/plugins/runtime/plugin-context.ts) [THREAD](../packages/agent/src/runtime/thread.ts) [RUST](../apps/desktop/src-tauri/src/lib.rs) [API](../packages/plugin-types/src/index.ts) |
...
[环境] 本轮没有运行所有能力的真实 Tauri E2E、packaged CSP、跨设备同步、远端账号/支付/模型服务或用户插件安装态；没有修改宿主实现，没有实现 Jumper。静态 HTML 采用模板固定 CDN 字体/图标等资源，需要网络；无 Mermaid 图。文档浏览器验证只证明文档能读，不能证明产品能力。
