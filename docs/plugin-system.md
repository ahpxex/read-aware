...
> **状态：** 方向已定（2026-07-21），实现未开始。
> 基于产品讨论与代码调研。参照模型：**Obsidian** 定插件的域与权限层
...
| 权限域 | 授予的能力面 |
|---|---|
| `reading-data` | 读书籍元数据、标注、笔记（只读；写入走各自的显式 API 再议） |
...
- 顶栏可见插件按钮设上限；未钉选的收进"更多"溢出菜单。
- 选区菜单空间更小：插件动作**默认进溢出**，用户在设置里手动提拔
  最常用的几个到一级。
...
- **TypeScript 类型包**：给插件作者发一个 `@read-aware/plugin-types`
  （只有类型，无运行时），降低写插件门槛；随阶段 5 一起出。
