...
第四种是**组合式 `blocks` 视图**：一串有序块（markdown、heading、
dictionary 词条、keyValue、quote、actions 按钮行、divider、嵌套
list/form）。这是词汇的增长路径——更丰富的界面靠新增块类型，不靠插件
自绘 UI。

**布局：受约束的 `row` 块，不是裸 flex。** 插件确实需要水平排布（统计
一行、并排卡、出处与引文并列），但开放裸 flexbox 会让插件重新拿回"随便
画界面"的自由、砸掉"观感由应用统治"。折中是一个受约束的 `row` 块：
2–4 个 cell 并排，每个 cell 装一个普通块，`weight` 定相对列宽（默认 1）。
间距、垂直对齐、以及**窄容器下自动降级为纵向堆叠**都由设计系统掌管——
按容器真实宽度降级，不看视口。cell 内的块不能再是 `row`（只一层深，杜绝
递归失控）。「布局能力」到此为止：够表达并排，不够让插件重画版式。
