...
4. **设备本地数据与呈现层由宿主掌管。** 视图偏好（书架布局/分组/排序）、
   ...
   （§6）。Reader Mode 也不例外：插件只对宿主提供纯文本 offset 分段策略，
   iframe DOM、`Range`、CFI 映射、Foliate 实例、输入接管、持久化与全部控件
   仍由应用持有。「自由但非无限」的轴线始终是：**数据域按权限开放，呈现
   与引擎域不开放**。
...
Reader Mode 同样遵守这条规则。`text-unit-navigator` 插件只返回每个宿主
文本 block 内的 `{start, end}` 半开区间；宿主校验有序、非重叠与边界后才
映射成 DOM Range。逐句遮罩、跨章节步进、滚轮/触摸/键盘/Android 音量键、
标注/查词/Ask AI 动作和设置界面全部是 ReadAware 自己的组件与引擎代码。
