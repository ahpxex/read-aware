...
-- 设计原则：SQLite 只保存结构化事实、设备本地配置、同步状态、blob 清单和 AI 记忆元数据；LanceDB 只保存可重建的向量索引。
...
--
-- 实施现状（implementation status）：本 schema 是目标设计，尚未落地（当前持久化仍是 IndexedDB+localStorage）。
--   memory/entity/vector/context 子系统（user_profile、entities、entity_aliases、memories、memory_evidence、working_memory、
--   context_bundles、context_bundle_items、vector_documents、embedding_jobs）以及 sync_* 远端中继表，目前都还**没有生产者**：
--   它们随数据模型一起建好（可空表），等 consolidation/embedding/同步管线落地时再启用。先做数据模型，不急着接云服务。
--
...
-- 注意：foreign_keys 是 per-connection 开关，不会持久化在数据库文件里。下面这行只对执行迁移的那条连接生效；
-- StorageAdapter 必须在**每一条**新建连接的 open hook 里重新执行 `PRAGMA foreign_keys = ON`，并在启动自检里断言它，
-- 否则本文件里所有 ON DELETE CASCADE / SET NULL / DEFERRABLE 约束都会静默失效。这行因此主要是文档性质。
...
CREATE TABLE settings ( -- [device-local] 设备本地的全局偏好单行表；对应 Settings 页面多个 section，不进 domain_events，也不跨设备同步。
  ...
  auto_update INTEGER NOT NULL DEFAULT 1, -- 在设置页面 General > Desktop integration 中，控制桌面 app 是否后台下载并安装新版本。
...
CREATE TABLE books ( -- [projection] 书籍读模型；由 book.imported/book.metadataEdited/book.coverExtracted/book.starred/book.removed 等事件重放生成，是书架核心。
  ...
  source_blob_key TEXT NOT NULL REFERENCES blob_objects(key) DEFERRABLE INITIALLY DEFERRED, -- 原始书籍文件的 blob key；ReadAware 直接读原文件，不做格式转换。
...
CREATE TABLE reading_time_totals ( -- [projection] 每本书累计主动阅读时长；替代当前 read-aware-reading-stats 的 totalMs/firstStartedAt/lastReadAt。由 reading.timeRecorded 重放。
...
CREATE TABLE reading_time_daily ( -- [projection] 每本书按本地日聚合的阅读时长；用于周/月统计和 heatmap。
  ...
  local_day TEXT NOT NULL, -- 本地日 YYYY-MM-DD；和当前 reading-stats 的 daily key 一致，避免时区换算影响图表。
...
CREATE TABLE reading_time_hourly ( -- [projection] 每本书按本地小时聚合的全量阅读时长；替代当前 byHour 24 槽数组。
  ...
  local_hour INTEGER NOT NULL CHECK (local_hour BETWEEN 0 AND 23), -- 本地小时 0-23；用于 RhythmCharts 展示一天中的阅读节律。
...
CREATE TABLE memories ( -- [projection] 长期记忆；AI 记忆的正文和检索信号在 SQLite，embedding 只放到 LanceDB。由 memory.promoted/revised/superseded/feedback/forgotten 重放。前瞻，暂无生产者。
