...
CREATE TABLE plugin_documents ( -- [device-local] 插件文档集合：插件的结构化私有数据（KV 之上、核心域之下的一层）。生命周期归插件（卸载即清）；book_id/anchor 是可选出处索引（无书籍级联——删书后文档存活，出处只是引用不是归属）。桌面迁移 v10 已落地。
  plugin_id  TEXT NOT NULL, -- 所属插件 id；命名空间隔离，跨插件不可读。
  collection TEXT NOT NULL, -- 集合名（插件自定，如 words、feeds）。
  id         TEXT NOT NULL, -- 文档 id（插件自定身份，如 `<language> <term>`）。
  json       TEXT NOT NULL, -- 文档内容 JSON。
...
CREATE TABLE vocabulary_entries ( -- [retired] 已退役：生词本随 Dictionary 内置插件迁入 plugin_documents（dictionary/words 集合，启动一次性搬迁后本表清空保留）。vocabulary.* 事件随之从目录移除（发生在任何 release 之前，无重放负担）。
