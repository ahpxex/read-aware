...
CREATE TABLE vocabulary_entries ( -- [projection] 生词本：阅读器词典保存的词；由 vocabulary.added/vocabulary.removed 事件重放（added 为 upsert 语义——同一 id 重加覆盖快照）。当前 interim 存储为 localKV read-aware-vocabulary，事件已双写。
