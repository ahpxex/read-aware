...
<section id="boundaries"><h2>验证边界</h2><p>${rows.length} 条证据、${catalog.size} 个当前 catalog 成员、129 个旧验收项、32 个旧场景都有模型归属；不等于所有未知行为零遗漏。</p><p>已陆续实现共享能力和 Jumper 等组合插件，局部桌面证据见执行账本；尚未完成全能力 Tauri、完整打包 CSP、跨设备同步、外部服务和用户插件安装态验收。</p><p>原 GAP01–GAP18 仍未关闭。静态文档浏览器检查不能替代产品验收；固定 CDN 字体/图标资源需要网络。本页无 Mermaid 图。</p></section>`;
