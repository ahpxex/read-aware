...
let revision = 1, unavailable = false, requests = 0;
const server = Bun.serve({ hostname: "127.0.0.1", port: 19846, async fetch(request) {
  const url = new URL(request.url);
  if (url.pathname === "/state") return Response.json({ revision, unavailable, requests });
  if (url.pathname === "/control" && request.method === "POST") {
    const value = await request.json() as { revision?: number; unavailable?: boolean };
    ...
    return Response.json({ revision, unavailable, requests });
  }
