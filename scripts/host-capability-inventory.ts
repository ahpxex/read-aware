...
const agentMap = pairs([
  ...
  ["get_reading_session", "READ07 TXT09 READ16"], ["navigate_reading", "READ02 READ04 READ06"],
...
const pluginMap = pairs([
  ["domains.settings.queries.discover domains.settings.queries.read domains.settings.commands.update", "CFG01"],
...
const catalogMap: Record<string, Record<string, string[]>> = {
  ...
  services: { storage:["SYS01","SYS02"], secrets:["SYS04"], ui:["EXT07","SYS10"], schedules:["MORE01"], session:["READ08"], network:["SYS06"], llm:["AI06"], clipboard:["SYS08"] },
...
export function collectInventory(): Inventory[] {
  ...
  for (const roster of ["LIBRARY_EVENTS","READING_EVENTS","ANNOTATION_EVENTS","CONVERSATION_EVENTS"]) {
    ...
    visitString(namedInitializer("apps/web/src/domain/events.ts",roster));
  }
  ...
  const expectedPlugins = pairs([["dictionary", "EXT09 AI12"],["rss-reader", "EXT10"],["editorial-themes", "EXT08"],["sentence-reader", "READ15 READ16"],["tts", "READ17 READ18"],["webdav-sync", "OPS04"],["jumper", "TXT02 TXT07 READ06 EXT02"],["annotation-desk", "ANN01 ANN04 ANN05 ANN08 EXT02 EXT05 SYS10"],["listening-desk", "READ16 READ18 READ06 EXT02"]]);
